...
//
// ICDEV_DASHBOARD_PORT is read by resolveBaseUrl() in
// tests/e2e/fixtures/base_url.ts, which is where `DASHBOARD_URL` below now comes
// from — so the port and the expression that builds the URL from it stay one
// definition.
...
//
// The expression itself lives in tests/e2e/fixtures/base_url.ts and is imported
// here, NOT restated (qa-fail-0d954757a83824da). It was restated in
// fixtures/auth.ts and fixtures/govcon_cpmp.ts, both without the
// ICDEV_E2E_BASE_URL leg — so a run with both variables set bootstrapped CSRF
// against 127.0.0.1 and sent the specs' own requests to localhost, two cookie
// jars on one server, and every mutating request 403'd CSRF_FAILED. See that
// file.
...
const config = defineConfig({
  ...
  // set ICDEV_DASHBOARD_PORT to get an isolated server instead (see the port
  // comment above).
