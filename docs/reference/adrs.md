...
### Phase 79 — EXA (External Adoption)

- **D394:** **ICDEV's primary agent adapter runs Claude Code with the vendor permission system disabled, and ICDEV owns the replacement.** `ClaudeCliAdapter.build_argv` (`tools/agents/adapters/claude_cli.py`) passes `--dangerously-skip-permissions`. This is deliberate, not incidental: the adapter exists for unattended dispatch (`spawn()` for the kanban runner, `invoke()` for the adversarial verifier and `pr_watcher`), where a permission prompt is not a control but a hang; a vendor keystroke satisfies none of AC-3/AC-6/AU-2/AU-12/CM-5, having no audit row, no impact level and no role; and a safety story living inside one vendor's prompt does not transfer across the `AgentAdapter` seam to `codex_cli`, `copilot_cli`, `local_agent` or a future Goose adapter. **The premise correction that came with measuring it:** `approval_gate.py` and `agent_tool_gate.py` are NOT the compensating controls for this adapter — both are hooks inside ICDEV's own in-process agent loop, and `claude_cli` `Popen`s a *separate* Claude Code process that never imports either. The only ICDEV code observing a tool call in that subprocess is `.claude/hooks/pre_tool_use.py`, constrained by `args/file_access_tiers.yaml`. `.claude/settings.json`'s `permissions.deny` list (`rm -rf *`, `git reset --hard*`, `git push --force*`, `DROP TABLE*`, branch deletes, forced worktree removes) is a precise inventory of what the flag gives up, because the permission system is what evaluates it. Full write-up and the measured four-category coverage matrix: [docs/security/agent-vendor-permission-bypass.md](../security/agent-vendor-permission-bypass.md); standing evidence: `tests/test_skip_permissions_compensating_controls.py` (exa-bench-04).

- **D395:** **Reversibility is the wrong axis for two of the four categories, and the gaps are carded rather than papered over.** Measured against `a3b4bcaba`: destructive-shell and network-egress are covered on the in-process surfaces; **write-outside-worktree and credential-access are not covered anywhere**. Three structural reasons, each recorded so nobody re-derives them. (1) `read_file` is tier `reversible` in `args/agent_approval_policy.yaml` and `classify()` rule 0 exempts a `reversible` tool from content escalation — correctly, since that exemption is what stopped `read_file("how do I git push safely")` prompting — but it means **no argument can ever escalate a read**, so a private key classifies identically to a docstring. A read is perfectly reversible and completely unrecoverable at once, and the tier vocabulary has no word for that. (2) `write_file` is `recoverable` on the rationale that git restores it, but git only restores paths *inside the repo*; there is no worktree-containment check on any surface, so `/etc/cron.d/pwn` and `~/.bashrc` are recoverable by nothing. (3) Egress is caught on the in-process surfaces only because `default_tier: unknown` halts, not because anything models the network. Follow-ups: `exa-bench-05` (critical — `.claude/settings.json` wraps the hook in `|| true`, so its exit-2 blocks return 0 and every hard block on the one surface guarding a skipped-permissions session is advisory; measured, and deliberately NOT fixed inside exa-bench-04 because that file is live for every concurrent session on the host), `exa-bench-06` (the Claude Code hook runs 9 of the 10 checks `hook_compat.HEADLESS_CHECKS` runs — `check_git_danger` is missing — and `_REDIRECT_TARGET_RE` captures `>` from a `>>` append so the file tiers miss it), `exa-bench-07` (no path boundary on writes), `exa-bench-08` (no egress control in the `claude_cli` path), `exa-bench-09` (credential reads are a glob list with holes, and `read_file` is unbounded by path on both in-process surfaces).
