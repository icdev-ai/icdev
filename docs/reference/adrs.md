...
### Phase 79 — FLX (Floci Cloud Emulator)

Replace LocalStack outright — across the DataBridge feature flag, the Studio IaC executors and the twelve canvas sim topologies — with **floci** (MIT, Java/Quarkus, port 4566, `2.0.1`), behind ONE seam, and then give the emulator a governed DataBridge door, a Twin Observatory adapter, an air-gap image bundle and the opt-in IaC CI gate `twx-spk-01` called pattern 1. Supersedes `docs/spikes/twx-spk-01-localstack-go-no-go.md` **on the licensing question only** — by a DATED ADDENDUM appended to that spike, never a rewrite. See `docs/features/phase-flx-floci-emulator.md`.

- **D398:** **The spike was right, the FACT moved, and the record shows both.** `twx-spk-01` ruled LocalStack NO-GO for air-gapped/classified because its 2026 single authenticated image validates an **auth token at container start**, and priced commercial CI per seat. That reading was correct and both facts still hold — of LocalStack. floci is a documented drop-in (keeps `/_localstack/health`, translates `LOCALSTACK_*` by default, consumes the identical stock `hashicorp/aws` provider shape) that validates **no token** and carries **no subscription**, so the spike's *conclusion* no longer follows while its *reasoning* stands untouched. The spike is therefore **appended to, not edited**: a rewritten spike destroys the reasoning that made the original call defensible, and a reader who cannot see the superseded verdict cannot tell a decision that was REVERSED from one that was NEVER MADE. **D382's LocalStack half is superseded on the licensing question ONLY** — its Batfish half (`twx-spk-02`) is untouched, and every non-licensing LocalStack finding STANDS: the Docker footprint still conflicts with this repo's pure-Python/offline preference (§2), Cloud Pods stays DEFER (our fixtures are already pure-Python and floci ships no equivalent), chaos injection stays NO-GO, and the **IAM-sandbox NO-GO and the performance-claim guard carry forward UNCHANGED** (D401). The spike's own follow-up `twx-ls-01` — an opt-in CI job requiring human sign-off on a paid subscription — is superseded by `flx-ci-01`, which builds that job with no subscription to sign off.

- **D399:** **ONE emulator switch, and the contradiction case is refused rather than guessed.** ICDEV carried TWO unrelated switches for the same emulator and neither knew the other existed: `databridge/feature_flags.py::localstack()` read `LOCALSTACK_ENABLED` and told the operator to run a compose profile `docker-compose.yml` had never declared, while `studio/executors/_base.py::detect_mode()` returned a mode on the bare PRESENCE of `LOCALSTACK_ENDPOINT` — and *that* is the switch under `terraform apply`. So the flag could be off while the executor ran against an emulator, or on while nothing was reachable. `tools/cloud/emulator.py` (flx-seam-01) is now the one answer and both existing switches delegate to it; `LOCALSTACK_ENABLED/_ENDPOINT/_REGION` stay readable as **deprecated aliases**, so an operator's existing configuration is not silently ignored. Three decisions inside the seam are load-bearing. (1) **An endpoint declared while the switch is OFF degrades to `dry_run`** — plan only. Falling through to `aws` on ambient credentials would send an apply written for a local emulator at a REAL account; neither reading is defensible, so nothing is applied. (2) **Credentials are always `test`/`test`, never the ambient AWS pair** — these values reach `docker run -e` and a Terraform provider block, the emulator accepts any non-empty pair, so honouring a real `AWS_ACCESS_KEY_ID` buys nothing and hands live GovCloud keys to a container talking to localhost. (3) **`docker_backed()` is TRI-STATE.** Measured on this host 2026-09-04, Docker Desktop 28.5.1 was RUNNING while `os.path.exists(r"\.\pipe\docker_engine")` returned False — a plain existence check reports a definite absence for a working daemon. Windows-with-no-`DOCKER_HOST` is `None` (cannot tell) and PERMITS the call; only a PROVEN absence refuses, because refusing on an unproven absence fabricates a refusal — the mirror image of fabricating an empty list.

- **D400:** **Air-gap capability is a MEASURED claim, not a licence claim.** Removing the auth token makes disconnected operation *possible*; it does not make it *true*, and three measurements say what it actually costs. (1) **Having `floci/floci:2.0.1` cached is NECESSARY AND NOT SUFFICIENT** — floci does not carry its container-backed runtimes inside its own image; Lambda, RDS, ElastiCache, OpenSearch, MSK and ECS/EC2/EKS each start a SEPARATE container from a base image resolved **from the public internet on first use**. Eleven such images were recorded from `docker events` while a live floci was driven through every container-backed service (2026-09-05, Docker 28.5.1) into `args/floci_runtime_images.yaml` — MEASURED, never read off a README — and the set is a function of the declared **variant** (a `python3.11` Lambda and a `nodejs20.x` one pull different images), not of the service, which is what makes a bare per-service list wrong. (2) **A pin is a DIGEST, never a tag** (`flx-airgap-01`): `tools/airgap/image_vendor.py` saves from the LOCAL cache and never pulls (structurally — an allowlisted `version|image|save|load` frozenset, AST-asserted), and verifies a bundle by re-hashing every OCI blob against its filename and matching `index.json` to the pin, with **no daemon required**, because media is verified before there is anywhere to load it. `unmeasured` exits 2 and is never a clean bundle. (3) A registry-mandating site cannot pre-seed hosts, so `flx-airgap-03` extends the ONE `airgap-emulator-runtime-images` rule with a second way to answer no — the pull is redirected to an **internal** mirror, judged against `args/twin_airgap_rules.yaml`'s own allowlist rather than a private copy. **There is deliberately no second rule**: two rules could disagree about what a run-time pull is, leaving a reviewer with two verdicts and no way to choose. Separately, the **host Docker socket is ROOT-EQUIVALENCE ON THE HOST** and ships as an explicit security decision — Gap 65 in `docs/security/sandbox-coverage.md`, behind the opt-in `floci` profile, every port bound to `127.0.0.1`, no `env_file`, and a test asserting floci is the ONLY service granted it.

- **D401:** **An emulated estate is never readable as an observed one, and no emulator verdict may be a required check.** Four honesty rules govern every surface this phase added, and each is enforced rather than stated. (1) **Provenance `emulated`** — the `flx-twin-01` adapter reads the connector's seven logical tables THROUGH `tools/databridge/broker.py` as `twin_observatory_analyst` (the `flx-bridge-02` grant), so every read is authorized and audited; a direct connector import is the ungoverned side channel `cef-fnd-03` exists to close, and an AST test refuses one. `_persist_snapshot` takes **no** provenance parameter (asserted structurally, and CHECKed in the database by migration `20260905070028`), because a behavioural test over today's callers would still pass the day somebody threads a kwarg through. (2) **`unknown` is never `pass`, and an unanswerable question is not an empty answer** — a container-backed table on a socket-less host returns `unsupported_without_docker`, never `[]`, which is the `rmf-disc-02` defect exactly; `resource_count` is `None`, never 0, when nothing was measured. (3) **NEVER source a performance, cost or capacity claim from emulator timings** — `twx-spk-01`'s standing guard, carried forward UNCHANGED and now travelling ON the capability (the commands doc, the `flx-ci-01` report path, the `flx-twin-01` manifest row) rather than sitting in a spike nobody re-reads. The **IAM-sandbox NO-GO** carries forward on the same terms: the PDP/PEP ABAC engine already answers IAM questions offline and deterministically, and a partial emulation would give a second answer with no rule for choosing between them. (4) **The IaC gate is opt-in and NEVER one of the four required checks** (`workflow_dispatch` + weekly + a `floci-gate` label): runners here are near-serial, so an emulator in front of every merge is how a gate earns itself a bypass. Its four cells carry ONE finding — `gate_missed_rejection`; `gate_stricter_than_api` is what a compliance gate IS and can never be a finding — and either side unmeasured is `unmeasurable`, never agreement. The **Azure, GCP and OCI siblings are NOT built** (`flx-az-01`, `flx-gcp-01`, `flx-oci-01`, backlog): each is gated on its own **dated parity measurement**, exactly as this spike gated LocalStack, and none inherits floci's AWS parity by sharing its authors.

### Phase 80 — XRV (External Review Adaptations)

Nine external repositories were reviewed and each got one verdict; **eight of the nine gaps turned out to be already-present ICDEV capabilities that nothing consumed**, so the repositories' value was as a list of places to look for this repo's signature defect rather than as a feature list. See `docs/features/phase-xrv-external-review.md`.

- **D402:** **A Claude Code dispatch's cost exists only in its transcript, an ABSENT price is never `$0.00`, and the two MCP usage counts are never merged.** None of the four token ledgers sees a `claude_cli` session: the adapter bypasses `router.invoke`, so every one of those calls carried no cost row, and `agent_token_usage.task_id` held **ZERO** rows on the live board although `_parse_cli_json` had been parsing `total_cost_usd` per dispatch all along and writing it to a task log nothing read back. Three decisions make the repair honest rather than merely present. (1) **The CLI's own `total_cost_usd` is recorded AS REPORTED and never re-priced** — `args/llm_config.yaml` prices no Claude Code model, so re-pricing would substitute our table's silence for the vendor's measurement; an unknown or `0.0/0.0` model is `unpriced` with `cost_usd` **None**, and a mixed bucket is `partial`, a stated lower bound. The direction of the error is what makes this load-bearing: `record_task_cost` originally fell back to `0.0` for an envelope carrying no cost field, so an absent price summed as a zero and **understated the bill in the direction that makes work look cheap**. `row_is_unpriced` now names that class and the count is carried APART from every outcome verdict, because a dispatch that reported no dollars still shipped or was still abandoned — folding it into `unmeasurable` would make the verdict answer a question about the price instead of about the outcome. (2) **"Did the spend ship" is a CLOSED verdict set** — `shipped | reverted | abandoned | in_flight | unmeasurable` — joined through `landed_check` (task -> main, per repo) and `pr_linker`, where an unreadable ref, an unreadable board, and a forge `MERGED` beside a default branch naming no commit (a **disagreement**, never a pick between two) are all `unmeasurable` and never fold. (3) **The two MCP usage figures measure different CALLERS and are printed side by side.** `studio_mcp_dispatch_audit` has exactly two writers, both Studio-internal, and nothing under `tools/mcp/` writes a dispatch row, so **every MCP call made from a Claude Code session is invisible to `capability_consumption --class mcp_dispatch_tool`**: measured on this host over 30 days, the transcripts say 3 of 472 registry tools were reached and the Studio audit says 1. Neither is a correction of the other and neither alone answers "is this tool used", so no probe and no budget was changed (`mcp_dispatch_tool: 467` is untouched) and the server-side audit write — a new writer to an append-only table — is its own card. Measured while building it: attributing a transcript tool name against `TOOL_REGISTRY` **bare** credited **Playwright's** 50 clicks and 176 navigations to ICDEV, because the registry and the Playwright MCP server both declare `browser_click`/`browser_navigate`/`browser_type`; attribution is now server-qualified from the checkout's own `.mcp.json`. The panel that renders all of this is a **section on the existing `/cache-savings` page** and computes no cost of its own (one `survey_by_verdict` call, forge deliberately not consulted, an AST test refusing a private cursor), because a panel with its own query is how a surface comes to disagree with the CLI about a number neither of them changed. Tasks: xrv-cost-01..05.

- **D403:** **Eleven dispatch guards and not one was a budget — so the cap refused the run's calls MID-RUN, and the aggregated verdict ships in `report` because the ledger it would enforce is a cost PROXY.** The scheduler ran six sequential early-return guards inside `_dispatch_to_claude` and five more in `_get_due_tasks`, each with its own log line and no shared record. All four USD guards the platform declares — `token_tracker.check_budget`, `module_budget_tracker.check_module_budget`, `cost_budget.apply_to_chain`, the proxy budgets — sit inside `tools/llm/router.py::invoke`, which a `claude_cli` dispatch never enters, so **every cap was bypassed at dispatch and then refused the work once the tokens were already being spent** (kpr-dup-09 was this shape: 95 `done`<->`backlog` flips in 5.5 hours, every individual transition legitimate). `tools/kanban/should_run.py::assess` folds all ten existing predicates plus the budget rung through ONE pure `classify`, calling each and re-implementing none (AST-pinned: no SQL naming the board table, the admission rule imported, the three budget seams called on the live path), and the circuit breaker's two inline copies of `failure_count >= max_retries` now read one function. **The survey is why `enforce` is off.** Replayed over 7,072 recorded scheduler dispatches through the SHIPPED `classify`: 71.89% `proceed`, **28.11% `wait`** — 0.00% in June and July, 63.00% in August, **100.00% in September** and 1,621 of 1,621 over the 30-day window. And **every fire is the module arm's TOKEN cap; not one is USD**: `module_budget_periods.spent_usd` is 0.0 in every month and `ai_telemetry`'s 13,995 rows sum to $0.00, because every call routes to a $0/1k provider. Corroborated from the writer side — 567 `budget exceeded` refusals in four hours across eight modules in the live NDJSON, 31,608 more in the rotated file, which are exactly the mid-run refusals. So the number says two things pointing at different repairs: the gate was genuinely missing since 2026-08-07, **and** arming it today would park every task on the board until 2026-10-01 — correct against the ledger and a stall against the estate. The repair is the token cap moved after routing or windowed to guard runaways, then a re-survey; **raising `monthly_tokens` to quieten the rung is forbidden**, because this rule is right about the ledger and the ledger is wrong about the cost. Capacity, sibling overlap and the external-repo park are deliberately NOT folded — they are decisions over the candidate SET or facts about where a task builds, not verdicts about one task. Tasks: xrv-run-01..02.

- **D404:** **A memory system that captures nothing and a retrieval that always pays full cost are the same defect twice; both are fixed by WIRING, and `<private>` is dropped before the seam, not filtered after it.** `auto_capture.py` was complete and had **no hook caller** — `memory_buffer` held 0 rows on the live board while `memory_maintenance_reflex` and `heartbeat_daemon` dutifully flushed a buffer nothing filled — and `.claude/settings.json` wired every hook kind **except** `SessionStart`, so the Session Start Protocol this file documents was a manual step nobody ran. Three decisions. (1) **The injected context block is BOUNDED and the bound is reported**: capped at 1,500 tokens by `context_budget.estimate_tokens` and WITHHELD past a 1 s wall clock with `reason: over_budget`, measured live at 0.48–0.54 s and 425 tokens over 20 entries; `tokens` is None, never 0, when nothing was composed. A context hook that can hang is a liveness failure wearing a feature's name, so the budget refuses rather than delays. (2) **Capture is DETERMINISTIC and the kinds are closed** — `edit`, `commit`, `test_summary`, `decision` — with no model call anywhere, and `<private>...</private>` spans are stripped **before** `capture()` is reached (a test asserts a private span never lands in `memory_buffer`), because a filter downstream of the buffer has already written the thing it means to exclude. Measured over a 7-day transcript replay of the shipped predicate: **median 156 rows/day, 144 on the last full day**, mean 232.5 — *quote the median*, the mean is dragged by two multi-session build days, and the 7-to-563 spread is the operator's day and not the predicate's. Cost is +10 ms for a non-capturing event and +280 ms when an event yields an observation (16.6% of events), so the storage import is paid only by the minority that needs it. (3) **Progressive disclosure reuses the dashboard's chronology instead of authoring a second one**: the `audit_trail UNION ALL hook_events` query moved out of the Flask blueprint into `tools/dashboard/api/activity_query.py` and an AST test asserts that UNION is spelled as SQL in exactly one file. Two defects surfaced and were fixed on the way, both invisible without a replay: the pytest summary regex demanded the `===== N passed =====` bars while this repo runs `-q` (4 summaries became **1,140**, with the DURATION stripped so `24 passed in 1.13s` and `1.19s` stop being two facts), and on SQLite `audit_trail`/`hook_events` hold **both** timestamp spellings, so a `T`-form window bound silently excluded every space-form row of the same day. `hook_events.hook_type` now derives from ONE tuple, which found that `user_prompt_submit` and `pre_compact` had been written by two shipped hooks since they were authored and admitted by **no** constraint. Tasks: xrv-mem-01..03.

- **D405:** **The scanners existed; nobody had pointed them at the files that configure the agents — and a brand-new check over an undrained surface registers at `warn`, with the false positives fixed BEFORE arming rather than excused after.** `prompt_injection_detector.scan_project`, `secret_detector.scan` and `mcp_scanner.scan_mcp_servers` had all shipped, and `SKIP_DIRS` never excluded `.claude/`, `.agents/` or `.cursor/` — `scan_project` would have walked them the first time anybody asked, and nobody asked. Pointing them there is `warn`-level in `coherence_checker` (capped in code and asserted by test), full-tier only, re-added to the fast tier exactly when a diff touches the config surface, because a hard refusal on day one over a 227-file surface nobody has drained is how a check earns itself a `|| true`. **Arming it meant fixing the scanner first, and the finding was large:** `role_hijack_jailbreak` spelled its pattern without word boundaries, so every occurrence of `dan` INSIDE a longer word was a CRITICAL jailbreak finding — **10,019 matches across 18,730 files, of which 9,929 (99.10%) were `guidance`, `accordance`, `dangerous`, `redundancy`, `pydantic`**, and 67 of 67 criticals on the config surface itself. Adding `\b` is a strict narrowing (it cannot create a match) and the existing DAN fixture still fires, so the fix is provably not a weakening. Two further defects are handled at the CALLER rather than at the scanner, on purpose: `secret_detector`'s `--exclude-files` regex is **unanchored**, so it silently drops every path containing `build` or `dist` (proven as a controlled pair — the same file with the same planted token yields 1 finding under `.agents/skills/x/` and **0** under `.agents/skills/icdev-build/`), and anchoring it widens what ~20 consumers including the `domain_leak_gate` CI gate report, which owes its own fire-rate survey; so `config-secrets` runs **both scanner arms** and unions the findings, making its live `pass` a measured zero over all 227 files. And `scan_mcp_servers` returns `passed: True` for a config it never found — a clean bill of health for nothing — so the check treats an `error` key as **unmeasurable** and an absent file as **absent**, never as a pass. The same discipline governs the routing epic: `args/tool_index.yaml` is the ONE declared list of the ~36 external binaries this platform shells to, `which(name)` REFUSES an undeclared name (an index that silently answers for anything can never grow), **`present` requires the binary to ANSWER its `version_cmd`** because a `present` verdict that never ran the binary is a claim about a file NAME — measured, two binaries called `helm` are on PATH and only running them settles which one you got — and `unmeasurable` is its own state that never folds into either other; the 195-case corpus over the **four** deliberately separate routers adds **no fifth router** and carries its four discovered defects as `xfail(strict)` cases, red while the defect stands and red the moment it is fixed and the declaration goes stale; and the 69-site pin census is a shrink-only ratchet grandfathered BY NAME, `warn` in coherence, with `requirements.txt`'s 45 ranges **surveyed and not gated** because the install-time pin for a deployment is the vendored wheel set. In every case the predicate is RE-DERIVED each run rather than kept as an exemption list — 21 of 29 compose images are built by this repository and a registry digest has no meaning for them, so the predicate excludes them and no reviewer has to check a list. Tasks: xrv-shield-01..02, xrv-route-01..03.

- **D406:** **Binary analysis is TWO TIERS with TWO declarations, an optional backend that reports its own absence is the whole point, and a version TOKEN is not a version.** ICDEV had zero binary analysis — no PE/ELF parser, no strings, no entropy, no decompiler — while `slsa_verify` and `attestation_verify` verified metadata about an artifact **without ever opening it**, which is precisely what "the binary does not match the SBOM" is about. The socket already existed (`args/analyzer_contract.yaml` + `tools/analyzers/sandbox.py`), so this is two analyzer entries and no dispatcher change. **The tiers are separate modules and separate declarations on purpose:** `binary_triage` is pure Python and its guarantee is that it **executes nothing**, asserted from its AST; `ghidra_headless` spawns a JVM. One entry covering both would silently widen the guarantee that makes `binary_triage`'s sandbox posture (Gap 71) arguable while leaving the wording unchanged. Ghidra needs a JDK 21+ and a ~400 MB install, so it is deliberately **not** in `requirements.txt` and the one thing it may never do is a successful-looking empty report: five statuses (`ok | truncated | unavailable | timeout | error`), and `functions`/`imports`/`strings` are **None — never `[]`** whenever nothing looked, with the reason named, because an `[]` there reads as "Ghidra looked at this binary and found no functions" — a claim about the ARTIFACT from a run that never opened it. A MEASURED `[]` survives as `[]`, and both directions are asserted. Two implementation facts are load-bearing rather than incidental: **the two timeouts are not the same timeout** (`-analysisTimeoutPerFile` is set BELOW the wall budget, so an over-running analysis still runs the postScript and yields a PARTIAL export this module can label `truncated`, instead of a killed process with no export to label at all), and **the process TREE is killed, not the process** — `analyzeHeadless` is a `.bat`/`sh` wrapping a JVM, so `proc.kill()` reaps the wrapper and leaves the JVM holding the pipes `communicate()` is blocked on. On the consumer side, **a binary's strings are a HINT and are labelled one**: `evidence: binary_strings`, `confidence: unasserted` (there is no second value), the RAW string carried so a reader can check, and no purl invented. **THE TOKEN IS NOT THE VERSION** — `_VERSION_TOKEN` ends at a word boundary, so the seam's hint for `OpenSSL 1.1.1k` is `1.1`, a *wrong* version rather than a vague one, and a CVE lookup against it answers about a release the artifact does not contain; the token is extended over the version characters following it in the same string and **both** values ride on the component with `version_basis` naming the rule. The residual (`v1.1.1k` yields no hint at all) is reported and pinned by a test rather than repaired here, because a second regex in the SBOM seam would make the SBOM and `binary_triage --json` disagree about one artifact. And corroboration is **reported beside the signature verdict and never folded into it**: `attach()` returns a new mapping with every existing key unchanged and REFUSES to overwrite, asserted as equality over the WHOLE result on both verifiers — so a triage that cannot load `pefile` can never downgrade a sound verification, and an adversary who can make the triage unmeasurable cannot move the verdict. The SBOM axis is deliberately asymmetric: only a version CONTRADICTION on a name both sides carry reaches `disagrees`, because static linking leaves no trace, a version string can name a protocol or somebody else's requirement, and the generator's honest `unknown` marker must never become a finding. Tasks: xrv-bin-01..03.

- **D407:** **The research loop was scoring an IDENTITY baseline, its master switch was read by nothing, and the fabricated `0.0` was deleting the loop's own audit record — so every unmutated run is now `unmeasurable`, and real mutation waits behind a manual gate.** `args/autoresearch_config.yaml: enabled: false` had shipped since the engine landed and the nightly `experiment` reflex ran the loop regardless; `autoresearch_enabled()` is now the ONE reading of it, with the env override outranking the config **in both directions** (it is the per-host knob for an opt-in loop that ships off) and an unreadable config **fail-closed** under `basis: config_unreadable`, so "declared off" and "could not tell" stay apart. Worse than the switch: `run_loop` evaluated a domain, created an experiment, ran it, evaluated **again with nothing changed**, and decided keep/discard on that delta — which is why every result carried `placeholder_metrics: True` and a note saying so, and why a nightly reflex reported 73 runs and 73 successes at `last_metric_value` 0.0. Any placeholder domain now makes the **whole** run `unmeasurable` (metric None, `total_kept`/`total_discarded`/`acceptance_rate` None, **no GKP export**), and a placeholder domain's keep count rides as `kept_unmeasured` and never `kept`, because a total summed across a measured and an unmeasured domain is not a total. **The 0.0 was self-erasing**: `base.run_reflex` suppresses the audit row when `metric_value == 0 and not details.get("tasks")`, so those 73 runs wrote 73 `reflex.started` rows and **not one** `reflex.completed`; `None == 0` is False, so the refusal is now recorded, and two daemon seams that coerced it back to a number (a `TypeError` on `evaluate_metric(cfg, None)` — an honest "could not measure" would have been scored a FAILURE and tripped the breaker, hiding the gap behind a disabled reflex — and a `float(metric or 0.0)` that staged a `pending_review` GKP for a run that did not run) are fixed. The Foundry is the same shape one module over: **4 of its 8 stage modules do not exist**, including the SEEDER, its only writer of a kanban row, so `tasks_emitted: 0` was never the gate verdict it reads as; `run_cycle` now distinguishes `stages_missing` / `stages_present` (which modules IMPORT) from `stages_ran` (which were INVOKED — a present stage still does not run under the circuit breaker), reports `unmeasurable` with the absent stages NAMED, and keeps `success: True`, because an unbuilt stage is a gap and not a failing cycle. A standing claim (`experiment_loop_measures_a_change`) guards it live with two derivations sharing no column, no reduction and no author: reported = the loop's own keep count, derived = DISTINCT experiment ids whose `pre_metric != post_metric`; verdict `agrees` at 0 and 0 over 9 rows, every one of which has `pre_metric == post_metric` **exactly**. **Real mutation is gated, not shipped**: `xrv-lab-02` sits behind `xrv-gate-00`, a manual sentinel whose risk line says why — opening it spends real tokens nightly on autonomous code mutation, and that is an operator's decision and not a scheduler's. Until it opens, the honest report is `unmeasurable`, which is the point: a loop that reports a measurement it did not make is worse than one that reports nothing. `experiment_programs` is created in both schemas and read and written by nothing, so it is registered as a substrate and probes `empty` — named rather than dropped, because deleting a table two schemas declare is its own decision. Tasks: xrv-lab-01..02, xrv-gate-00.

- **D408:** **Two files agreeing with each other is not currency, and a freshness check NEVER pulls, never edits a pin, and never reads "I could not reach the registry" as "the pin is fine".** The floci pins are cryptographically verifiable — `parse_pin` refuses a tag and `verify_bundle` re-hashes every OCI blob against its filename — and that machinery answers "does this bundle contain exactly what we pinned". It cannot answer, and nothing else asked, **"is what we pinned still the current release"**: the `floci/floci:2.0.1` pin's only ongoing assertion was a test that two files agree, and two files can agree perfectly about a version that shipped a year ago. AGREEMENT IS NOT CURRENCY. `args/pinned_artifacts.yaml` enumerates 16 artifacts and is a **declaration, never a second copy of a pin** — each entry names the file the pin actually lives in and the survey reads it through `image_vendor.parse_pin`, so there is no second parser to drift from, and a `pin_source` disagreeing with the manifest is `unmeasurable` rather than resolved in favour of one side. Four decisions. (1) **Three states, never merged: `current | behind` (the newer release NAMED) `| unmeasurable`** — and an air-gapped deployment, the one `tools/airgap` exists to serve, reports every artifact `unmeasurable` and **never `current`**, because a freshness check that read an unreachable registry as a clean pin hands exactly the disconnected operator a fabricated clean bill. `current_pct` is None, never 100.0, over an empty denominator, while a measured 0.0 stays a real red bar. (2) **Two bases for `behind`, and which one decided is recorded**: `version_tag` where the pinned tag carries an ordering (same SUFFIX and component count only, so `16.4-alpine` supersedes `16.3-alpine` and `16.4` does not, because a different suffix is a different image line), and `digest` where it does not — `redpanda:latest` and `k3s:latest` are pinned by a MUTABLE tag, so "newer" for them is knowable only as the tag having MOVED off the recorded digest, and `digest_drift` is None and never False when it could not be compared. (3) **It never acts on a pin.** `subprocess`, `os` and `shutil` are unimportable in both modules, the one HTTP door is GET/HEAD only, nothing opens a pin file for writing, and the reflex files ONE card per `behind` artifact through `task_factory.create_tasks` into `suggested` rather than moving the pin — moving a digest pin is a supply-chain act that belongs in a reviewed diff, which is the whole reason the pins are digests. Asserted against the AST and not the source text, because both modules explain in prose that they never touch `subprocess`, so a grep would flag their own explanation of themselves. (4) **ONE code path for every registry**: the host comes from the ref and the bearer token from the registry's own 401 challenge, so Docker Hub, `public.ecr.aws` and `ghcr.io` need no per-registry table, and the declared upstream is cross-checked against the host the ref implies so a label cannot disagree with the fetch. First live survey, 2026-09-12: **16 declared, 10 current, 6 behind, 0 unmeasurable** — postgres `16.3-alpine -> 18.6-alpine`, mysql `8.0.36 -> 26.7.0`, valkey `8 -> 9`, opensearch `2.19.5 -> 3.8.0`, amazonlinux `2023 -> 2027`, registry `2 -> 3` — with **three also carrying digest drift**, meaning an air-gap bundle re-cut from the tag today would not contain what the pin says. Both `:latest` pins measured UNMOVED since 2026-09-05, the positive control for the digest lane. Task: xrv-pin-01.
