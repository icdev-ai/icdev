...
- **Guardrails:**
  ...
  - **Bypass is explicit** — `get`, `execute_script`, `execute_cdp_cmd` and the timeout setters raise `ScopeViolation` on the wrapper; `.driver` is the single documented escape hatch.
...
- **Rationale:** Hostile web content is exactly the workload Chromium's renderer sandbox and site isolation were built for; wrapping the driver in `SandboxExecutor` would add a container boundary around a process that already has a stronger, purpose-built one, at the cost of making the vendored-driver path unusable. On the Python side the ingested content is inert: `_EXTRACT_JS` returns plain JSON, and no page-derived string is ever passed to `exec()`, `eval()`, `subprocess`, or a native parser. The only file mutation is a PNG written to `playwright/screenshots/` under a sanitised stem (`[^A-Za-z0-9_.-]` stripped, `..` collapsed, extension forced) — page content cannot influence the path.
...
- **Trigger fired (oss-browse-03) → discharged by Gap 39.** The MCP-tool clause above came true: `AgentBrowser` is now reachable from four agent-tool registries, one of them the MCP gateway. The exposure is authenticated and deny-first authorized rather than unauthenticated — see **Gap 39** for the reachability analysis, the RBAC wiring, and the session ceiling. The content decision in this entry is unchanged.

### Gap 39 — Agent browser session broker + four-seam registration (`tools/browser/session.py`)

**Modules:** `tools/browser/session.py`, and the registration seams it feeds — `tools/agent_toolkit/_browser.py`, `tools/mcp/gap_handlers.py` (`handle_browser_*`), `args/agent_toolsets.yaml` (`bundles.browser`), `tools/ace/agent_tools.py` (OSS adaptation oss-browse-03).

**Why this entry exists:** Gap 38 closed with an explicit revisit trigger — *"`AgentBrowser` is exposed through an unauthenticated surface (a dashboard route or an MCP tool) rather than in-process to an agent loop."* oss-browse-03 does exactly that, so the trigger has fired and this entry discharges it. Gap 36 covers the policy seam and Gap 38 the content seam; this entry covers the **reachability** seam: who can get to the browser at all, and how many of them.

**Ingress path:** No new *content* ingress — every operation still routes `session.py` → `AgentBrowser` → `scope.GuardedDriver`, so the entire content analysis in Gap 38 and the entire policy analysis in Gap 36 carry over unchanged. What is new is *reach*: the browser is now addressable from four registries instead of one in-process call site, three of which (toolkit, MCP gateway, SAG bundle) dispatch stateless calls and therefore need a broker to hold the per-instance element indices between them. The untrusted inputs are unchanged (agent-authored action arguments; remote page content); the new attack surface is an agent or MCP client reaching a browser it was never meant to have.

- **Decision:** **bypass-documented** — no new sandbox. `session.py` is a registry plus eight thin delegations; the isolation boundary remains the browser's own renderer sandbox (Gap 38) and the enforcement boundary remains `scope.py` (Gap 36).
- **Rationale:** Adding a sandbox here would wrap a dictionary lookup. The module never parses, renders, or executes page content — it holds `AgentBrowser` instances in a dict and forwards keyword arguments. The real risk of a four-seam registration is not code execution but **divergence**: a second, unguarded path to a driver behind one of the seams (a raw `get_driver()` that skips `GuardedDriver`) would silently void the allowlist, the budget, and the audit trail while all four registrations still looked correct. The control for that is single-implementation plus cross-seam tests, not a container.
- **Guardrails additional to Gaps 36 and 38:**
  - **One implementation, asserted by identity.** Every seam resolves to the same `session.py` operations; `tests/browser/test_browser_tool_registration.py::test_toolkit_does_not_reimplement_the_browser` asserts function *identity*, not equivalent behaviour, and `test_all_four_seams_expose_the_same_tool_names` fails if a rename strands a seam. No handler in any seam touches a raw WebDriver.
  - **Deny-first RBAC is wired, not merely declared.** `browser_*` is an explicit deny for `pm` / `developer` / `isso` / `co` in `args/owasp_agentic_config.yaml`, *and* every `handle_browser_*` calls `_mcp_authz_gate` before argument validation. That call is the load-bearing half: the gate is opt-in per handler in `gap_handlers.py`, so the policy would have been inert without it — the tool would have read as governed while being reachable by any role. A caller supplying no `mcp_role` is refused. Both halves are pinned by tests (`test_mcp_handler_denies_non_admin_roles`, `test_rbac_deny_is_explicit_not_merely_the_default_policy`).
  - **Authorization precedes argument validation**, so a denied caller cannot use error messages to enumerate the tool surface (`test_mcp_handler_authorization_precedes_argument_validation`).
  - **The whole surface is mutating.** `browser_read_state` and `browser_screenshot` do not change the page, but they are how untrusted remote content enters the model's context, and `discovery.py`'s read-only heuristic cannot distinguish a browser read from a filesystem read. Marking either read-only would skip `default_safety_gate` in `tools/agent_runtime/dispatch.py` entirely, so the bundle is gated as one unit and no browser tool may be `read_only` (`test_discovery_treats_every_browser_tool_as_mutating`).
  - **Resource ceiling.** `ICDEV_BROWSER_MAX_SESSIONS` (default 4) caps live drivers per process; `SessionLimitExceeded` is what stops a looping agent from opening browsers until the host exhausts memory. Sessions are process-local and never shared across processes.
  - **Sessions are attributable.** `run_id` is the session name, so every `audit_trail` row `GuardedDriver` writes names the session that produced it. ACE keys sessions by co-worker instance id, so two concurrent co-workers can never address each other's element indices.
  - **Opt-in everywhere.** The browser is absent from ACE's default tool set and from every other bundle; a co-worker gets it only by naming it in `icdev_tools`, and a standalone agent only by requesting the `browser` bundle *and* setting `ICDEV_SAG_ALLOW_MUTATION`.
- **Residual risk (accepted, stated plainly):** unchanged from Gap 38 — indirect prompt injection via page text. Broadened reach does not deepen that risk, but it does widen who can incur it, which is precisely what the deny-first RBAC and the mutating-by-default gate are there to bound. Note also that `ICDEV_MCP_AUTHZ_BYPASS=1` disables the RBAC gate wholesale; it is a dev/test opt-out and must never be set in a deployed configuration.
- **Revisit if:** a fifth seam is added without a corresponding case in `tests/browser/test_browser_tool_registration.py`; any seam gains a browser path that does not go through `tools/browser/session.py`; `browser_*` is granted to a non-admin role; a browser tool is marked `read_only`; or the browser becomes reachable from an *unauthenticated* surface (a dashboard route, or an MCP transport with no `mcp_role`) — the current registration is authenticated-and-authorized at every seam, and that is what makes bypass-documented the right call here.
