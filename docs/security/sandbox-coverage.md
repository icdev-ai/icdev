...
### Gap 37 — Agent browser: indexed page representation (`tools/browser/agent_browser.py`)
...
- **Guardrails:**
  - **Localhost-only by default, canonical policy authoritative** — `navigate()` refuses any scheme outside `agent_browser.allowed_schemes`, then hands every http(s) target to `scope.check_navigation()` (Gap 36) and refuses anything that decision denies, raising `BrowserScopeError`. `agent_browser.allowed_domains` is applied *in addition*, so this surface can only ever be narrower than the canonical allowlist — never wider. Widening either is a deliberate configuration act.
...
- **Revisit if:** page content is ever fed to `exec`/`eval`/a shell, the extraction JS is ever built by interpolating page-derived strings, `allowed_domains` is widened to the open internet, or `AgentBrowser` stops deferring to `check_navigation()` and reverts to its own standalone host policy — any of which changes the trust boundary this decision rests on. Credentials must never be typed into a page through this surface directly: that path is `scope.SensitiveDataResolver` (Gap 36), which brokers the value at the driver.
