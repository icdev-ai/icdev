...
- **Rationale:** The DIC modules themselves contain **no `exec()`/`eval()`/dynamic import and no shell surface**: the only `subprocess` call in `extractors.py` is `yt-dlp` on the *video/YouTube URL* path, which is a different route (see *Not in scope* below). Extracted text is never rendered unescaped, never passed to a shell, and never dynamically imported — it is chunked, embedded and stored. The exposure is therefore entirely the **native decode step inside third-party libraries** on a user-supplied file, plus one spawn of the `tesseract` binary on user-supplied image bytes. Uploads come from authenticated operators (the dashboard's global `before_request` 401s anonymous requests), not from anonymous input. That is the same reasoning Gap 3 and Gap 25 record, over a strictly larger parser set.
- **Guardrails:**
...
- **Revisit if:** a per-route cap is added (update the guardrail above); DIC extraction is routed through `SandboxExecutor` (promote the on-demand half from refusal to isolation); the route becomes reachable unauthenticated or by an external tenant; extracted text is ever rendered unescaped or passed to a shell; or a parser CVE with known ICDEV™ exposure ships (promote to `sandboxed`).
