...
- **Decision:** **trusted-first-party** (import target), with authorization of *which* tools a step may call deferred to dwo-mcp-02.
...
- **Guardrails:**
  ...
  - **Not yet reachable from a run** — the node type is intentionally unregistered and referenced by no template until dwo-mcp-02 lands the allowlist / `requires_approval` gate, so no workflow can reach the dispatcher in the interim.
  - **Regression test** — `tests/studio/test_mcp_executor.py` (24 tests) covers registry lookup, resource rejection, each validation failure mode, handler-exception propagation, and the CLI exit-code contract.
- **Revisit if:** the tool name or module path ever becomes derivable from tenant/end-user content rather than an authenticated template author; the registry lookup is relaxed to accept an arbitrary dotted module path; or dwo-mcp-02's allowlist is removed or made default-allow → re-decide as **sandboxed** (`tools/security/sandbox_executor.py`).
