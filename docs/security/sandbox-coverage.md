...
- **Rationale:** `python-pptx` only parses OOXML (a ZIP of XML parts) via `xml.etree`/`lxml` — no macro execution, no embedded-script evaluation, no `exec()`/`eval()`/`subprocess`. `svg_to_pptx.py` (used for the related `svg_art` slide type and the `slides_svg` graphics fallback) similarly only parses SVG via `defusedxml.ElementTree.fromstring` (XXE/entity-expansion-hardened; see B314 fix) with a bounded element allowlist — unrecognized elements are skipped, not executed. No user content is ever passed to a shell, template-rendered unescaped, or dynamically imported.
- **Guardrails:**
