...
### Gap 39 — Web fetch provenance (`tools/provenance/web_citation.py`)

**Module:** `tools/provenance/web_citation.py` (OSS adaptation oss-cite-01).

**Ingress path:** `fetch_with_provenance()` performs an outbound GET against a caller-supplied URL and returns the response body. Unlike Gap 37 (which takes an HTML *string* and never fetches) this module owns the fetch itself, so it holds two distinct risks: the **request** side (SSRF — a URL that resolves inward) and the **response** side (attacker-controlled bytes that are hashed, persisted, and returned to a caller who may put them in an LLM prompt).

- **Decision:** **bypass-documented** — for the response side. The module never interprets the bytes it fetches; the request side is covered by an explicit egress guard rather than by a sandbox.
- **Rationale:** What this module does with a response is hash it (`hashlib.sha256`), read five header values, and write those plus the URL pair into a table. There is no parse, no `exec`/`eval`/`subprocess`, no deserialisation of response bytes into objects, and no filesystem write. The content string is handed back to the caller unmodified and un-inspected — placing a process boundary around a sha256 and an INSERT would remove no capability. The fetch goes through `tools/http/client.py::request`, so it inherits that client's mTLS, proxy and retry/backoff posture rather than opening a bare socket. Notably the module does **not** render, execute, or follow anything in the body: redirects are followed by the HTTP client (and recorded as evidence in `redirect_chain`), but no link, script, image, or import inside the returned document is ever resolved.
- **Guardrails:**
  - **SSRF gate, fail-closed when configured** — `check_egress()` runs before any socket is opened. It is default-off (`egress.enabled: false` in `args/http_client.yaml`, matching the guard's original semantics in `tools/doc_modernization/link_check.py`); when enabled it applies HTTPS-only, suffix allow/denylist with deny-wins, and resolve-then-reject on any private/loopback/link-local answer, raising `EgressDenied`. A guard that is *configured but unimportable* returns `(False, "guard_unavailable")` — the gate fails closed, because a silently no-op SSRF check is worse than none while the config claims cover.
  - **Response bytes are never interpreted** — the content is hashed and returned; no branch in this module parses, renders, or executes it. Callers that go on to parse it (e.g. `page_extract`, Gap 37) own that seam under their own entry.
  - **Non-2xx is recorded, not raised** — the HTTP status is part of the evidence, so a 404 or a 500 produces a real provenance row rather than an exception. This is deliberate: "the page said nothing at that instant" is a citeable fact.
  - **Evidence is append-only** — `web_fetch_provenance` is registered in `APPEND_ONLY_TABLES` in `.claude/hooks/pre_tool_use.py` (NIST AU). Every fetch inserts a new row; a re-fetch never overwrites a prior observation, so a citation registered against an earlier fetch keeps validating against the bytes that were actually seen.
  - **Citation validation is not reimplemented** — parsing and report shape come from the shared `tools/quality/citation_grounding.py`; this module contributes only the allow-set of ids that exist in the table. A `[source: wfp-...]` tag therefore fails the same way, in the same shape, as a bad DIC or proposal citation.
  - **The stored URL pair is the honest one** — `requested_url` and `final_url` are persisted separately, so a citation that silently resolved through redirects to a different origin remains distinguishable after the fact from one that did not.
  - **Regression tests** — `tests/provenance/test_web_citation.py` (35 tests) covers egress denial and fail-closed-on-missing-guard, redirect-chain capture, header capture, hashing, the append-only re-fetch shape, the `citation_type` CHECK repair on both backends, and citation validation against persisted rows.
- **Residual risk (accepted, stated plainly):** with the egress guard at its default `enabled: false`, this module will fetch any URL a caller hands it, including an internal one. That is the pre-existing platform default for outbound fetches, not a regression introduced here; deployments that expose URL entry to untrusted users should set `egress.enabled: true`.
- **Revisit if:** the module begins parsing or rendering response bodies, resolves anything *inside* a fetched document (images, imports, `srcset`), gains a headless-browser path, or is exposed through an unauthenticated dashboard route or MCP tool — the last would turn the platform into an open redirect/scanner proxy and would make `egress.enabled: true` mandatory rather than advisory.
