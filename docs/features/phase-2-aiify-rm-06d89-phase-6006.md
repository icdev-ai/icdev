# Phase 2 — aiify-rm-06d89-phase-6006 (determination)

**Opportunity:** 6006 (scan_id 43, roadmap `rm-06d89040cf`)
**Pattern → paradigm:** `hardcoded_threshold` → `anomaly_detection`
**External module:** `…/aiify_git_zwu66zfu/src/documents/consumer.py` (paperless-ngx clone)
...
`aiify-rm-06d89-phase-5989/5990/5996/5997/5998/6000/6003/6004`) — same
`hardcoded_threshold` → `anomaly_detection` pattern, same `scan_id 43`, same
roadmap `rm-06d89040cf`, same external paperless-ngx `documents/` path
(`consumer.py` this time). All prior siblings closed as duplicates of the same
commit.
...
- `_load_anomaly_cfg` / config-driven `anomaly_detection` paradigm present in
  `tools/monitor/log_analyzer.py` (L427+), wired through frequency/error-rate. ✓
