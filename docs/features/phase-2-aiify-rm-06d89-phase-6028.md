# Phase 2 — aiify-rm-06d89-phase-6028 (determination)

**Opportunity:** 6028 (scan_id 43, roadmap `rm-06d89040cf`)
**Pattern → paradigm:** `hardcoded_threshold` → `anomaly_detection`
**External module:** `…/aiify_git_zwu66zfu/src/documents/management/commands/document_fuzzy_match.py` (paperless-ngx clone)
...
AI-ify engine shallow-clones, scans, then deletes. The clone directory
`aiify_git_zwu66zfu` is a stale leftover and the specific target file
`src/documents/management/commands/document_fuzzy_match.py` is **non-existent at
HEAD** (verified gone), so the path is unmodifiable. Per the established
