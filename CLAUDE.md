...
#   /proposal-genesis  — Proposal Genesis: autonomous capture pipeline — stat grid, reflex table, quality scores, audit trail, pipeline runner (Phase A)
#   /filesync          — File Sync dashboard: stat grid, sync job table, activity log, create modal, run/delete actions (D-SYNC-1 through D-SYNC-12)
#   /orchestration     — Real-time orchestration dashboard: agent grid with status/tier/elapsed, workflow DAG (SVG), mailbox feed (SSE), prompt chains, ATLAS critiques (Phase 61)
...
| Genesis Daemon | `goals/genesis_daemon.md` | v2.0 autonomous research lab — 13 Reflexes, Trust Kernel, circuit breakers, always-on daemon |
