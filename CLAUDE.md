...
- **New tool/module registration checklist (8 points):**
  ...
  5. `tools/hooks/shared_checks.py` — add append-only tables (+ mirror to `icdev/`)
...
- **Component configuration audit:** when adding an append-only component-audit event, use `log_component_audit()` from `tools.config.component_registry`. New component-audit tables must be added to `APPEND_ONLY_TABLES` in `tools/hooks/shared_checks.py`.
