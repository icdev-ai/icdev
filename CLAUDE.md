...
### Development Rules
- **PostgreSQL is the native runtime backend — author all SQL/DB access for PG** (applies to ICDEV code AND generated child apps). Get connections via `from tools.db.storage import get_connection` (or `get_canvas_connection()` for canvas/app-local tables) and honor `ICDEV_STORAGE_BACKEND`. In runtime code NEVER use SQLite-only constructs: `sqlite3.connect()` for data access, `conn.executescript(...)`, `SELECT ... FROM sqlite_master` (use a backend-aware table-list helper), or the JSON1 builtins `json_extract` / `json_each` / `json_array_length` (use PG `jsonb` operators, or compute in Python — the runtime SQL translator has gaps and is being retired to a thin safety-net). **SQLite is a fallback ONLY at initialization when PG is unreachable; the running process must never silently switch backends.** Enforced by `pg_portability_linter` + the coherence gate (project PGP).
