# CUI // SP-CTI
"""rmf-ui-03: /ato-package is GOVERNED -- it lives on the Boundary canvas as /boundary/ato-package.

The ATO Package Builder was one of ~15 compliance pages that were bare
...
It is an RMF ARTIFACT surface -- the wizard that assembles SSP/SAR/POAM/SBOM
into the accreditation package over the unchanged ``/api/ato-package/*``
blueprint -- so it belongs beside the ATO boundary, enclave and cATO pages BDC
already owns. Nothing on the canvas overlapped it, so nothing was folded.
...
TEMPLATE = TEMPLATE_DIR / "boundary_canvas" / "ato_package.html"
TEMPLATE_MIRROR = TEMPLATE_DIR_MIRROR / "boundary_canvas" / "ato_package.html"
OLD_TEMPLATE = TEMPLATE_DIR / "ato_package.html"
OLD_TEMPLATE_MIRROR = TEMPLATE_DIR_MIRROR / "ato_package.html"
BASE_HTML_COPIES = (
...
def test_no_template_links_the_ungoverned_url(root):
    """compliance.html (the hub) linked /ato-package too -- every href is repointed."""
    offenders = sorted(
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/ato_package.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/ato_package.html"' in src
    ), "blueprint does not render boundary_canvas/ato_package.html"
