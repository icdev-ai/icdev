...
_SCHEMA_KEYS = ("entity_currency", "dic_section_annotations", "dic_sme_assertions")
...
def _comment(conn, ann_id, comment, author, created_at, *, doc_id="doc-1",
             section_id="sec-1", category="compliance"):
    conn.execute(
        "INSERT INTO dic_section_annotations "
        "(ann_id, section_id, doc_id, selected_text, category, comment, author, "
        " status, classification, created_at, updated_at) "
        "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        (ann_id, section_id, doc_id, "", category, comment, author,
         "open", "CUI", created_at, created_at),
    )
    conn.commit()
    return ann_id
