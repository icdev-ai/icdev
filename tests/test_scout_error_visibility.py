...
class TestTheReasonActuallyReachesTheDaemon:
    """The end-to-end link that nothing else asserted.

    Classifying a failure inside ``_github_api`` is worthless if the reason never
    reaches the state row. Mutation testing found this gap: renaming
    ``details["error"]`` in ``scout.run`` to anything else left BOTH
    test_scout_error_visibility.py and test_reflex_circuit_breaker_recovery.py
    green — 32 passing tests, and the entire point of xbm-wake-01 (a dead reflex
    must say WHY) was unasserted.

    ``daemon.base.classify_failure`` reads exactly ``details["error"]``, so that
    key name is a contract between the two modules, not an implementation detail.
    """

    def _all_repos_fail(self, monkeypatch, tmp_path):
        monkeypatch.setattr(scout, "_is_air_gapped", lambda: False)
        monkeypatch.setattr(scout, "_load_targets", lambda: [
            {"repo": "o/a", "category": "x"}, {"repo": "o/b", "category": "x"},
        ])

        def _fail(endpoint, timeout=None, error_sink=None):
            if error_sink is not None:
                error_sink.append("http_401_bad_github_token")
            return None

        monkeypatch.setattr(scout, "_github_api", _fail)
        monkeypatch.setattr(scout, "BASE_DIR", tmp_path)
        return scout.run({}, None)

    def test_total_failure_reports_a_reason_not_a_threshold_miss(self, monkeypatch, tmp_path):
        result = self._all_repos_fail(monkeypatch, tmp_path)
        assert result["success"] is False
        details = result["details"]
        assert "error" in details, (
            "scout scouted 0 repos and said nothing about why — the daemon will "
            "record the generic 'metric_threshold_not_met' that hid this outage "
            "for five weeks"
        )
        assert "github_api_unavailable" in details["error"]
        assert "http_401_bad_github_token" in details["error"]

    def test_the_key_matches_what_classify_failure_reads(self, monkeypatch, tmp_path):
        """Contract between scout and daemon.base — a rename breaks it silently."""
        from tools.daemon.base import classify_failure

        details = self._all_repos_fail(monkeypatch, tmp_path)["details"]
        reason = classify_failure(False, details, "repos_scouted", 0.0,
                                  {"operator": "gte", "threshold": 1})
        assert reason == details["error"], (
            "classify_failure did not surface scout's reason — the two modules "
            "disagree on the key"
        )
        assert "metric_threshold_not_met" not in reason
