# CUI // SP-CTI
"""rmf-ui-16: /ai-accountability is GOVERNED -- it lives on the Security canvas.

The AI Accountability page (human oversight plans, appeals, CAIO designation,
incident response, ethics reviews -- Phase 49) was one of ~15 compliance pages that were bare
...
ONE route per card, on purpose (rmf-ui-01 set the shape, rmf-ui-15 is the SDC
sibling this file clones):
...
NEW_PATH = "/security/ai-accountability"
OLD_PATH = "/ai-accountability"
...
TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "security_canvas" / "ai_accountability.html"
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "security_canvas" / "ai_accountability.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "ai_accountability.html"
OLD_TEMPLATE_MIRROR = REPO / "icdev" / "tools" / "dashboard" / "templates" / "ai_accountability.html"
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(sdc_app):
    ...
    assert "<h1>AI Accountability</h1>" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/ai-accountability/stats" in body
    assert "/api/ai-accountability/audit" in body
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("security_canvas/ai_accountability.html"' in src or (
        "render_template(\n" in src and '"security_canvas/ai_accountability.html"' in src
    ), "blueprint does not render security_canvas/ai_accountability.html"
