# CUI // SP-CTI
"""rmf-ui-12: /stig-manager is GOVERNED -- it lives on the Security canvas.

The STIG Benchmark Manager was one of ~15 compliance pages that were bare
...
completeness gate owns the template directory. SDC already owns STIGs (its
registry description: "Threat model, hardening, STIGs, attack-path analysis").
...
NEW_PATH = "/security/stig-manager"
OLD_PATH = "/stig-manager"
...
TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "security_canvas" / "stig_manager.html"
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "security_canvas" / "stig_manager.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "stig_manager.html"
OLD_TEMPLATE_MIRROR = REPO / "icdev" / "tools" / "dashboard" / "templates" / "stig_manager.html"
BASE_HTML_COPIES = (
...
def _isolated_app(blueprint, url_prefix):
    ...
    Same shape as tests/cortex/test_blueprint_routes.py and the rmf-ui-01
    exemplar: registering onto the shared ``tools.dashboard.app`` singleton is
    order-dependent, so copy the singleton's template folder, filters, globals
    and context processors onto a fresh app and register the blueprint there.
    """
...
def test_governed_page_renders_for_an_authenticated_session(sdc_app):
    ...
    assert "STIG Benchmark Manager" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/stig-manager/stats" in body
...
def test_no_template_links_the_ungoverned_url():
    """compliance.html is the hub that links its siblings; it must be repointed too."""
    roots = (
    ...
    assert not stale, f"templates still link {OLD_PATH}: {stale}"
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("security_canvas/stig_manager.html"' in src or (
        "render_template(\n" in src and '"security_canvas/stig_manager.html"' in src
    ), "blueprint does not render security_canvas/stig_manager.html"
