...
def test_an_unknown_license_is_stated_explicitly_with_a_reason(declared, reason):
    ...
    # The reason is stated through the shared convention, not a second vocabulary of
    # this element's own — and the element-level precision survives as the detail.
    disclosure = cl.record_license_disclosure(result)
    assert disclosure.state_of(FIELD_LICENSE) == cl.UNKNOWN
    assert disclosure.details[FIELD_LICENSE] == reason


def test_this_element_emits_no_unknown_reason_property_of_its_own():
    """Two competing reason properties on one component is the conflation prc-01 forbids."""
    properties = _properties(
        {"properties": cl.license_properties(cl.resolve_declared_license(None))}
    )
    assert not [name for name in properties if name.endswith("-unknown-reason")]


@pytest.mark.parametrize(
    "resolved,expected",
    [
        (True, REASON_NOT_PROVIDED_BY_PRODUCER),
        (False, REASON_NOT_RESOLVABLE_OFFLINE),
    ],
)
def test_an_undeclared_license_reports_why_based_on_where_it_came_from(resolved, expected):
    """Reading a package's real metadata and finding no license means the producer
    published none. Reading a requirements.txt line means nothing about the package at
    all — that format cannot carry a license, so the value is one an offline build
    cannot reach. Collapsing the two would assert a fact the manifest never stated."""
    disclosure = cl.record_license_disclosure(
        cl.resolve_declared_license(None), resolved=resolved
    )
    assert disclosure.unknown_fields[FIELD_LICENSE] == expected


def test_the_shared_reason_is_always_in_the_closed_vocabulary():
    """A reason outside UNKNOWN_REASONS is unreadable to a recipient's tooling, and
    `Disclosure.unknown` rejects it — so every local detail must map onto the set."""
    for declared in _DECLARATION_CORPUS:
        result = cl.resolve_declared_license(declared)
        if not cl.is_unknown(result):
            continue
        for resolved in (True, False):
            # Raises ValueError if the mapping ever produces an off-vocabulary reason.
            cl.record_license_disclosure(result, resolved=resolved)


def test_a_withheld_license_discloses_nothing_about_itself():
    """A URL or a license name left in place beside a withheld marker would publish
    exactly what the operator withheld."""
    result = cl.resolve_declared_license("MIT")
    disclosure = Disclosure().withheld(FIELD_LICENSE, REASON_CONTRACTUAL_RESTRICTION)

    assert cl.license_db_value(result, disclosure) == WITHHELD
    assert cl.license_entries(result, disclosure) == []

    properties = _properties({"properties": cl.license_properties(result, disclosure)})
    assert properties[cl.PROPERTY_LICENSE] == WITHHELD
    assert properties[cl.PROPERTY_DECLARATION] == WITHHELD
    # The pair that keeps the element present is still complete.
    assert properties[cl.PROPERTY_PROPRIETARY] == cl.UNKNOWN
    assert "MIT" not in json.dumps(properties)


def test_a_withheld_license_is_not_overwritten_by_the_unknown_path():
    """"We are not telling you" outranks "we could not find it"."""
    disclosure = Disclosure().withheld(FIELD_LICENSE, REASON_CONTRACTUAL_RESTRICTION)
    cl.record_license_disclosure(cl.resolve_declared_license(None), disclosure)

    assert disclosure.state_of(FIELD_LICENSE) == WITHHELD
...
def test_every_component_in_a_generated_document_carries_the_element():
    ...
    # No declaration anywhere — explicitly unknown, not omitted.
    flask = _properties(by_name["flask"])
    assert flask[cl.PROPERTY_LICENSE] == cl.UNKNOWN
    assert flask[f"{PROPERTY_UNKNOWN_PREFIX}{FIELD_LICENSE}"] == REASON_NOT_PROVIDED_BY_PRODUCER
    assert flask[f"{PROPERTY_UNKNOWN_DETAIL_PREFIX}{FIELD_LICENSE}"] == cl.REASON_NOT_DECLARED
    assert "licenses" not in by_name["flask"]


def test_the_operator_can_withhold_a_license_from_the_generated_document():
    """End to end: a policy rule reaches the document, and the license it withheld is
    nowhere in the component it applies to."""
    policy = {
        ...
        "withhold": {
            ...
            "components": [
                {
                    "field": FIELD_LICENSE,
                    "reason": REASON_CONTRACTUAL_RESTRICTION,
                    "match": {"name": "left-pad"},
                }
            ],
        },
    }
    sbom, _ = _build_cyclonedx_sbom(_PROJECT, _COMPONENTS, disclosure_policy=policy)
    by_name = {c["name"]: c for c in sbom["components"]}

    withheld = by_name["left-pad"]
    properties = _properties(withheld)
    assert properties[cl.PROPERTY_LICENSE] == WITHHELD
    assert properties[f"{PROPERTY_WITHHELD_PREFIX}{FIELD_LICENSE}"] == REASON_CONTRACTUAL_RESTRICTION
    assert "licenses" not in withheld
    assert "MIT" not in json.dumps(withheld)

    # The rule was scoped to one component and did not leak to the others.
    assert _properties(by_name["acme-sdk"])[cl.PROPERTY_LICENSE] == "UNLICENSED"


def test_a_declared_only_component_reports_the_offline_reason():
    """A manifest that cannot carry a license did not say the producer shipped none."""
    components = [dict(_COMPONENTS[2], resolution=RESOLUTION_DECLARED)]
    sbom, _ = _build_cyclonedx_sbom(_PROJECT, components)

    properties = _properties(sbom["components"][0])
    assert properties[f"{PROPERTY_UNKNOWN_PREFIX}{FIELD_LICENSE}"] == REASON_NOT_RESOLVABLE_OFFLINE
...
def test_package_lock_declares_licenses(tmp_path):
    ...
    by_name = {c["name"]: c for c in _resolve_package_lock(lock)["components"]}
...
@pytest.mark.parametrize(
    "metadata,expected",
    [
        ({"License-Expression": "Apache-2.0", "License": "ignored"}, "Apache-2.0"),
        ({"License": "MIT"}, "MIT"),
        ({"Classifier": ["License :: OSI Approved :: MIT License"]}, "MIT License"),
        # "OSI Approved" on its own names no license.
        ({"Classifier": ["License :: OSI Approved"]}, None),
        ({}, None),
        # The whole license body pasted into the field is not a license *name*.
        ({"License": "Copyright (c)\n\nPermission is hereby granted"}, None),
        ({"License": "x" * 200}, None),
    ],
)
def test_installed_python_metadata_yields_a_license(metadata, expected):
    """dist-info METADATA is the only place an air-gapped build can read a Python
    package's license, and it is where most of this repo's components get one."""
    ...
    assert _python_metadata_license(_Meta(metadata)) == expected


def test_the_row_id_is_the_bom_ref():
    """A stored row and the document entry describing it must be correlatable — they
    are the same string, so correlation is identity rather than a prefix rule."""
    component = _COMPONENTS[0]
    sbom, _ = _build_cyclonedx_sbom(_PROJECT, [component])

    assert sbom["components"][0]["bom-ref"] == _generate_bom_ref(component)
...
CREATE TABLE IF NOT EXISTS sbom_records (
    ...
    classification TEXT NOT NULL DEFAULT 'CUI',
    tenant_id TEXT,
    -- sbx-fnd-02 / sbx-sig-01 / sbx-prc-02. Present here because the generator writes
    -- them on every run: a fixture schema that stops at migration 209 makes this
    -- module fail for a reason that has nothing to do with the element under test.
    sbom_author TEXT,
...
def generated_sbom(tmp_path, monkeypatch):
    ...
    def run():
        ...
        rows = (
            ...
            .execute(
                "SELECT component_name, id, version, license, purl, producer, "
                "unknown_fields_json, withheld_fields_json FROM sbom_components"
            )
            .fetchall()
        )
        return SimpleNamespace(
            ...
            rows={
                name: {
                    ...
                    "purl": purl,
                    "producer": producer,
                    "unknown": json.loads(unknown_json),
                    "withheld": json.loads(withheld_json),
                }
                for name, row_id, version, license_value, purl, producer, unknown_json, withheld_json in rows
            },
            regenerate=run,
        )
...
def test_the_document_and_the_table_agree_on_which_components_exist(generated_sbom):
    """The rows are written FROM the finished document, so a row cannot describe a
    component the document left out — or omit one it published."""
    document = generated_sbom.document
    assert {c["name"] for c in document["components"]} == set(generated_sbom.rows)
    assert {c["bom-ref"] for c in document["components"]} == {
        row["id"] for row in generated_sbom.rows.values()
    }


def test_the_persisted_row_carries_the_reason_the_license_is_unknown(generated_sbom):
    """`license = 'unknown'` alone says nothing about WHY. The disclosure blobs
    sbx-fnd-02 created for that purpose are written by this same row."""
    assert generated_sbom.rows["mystery"]["unknown"][FIELD_LICENSE] in (
        REASON_NOT_PROVIDED_BY_PRODUCER,
        REASON_NOT_RESOLVABLE_OFFLINE,
    )
    assert generated_sbom.rows["left-pad"]["unknown"].get(FIELD_LICENSE) is None
...
def test_the_persisted_row_and_the_document_report_the_same_license(generated_sbom):
    ...
    in_document = {
        component["name"]: _properties(component)[cl.PROPERTY_LICENSE]
        for component in generated_sbom.document["components"]
    }
    assert {name: row["license"] for name, row in generated_sbom.rows.items()} == in_document
