...
def _as_document(sources, spec_version="1.6"):
    """Run sources through the real document builder and return its components.

    `_persist_components` reads the FINISHED CycloneDX components, not the parser
    dicts — that is what makes the table incapable of disagreeing with the
    document. Going through `_build_cyclonedx_sbom` here keeps these tests on that
    same contract instead of hand-rolling a component shape the generator never
    produces.
    """
    from tools.compliance.sbom_generator import _build_cyclonedx_sbom

    sbom, _ = _build_cyclonedx_sbom(
        {"id": "p1", "name": "Proj"}, [dict(s) for s in sources], spec_version=spec_version
    )
...
def test_identifiers_round_trip_through_the_sbom_components_column(components_db):
    ...
    sources = [PYPI, MAVEN, GOLANG_PSEUDO, {**NPM_SCOPED, "commit_hash": "f" * 40}]
    cdx_components = _as_document(sources)
    expected = {c["bom-ref"]: parse_identifiers_from_cyclonedx(c) for c in cdx_components}

    written = _persist_components(_ParamShim(components_db), cdx_components)
...
def test_row_id_is_the_documents_bom_ref_so_the_two_cannot_drift(components_db):
    """The catalog key and the document's own reference are one derived string."""
    from tools.compliance.sbom_generator import _persist_components

    cdx_components = _as_document([PYPI])
    _persist_components(_ParamShim(components_db), cdx_components)

    row_id = components_db.execute("SELECT id FROM sbom_components").fetchone()["id"]
    assert row_id == cdx_components[0]["bom-ref"] == component_id(PYPI)


def test_persisting_twice_updates_in_place_rather_than_duplicating(components_db):
    from tools.compliance.sbom_generator import _persist_components

    first = _as_document([PYPI])
    _persist_components(_ParamShim(components_db), first)

    upgraded = _as_document([{**PYPI, "version": "2.32.0", "purl": "pkg:pypi/requests@2.32.0"}])
...
def test_persisted_json_is_valid_json_with_the_documented_envelope(components_db):
    ...
    _persist_components(_ParamShim(components_db), _as_document([PYPI]))
...
def test_a_component_with_no_identifiers_persists_an_empty_set_not_a_lost_row(components_db):
    """The row is inventory; an empty identifier set is a statement, not a reason to drop it.

    A conformance failure belongs to the validator, which reports "no component
    identifier". Silently omitting the row would hide it instead.
    """
    from tools.compliance.sbom_generator import _persist_components

    bare = {"type": "library", "bom-ref": "no-ids", "name": "bare", "version": "1.0"}
    assert _persist_components(_ParamShim(components_db), [bare]) == 1

    raw = components_db.execute("SELECT identifiers_json FROM sbom_components").fetchone()[0]
    assert identifiers_from_json(raw) == []
    assert not validate_identifiers([], "bare")["valid"]
...
def test_unknown_component_type_is_recorded_as_other_not_rejected(components_db):
    ...
    _persist_components(_ParamShim(components_db), _as_document([odd]))
...
def test_scoped_npm_packages_get_a_valid_purl(tmp_path):
    ...
    from tools.compliance.sbom_generator import _parse_package_json

    manifest = tmp_path / "package.json"
    manifest.write_text(
        json.dumps({"dependencies": {"@babel/core": "^7.24.0", "express": "4.19.2"}}), encoding="utf-8"
    )
    # The declared-manifest parser and the resolver both mint npm purls, and
    # sbx-cov-01 moved lockfile reading to the latter. Both encodings are checked
    # here, because either one reaching the document fails the element.
    ...
    purls |= {c["purl"] for c in resolve_project(tmp_path)["components"] if c.get("purl")}
...
CREATE TABLE sbom_records (
    ...
    classification      TEXT NOT NULL DEFAULT 'CUI',
    tenant_id           TEXT,
    -- sbx-fnd-02 / sbx-sig-01 / sbx-prc-02. The generator writes these on every
    -- run, so a fixture that stops at migration 209 fails for a reason that has
    -- nothing to do with the element under test.
    sbom_author         TEXT,
