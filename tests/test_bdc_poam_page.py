# CUI // SP-CTI
"""rmf-ui-07: /poam is GOVERNED -- it lives on the Boundary canvas.

The POA&M Findings page was one of ~15 compliance pages that were bare
...
The /api/poam/* routes stay in app.py; only the PAGE route moved.
...
TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "boundary_canvas" / "poam" / "list.html"
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "boundary_canvas" / "poam" / "list.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "poam" / "list.html"
OLD_TEMPLATE_MIRROR = REPO / "icdev" / "tools" / "dashboard" / "templates" / "poam" / "list.html"
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(bdc_app):
    ...
    assert "Plan of Action &amp; Milestones" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/poam/summary" in body
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/poam/list.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/poam/list.html"' in src
    ), "blueprint does not render boundary_canvas/poam/list.html"
