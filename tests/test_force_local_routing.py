...
@pytest.fixture
def bridge_override():
    """Set the CLI-bridge override and always unwind it.

    It is a ContextVar, so a leaked value would poison sibling tests; reset takes
    the Token the setter handed back.
    """
    tokens = []

    def _set(value):
        tokens.append(cli_bridge_override(value))
