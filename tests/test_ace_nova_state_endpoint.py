...
def _seed_improvement(nova_db_path: str, task_type: str, text: str, score: float = 0.9) -> None:
    conn = sqlite3.connect(nova_db_path)
    conn.execute(
        "INSERT INTO agent_improvement_artifacts "
        "(artifact_id, task_type, improvement_text, composite_score, status) "
        "VALUES (?, ?, ?, ?, 'applied')",
        (f"art-{uuid.uuid4().hex[:8]}", task_type, text, score),
    )
    conn.commit()
    conn.close()
