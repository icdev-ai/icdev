...
def test_local_allowlist_cannot_widen_canonical_scope(monkeypatch):
    """A host allowed here is still refused when browser_scope.yaml denies it.

    The two allowlists are ANDed, so widening agent_browser.allowed_domains
    can never buy reach the canonical oss-browse-02 policy withholds.
    """
    import tools.browser.scope as scope

    denied = scope.ScopeDecision(False, "not_allowlisted", "https://evil.test/")
    monkeypatch.setattr(scope, "check_navigation", lambda url, *a, **k: denied)

    ab = AgentBrowser(
        driver=object(),
        config=dict(DEFAULT_CONFIG, allowed_domains=["evil.test"]),
    )
    with pytest.raises(BrowserScopeError, match="not_allowlisted"):
        ab.navigate("https://evil.test/")
