# CUI // SP-CTI
"""rmf-ui-09: /compliance-debt is GOVERNED -- it lives on the Boundary canvas.

The Compliance Debt Burndown page was one of ~15 compliance pages that were
bare ``@app.route`` handlers in a 10,500-line app.py: no registry entry, NO
RBAC GUARD, no completeness gate, no IQE dispatch. A route on the Boundary &
Supply Chain Canvas blueprint gets all of those by construction -- app.py
attaches ``guard_component_access("bdc", <min_il>)`` as a ``before_request``
on every registered canvas blueprint, the registry's ``url_prefix`` + IQE
adapter put ``/boundary/*`` on the client-side path->canvas map, and the canvas
completeness gate owns the template directory.

An RMF ARTIFACT surface -- POA&M, control and STIG debt burndown, ATO
expirations, SLA compliance -- over the unchanged ``/api/compliance-debt/*``
blueprint, so it lands on the canvas that owns the ATO boundary.
...
NEW_PATH = "/boundary/compliance-debt"
OLD_PATH = "/compliance-debt"
...
TEMPLATE = TEMPLATE_DIR / "boundary_canvas" / "compliance_debt.html"
TEMPLATE_MIRROR = TEMPLATE_DIR_MIRROR / "boundary_canvas" / "compliance_debt.html"
OLD_TEMPLATE = TEMPLATE_DIR / "compliance_debt.html"
OLD_TEMPLATE_MIRROR = TEMPLATE_DIR_MIRROR / "compliance_debt.html"
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(bdc_app):
    ...
    assert "Compliance Debt Burndown" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/compliance-debt/summary" in body
...
def test_no_template_links_the_ungoverned_url(root):
    """compliance.html linked /compliance-debt too -- every href is repointed."""
    offenders = sorted(
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/compliance_debt.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/compliance_debt.html"' in src
    ), "blueprint does not render boundary_canvas/compliance_debt.html"
