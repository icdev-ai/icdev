...
def _browser(driver=None, **cfg_overrides) -> AgentBrowser:
    ...
    return AgentBrowser(driver=driver or FakeDriver(), config=config)
...
def test_config_loads_from_args_yaml():
    ...
    assert "http" in cfg["navigation"]["allowed_schemes"]
...
def test_config_falls_back_when_file_missing(tmp_path):
    ...
    assert cfg["navigation"]["allowed_schemes"] == ["http", "https", "about"]
...
def test_url_gate_allows_http_and_https():
    nav = load_config()["navigation"]
    check_url("http://localhost:5050/kanban", nav)
    check_url("https://example.gov/a/b?c=d", nav)
...
def test_url_gate_blocks_dangerous_schemes(url):
    nav = load_config()["navigation"]
    with pytest.raises(NavigationBlockedError):
        check_url(url, nav)


def test_url_gate_blocked_domains_win():
    nav = {"allowed_schemes": ["https"], "blocked_domains": ["evil.test"], "allowed_domains": []}
    with pytest.raises(NavigationBlockedError):
        check_url("https://sub.evil.test/x", nav)


def test_url_gate_allowed_domains_restrict():
    nav = {"allowed_schemes": ["https"], "blocked_domains": [], "allowed_domains": ["icdev.ai"]}
    check_url("https://docs.icdev.ai/x", nav)
    with pytest.raises(NavigationBlockedError):
        check_url("https://elsewhere.test/x", nav)


def test_navigate_refuses_blocked_url_without_touching_driver():
    driver = FakeDriver()
...
def test_real_browser_indexes_a_live_page(tmp_path):
    ...
    config = load_config()
    config["navigation"] = dict(config["navigation"])
    config["navigation"]["allowed_schemes"] = ["file", "http", "https", "about"]
    config["navigation"]["settle_ms"] = 100
    config["screenshot"] = {"dir": str(tmp_path), "default_name": "fixture"}

    browser = AgentBrowser(config=config, headless=True)
    try:
        try:
            state = browser.navigate(fixture.as_uri())
        ...
        assert browser.driver.title == "Saved"
