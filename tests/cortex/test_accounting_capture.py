...
* ``cortex.govern`` records latency too — its operation body is the identity
  function, so its honest cost is the gate chain, and ``pipeline_ms`` is what
  measures it.
...
def test_search_records_non_zero_latency_in_metrics(cortex_db, stub_gates, monkeypatch):
    ...
    assert gates["pipeline_ms"] >= gates["operation_ms"]
...
def test_govern_records_latency(cortex_db, stub_gates, monkeypatch):
    ...
    assert gates["pipeline_ms"] >= SLOW_MS
    assert gates["latency_ms"] >= SLOW_MS
...
def test_cortex_result_accounting_is_unchanged(cortex_db, stub_gates):
    ...
    assert gates["operation_ms"] >= 0.0 and gates["pipeline_ms"] > 0.0
...
# ---------------------------------------------------------------------------
def _timing(operation_ms=0.0, started_ago_s=0.0):
    return {"started": time.perf_counter() - started_ago_s, "operation_ms": operation_ms}
...
def test_accounting_prefers_the_results_own_provider_latency():
    ...
    assert out["operation_ms"] == 12.0


def test_accounting_falls_back_to_pipeline_time_for_a_list_result():
    out = governance._accounting_fields(
        [_hit()], _timing(operation_ms=37.5, started_ago_s=0.05), None
    )
    assert out["operation_ms"] == 37.5
    assert out["latency_ms"] == out["pipeline_ms"] >= 45.0
...
def test_latency_fallback_is_pipeline_time_not_operation_time():
    ...
    replaced. The fallback is deliberately the whole-call measurement.
    """
    out = governance._accounting_fields(
        "some text", _timing(operation_ms=0.005, started_ago_s=0.05), None
    )
    assert out["operation_ms"] == 0.005
    assert out["pipeline_ms"] >= 45.0
    assert out["latency_ms"] == out["pipeline_ms"]


def test_accounting_uses_the_tally_when_the_result_reports_nothing():
    out = governance._accounting_fields(
        "some text", _timing(operation_ms=5.0),
        {"calls": 2, "cost_usd": 0.02, "input_tokens": 40, "output_tokens": 8,
    ...
    assert out["latency_ms"] == out["pipeline_ms"] > 0.0


def test_accounting_tolerates_missing_timing_and_tally():
    out = governance._accounting_fields(None, None, None)
    assert out["latency_ms"] == 0.0
    assert out["operation_ms"] == 0.0 and out["pipeline_ms"] == 0.0
