...
def test_backend_constant():
    assert CORTEX_BACKENDS == ("rag", "graph", "dic", "kb")
...
def test_context_rls_ready_defaults():
    ...
    # None == "unset": the pipeline resolves the effective policy from the
    # classification (redaction fails closed for CUI+ by default).
