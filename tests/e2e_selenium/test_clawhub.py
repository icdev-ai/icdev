...
Verifies the ClawHub Skill Browser page loads correctly with search, results,
and import queue sections.
Ported from .claude/commands/e2e/clawhub.md.
...
@pytest.fixture(scope="module")
def page(driver):
    """Return a BasePage bound to the ClawHub base URL."""
...
class TestClawHubPage:
    """ClawHub Skill Browser — ported from clawhub.md."""

    def test_clawhub_page_loads_with_cui_banner(self, page: BasePage):
        """Steps 1-5: Navigate to /clawhub and verify heading + CUI banner."""
        page.navigate("/clawhub")
        page.screenshot("clawhub_01_main")

        body_text = page.driver.find_element(By.TAG_NAME, "body").text
        assert body_text, "ClawHub page body is empty"

        # Step 4: Heading contains ClawHub or Skill Browser
        heading_found = any(
            term in body_text for term in ["ClawHub", "clawhub", "Skill Browser", "skill browser"]
        )
        assert heading_found, "No 'ClawHub' or 'Skill Browser' heading found"

        # Step 5: CUI banner present
        assert CUI_BANNER in body_text, f"CUI banner '{CUI_BANNER}' not found on ClawHub page"
    ...
    def test_search_form_exists_and_submits(self, page: BasePage):
        ...
        page.screenshot("clawhub_02_search_results")
        ...
        assert search_inputs or search_ui_found, "No search form found on ClawHub page"
