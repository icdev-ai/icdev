...
# ---------------------------------------------------------------------------
# Runner entry point — promote_findings_to_kanban (xbm-promote-01-d4)
#
...
def test_entry_point_is_off_when_env_unset(monkeypatch):
    ...
    result = kp.promote_findings_to_kanban(dry_run=False)
...
def test_entry_point_off_does_not_touch_the_database(monkeypatch):
    ...
    before any DB work — and says so without patching the connection factory.
    """
    monkeypatch.delenv(kp.PROMOTE_ENABLED_ENV, raising=False)
    ...
    monkeypatch.setattr(kp, "run_promotion", _boom)

    assert kp.promote_findings_to_kanban()["enabled"] is False
...
def test_entry_point_stays_off_for_negative_values(monkeypatch, value):
    ...
    monkeypatch.setattr(kp, "run_promotion", lambda **_: pytest.fail("ran"))
...
def test_entry_point_runs_when_switched_on(monkeypatch, value):
    ...
    monkeypatch.setattr(kp, "run_promotion", _fake_run)
...
def test_entry_point_never_raises_at_its_caller(monkeypatch):
    ...
    monkeypatch.setattr(kp, "run_promotion", _explode)
...
def test_env_is_read_per_call_not_at_import(monkeypatch):
    ...
    monkeypatch.setenv(kp.PROMOTE_ENABLED_ENV, "true")
    monkeypatch.setattr(kp, "run_promotion", lambda **_: {"created": 0})
    assert kp.promote_findings_to_kanban()["enabled"] is True
