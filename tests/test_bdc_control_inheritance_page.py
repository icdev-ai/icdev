# CUI // SP-CTI
"""rmf-ui-08: /control-inheritance is GOVERNED -- it lives on the Boundary canvas.

The Control Inheritance Visualizer (CSP vs customer responsibility mapping:
inherited / shared / customer-owned controls per provider) was one of ~15
compliance pages that were bare ``@app.route`` handlers in a 10,500-line
app.py: no registry entry, NO RBAC GUARD, no completeness gate, no IQE
dispatch. A route on the Boundary & Supply Chain Canvas blueprint gets all of
those by construction -- app.py attaches ``guard_component_access("bdc",
<min_il>)`` as a ``before_request`` on every registered canvas blueprint, the
registry's ``url_prefix`` + IQE adapter put ``/boundary/*`` on the client-side
path->canvas map, and the canvas completeness gate owns the template directory.

ONE route per card, on purpose (rmf-ui-01 is the exemplar this file clones):
this file pins exactly one migration end to end -- the governed home renders,
the old URL redirects rather than 404s, both base.html copies link the new
path, the template is mirrored, and the page lands on the ``bdc`` IQE canvas.
The /api/control-inheritance/* blueprint stays in tools/dashboard/api/; only
the PAGE route moved.
...
NEW_PATH = "/boundary/control-inheritance"
OLD_PATH = "/control-inheritance"
...
TEMPLATE = (
    REPO / "tools" / "dashboard" / "templates" / "boundary_canvas" / "control_inheritance.html"
)
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "boundary_canvas"
    / "control_inheritance.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "control_inheritance.html"
OLD_TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "control_inheritance.html"
)
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(bdc_app):
    ...
    assert "Control Inheritance Visualizer" in body
    # The page still drives its own API -- only the PAGE route moved. The
    # sweep saw no /api/ prefix because the script composes its URLs from
    # this constant; the five endpoints it calls hang off it.
    assert "const API = '/api/control-inheritance'" in body
    for endpoint in ("/csps", "/summary?", "/model?", "/gap?", "/controls?"):
        assert f"API + '{endpoint}" in body, f"page no longer calls {endpoint}"
    # nav-comp-06: the reference-model badge survives the move.
    assert "Typical inheritance (reference)" in body
...
def test_no_template_links_the_ungoverned_path(root):
    """compliance.html carried a card to the old URL -- every href is repointed."""
    offenders = [
...
def test_template_lives_on_the_canvas_and_is_mirrored():
    ...
    assert 'href="/boundary/"' in _read(TEMPLATE), "template must breadcrumb back to /boundary/"
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/control_inheritance.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/control_inheritance.html"' in src
    ), "blueprint does not render boundary_canvas/control_inheritance.html"
