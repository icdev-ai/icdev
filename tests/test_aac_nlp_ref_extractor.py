...
# ── _build_summary: NLP-aware project name extraction ────────────────────────

class TestBuildSummaryNlpIntegration:
    """_build_summary should prefer nlp_ref_info project_name over regex parsing."""

    def test_uses_nlp_project_name_when_provided(self):
        nlp_info = {"project_name": "nlp-detected-name", "hosting_platform": "github"}
        result = eng._build_summary(
            "https://github.com/org/original-name",
            [],
            nlp_ref_info=nlp_info,
        )
        assert "nlp-detected-name" in result
        assert "original-name" not in result

    def test_falls_back_to_regex_when_nlp_absent(self):
        result = eng._build_summary(
            "https://github.com/org/regex-name.git",
            [],
            nlp_ref_info=None,
        )
        assert "regex-name" in result

    def test_falls_back_to_regex_when_nlp_empty_dict(self):
        result = eng._build_summary(
            "https://github.com/org/regex-name",
            [],
            nlp_ref_info={},
        )
        assert "regex-name" in result

    def test_falls_back_to_regex_when_project_name_blank(self):
        result = eng._build_summary(
            "https://github.com/org/regex-name",
            [],
            nlp_ref_info={"project_name": "", "hosting_platform": "github"},
        )
        assert "regex-name" in result

    def test_local_path_uses_nlp_name_when_provided(self):
        nlp_info = {"project_name": "my-service", "hosting_platform": "local"}
        result = eng._build_summary("/home/user/some-dir", [], nlp_ref_info=nlp_info)
        assert "my-service" in result
        assert "some-dir" not in result

    def test_local_path_falls_back_to_pathlib_name(self):
        result = eng._build_summary("/home/user/some-dir", [], nlp_ref_info=None)
        assert "some-dir" in result

    def test_backward_compat_two_arg_call(self):
        result = eng._build_summary("https://github.com/org/compat-repo", [])
        assert "compat-repo" in result
