...
test.describe('ClawHub Skill Browser', () => {
  ...
  test('clawhub page loads with heading and CUI banner', async ({ page }) => {
    ...
    await page.screenshot({
      path: '.tmp/test_runs/screenshots/clawhub_01_main_1920x1080.png',
      fullPage: true,
    });

    // Step 4: Verify heading contains "ClawHub" or "Skill Browser"
    const bodyText = await page.textContent('body');
    const headingTerms = ['ClawHub', 'Skill Browser', 'clawhub', 'skill'];
  ...
  test('search form exists and accepts input', async ({ page }) => {
    ...
    await page.screenshot({
      path: '.tmp/test_runs/screenshots/clawhub_02_search_results_1920x1080.png',
      fullPage: true,
    });
  ...
  test('import queue section is visible with action buttons', async ({ page }) => {
    ...
    await page.screenshot({
      path: '.tmp/test_runs/screenshots/clawhub_03_import_queue_1920x1080.png',
      fullPage: true,
    });
  ...
  test('no JavaScript console errors on ClawHub page', async ({ page }) => {
    ...
    if (jsErrors.length > 0) {
      console.warn('ClawHub JS errors:', jsErrors);
    }
    if (failedRequests.length > 0) {
      console.warn('ClawHub 500 responses:', failedRequests);
    }
