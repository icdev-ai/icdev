# CUI // SP-CTI
"""rmf-ui-04: /oscal is GOVERNED -- it lives on the Boundary canvas.

The OSCAL Ecosystem page (SSP/SAP/SAR/POA&M validation, catalog lookup, format
conversion -- D302-D306) was one of ~15 compliance pages that were bare
...
completeness gate owns the template directory. BDC already ships
oscal_cato_exporter.py, so an RMF ARTIFACT surface belongs here.
...
the new path, the template is mirrored, and the page lands on the ``bdc`` IQE
canvas.
...
TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "boundary_canvas" / "oscal.html"
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "boundary_canvas" / "oscal.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "oscal.html"
OLD_TEMPLATE_MIRROR = REPO / "icdev" / "tools" / "dashboard" / "templates" / "oscal.html"
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(bdc_app):
    ...
    assert "OSCAL Ecosystem" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/oscal/status" in body
    assert "/api/oscal/validate" in body
...
def test_no_template_links_the_ungoverned_url(root):
    """compliance.html (the hub) linked /oscal too -- every href must be repointed."""
    offenders = sorted(
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/oscal.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/oscal.html"' in src
    ), "blueprint does not render boundary_canvas/oscal.html"
