# CUI // SP-CTI
"""rmf-ui-05: /cato is GOVERNED -- it lives on the Boundary canvas as /boundary/cato-health.

The Continuous ATO page was one of ~15 compliance pages that were bare
...
It is deliberately NOT folded into the existing ``/boundary/cato``: that page is
a per-design assessment table over ``bd_assessments`` and the fabric posture
roll-up; this one is the fleet-wide health gauge, evidence stream,
certifications and timeline over the unchanged ``/api/cato/*`` blueprint. Two
views over two data models keep two routes and two DISTINCT titles -- a test
below pins that the two pages cannot both be called "cATO Dashboard".
...
NEW_PATH = "/boundary/cato-health"
OLD_PATH = "/cato"
SIBLING_PATH = "/boundary/cato"  # the per-design cATO Dashboard, untouched
...
TEMPLATE = TEMPLATE_DIR / "boundary_canvas" / "cato_health.html"
TEMPLATE_MIRROR = TEMPLATE_DIR_MIRROR / "boundary_canvas" / "cato_health.html"
SIBLING_TEMPLATE = TEMPLATE_DIR / "boundary_canvas" / "cato.html"
OLD_TEMPLATE = TEMPLATE_DIR / "cato.html"
OLD_TEMPLATE_MIRROR = TEMPLATE_DIR_MIRROR / "cato.html"
BASE_HTML_COPIES = (
...
def test_route_is_declared_on_the_boundary_blueprint(bdc_app):
    ...
    # The sibling stays: this card ADDS a route, it does not replace one.
    assert SIBLING_PATH in rules


def test_the_two_cato_pages_carry_distinct_titles():
    """Do NOT ship two pages both titled cATO Dashboard.

    /boundary/cato is the per-design assessment table; /boundary/cato-health is
    the fleet health gauge over /api/cato/*. A reader must be able to tell them
    apart from the tab title alone.
    """
    ...
    new_title = _title(TEMPLATE)
    sibling_title = _title(SIBLING_TEMPLATE)
    assert new_title != sibling_title
    assert "cATO Dashboard" not in new_title, new_title
...
def test_no_template_links_the_ungoverned_url(root):
    """compliance.html and mosa.html linked /cato too -- every href is repointed."""
    offenders = sorted(
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/cato_health.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/cato_health.html"' in src
    ), "blueprint does not render boundary_canvas/cato_health.html"
