# CUI // SP-CTI
"""E2E Selenium lifecycle test — ODC (Observability Design Canvas).

Tests:
  1. Page loads 200      — /observability/ returns content, no SEVERE JS errors
  2. Canvas container    — /observability/canvas/new has #canvas-container
  3. Runbooks API        — GET /observability/api/runbooks returns list
  4. Objects endpoint    — GET /observability/api/objects returns valid data

Screenshots: tests/playwright/screenshots/odc-*.png
Pass threshold: >= 3/4
"""
...
def screenshot(driver: webdriver.Chrome, name: str) -> str:
    path = SCREENSHOT_DIR / f"odc-{name}-1920x1080.png"
    try:
...
# ---------------------------------------------------------------------------
# Test 1: Page loads 200 — /observability/ returns content, no SEVERE JS errors
# ---------------------------------------------------------------------------
def test_page_load(driver: webdriver.Chrome, results: TestResult) -> None:
    """Load /observability/ and verify it renders with no SEVERE JS errors."""
    try:
        try:
        ...
        driver.get(f"{BASE_URL}/observability/")
        time.sleep(1.5)
...
# ---------------------------------------------------------------------------
# Test 2: Canvas container in DOM — /observability/canvas/new has #canvas-container
# ---------------------------------------------------------------------------
def test_canvas_container(driver: webdriver.Chrome, results: TestResult) -> None:
    ...
    try:
        driver.get(f"{BASE_URL}/observability/canvas/new")
        time.sleep(2)
...
# ---------------------------------------------------------------------------
# Test 3: Runbooks API — GET /observability/api/runbooks returns list
# ---------------------------------------------------------------------------
def test_runbooks_api(driver: webdriver.Chrome, results: TestResult) -> None:
    """Verify GET /observability/api/runbooks returns a valid response."""
    try:
        driver.get(f"{BASE_URL}/observability/")
        time.sleep(0.5)
        ...
        data = driver.execute_async_script("""
            ...
            fetch('/observability/api/runbooks')
                .then(function(r) {
        ...
        assert data is not None, "No response from /observability/api/runbooks"
        assert "__status" not in data or data["__status"] < 500, \
            f"API returned error status {data.get('__status')}"

        screenshot(driver, "03-runbooks-api")
        results.ok("runbooks_api", "GET /observability/api/runbooks — ok")
    except Exception as exc:
        screenshot(driver, "03-runbooks-api-error")
        results.fail("runbooks_api", str(exc))


# ---------------------------------------------------------------------------
# Test 4: Objects endpoint — GET /observability/api/objects returns valid data
# ---------------------------------------------------------------------------
def test_objects_endpoint(driver: webdriver.Chrome, results: TestResult) -> None:
    """Verify GET /observability/api/objects returns a 200-level response."""
    try:
        driver.get(f"{BASE_URL}/observability/")
        time.sleep(0.5)
        driver.set_script_timeout(10)
        ...
        data = driver.execute_async_script("""
            var done = arguments[arguments.length - 1];
            fetch('/observability/api/objects')
                .then(function(r) {
                    if (!r.ok) { done({__status: r.status, ok: false}); return; }
                    return r.json();
                })
        ...
        assert data is not None, "No response from /observability/api/objects"
        ...
        results.ok("objects_endpoint", "GET /observability/api/objects — ok")
    except Exception as exc:
...
def main() -> int:
    print("\n" + "=" * 70)
    print("  ICDEV(tm) ODC Lifecycle E2E — Observability Design Canvas")
    ...
    try:
        ...
        test_runbooks_api(driver, results)
