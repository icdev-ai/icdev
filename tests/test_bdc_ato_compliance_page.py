# CUI // SP-CTI
"""rmf-ui-01: /ato-compliance is GOVERNED -- it lives on the Boundary canvas.

The ATO Compliance Dashboard was one of ~15 compliance pages that were bare
...
mode is a silently dropped page. So this file pins exactly one migration end to
end -- the governed home renders, the old URL redirects rather than 404s, both
base.html copies link the new path, the template is mirrored, and the page
lands on the ``bdc`` IQE canvas -- and the next card copies the shape.
...
NEW_PATH = "/boundary/ato-compliance"
OLD_PATH = "/ato-compliance"
...
TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "boundary_canvas" / "ato_compliance.html"
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "boundary_canvas" / "ato_compliance.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "ato_compliance.html"
OLD_TEMPLATE_MIRROR = REPO / "icdev" / "tools" / "dashboard" / "templates" / "ato_compliance.html"
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(bdc_app):
    ...
    assert "ATO Compliance Dashboard" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/ato-compliance/dashboard/" in body
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/ato_compliance.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/ato_compliance.html"' in src
    ), "blueprint does not render boundary_canvas/ato_compliance.html"
