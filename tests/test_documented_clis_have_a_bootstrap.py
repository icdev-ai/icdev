#!/usr/bin/env python3
# NOTE (merge of kax-conflict-04 and kax-conflict-05): two sweeps rewrote
# tests/test_script_bootstrap_import_order.py at once, and NEITHER was a superset
# — one gates ORDER (a bootstrap exists but an import sits above it), this one
# gates EXISTENCE (no bootstrap at all: 705 modules, 280 of them documented in
# PATH form). Splicing two 400-line modules with different helper architectures
# risks losing a check silently, so they stay as two files, one per defect. The
# few tests that appear in both are duplicate coverage, which is harmless.
