...
"""
        ...
        assert any(f["name"] == "api_route_without_auth_decorator" for f in result["findings"])
...
"""
        result = scanner.scan_content(code, "python")
        auth_findings = [f for f in result["findings"] if f["name"] == "api_route_without_auth_decorator"]
...
"""
        result = scanner.scan_content(code, "python")
        val_findings = [f for f in result["findings"] if f["name"] == "write_route_without_input_validation"]
...
class TestFileExclusion:
    def test_test_file_excluded(self, scanner):
        with tempfile.NamedTemporaryFile(suffix=".py", prefix="test_", mode="w", delete=False) as f:
...
class TestCLI:
    ...
    def test_cli_json_output(self):
        with tempfile.TemporaryDirectory() as td:
            p = Path(td) / "api.py"
            p.write_text('@app.route("/test")\ndef t(): pass\n', encoding="utf-8")
            result = subprocess.run(
                [sys.executable, "tools/security/endpoint_security_scanner.py", "--dir", td, "--json"],
                capture_output=True,
                text=True,
                cwd=str(Path(__file__).parent.parent),
            )
