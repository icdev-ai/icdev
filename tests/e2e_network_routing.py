...
def run_tests():
    ...
    try:
        ...
        try:
            d = api(driver, "POST", "/network/api/routing-entries", {
                "device_ip": "10.0.0.1",
                "hostname": "core-r1",
                "entries": [
                    {"prefix": "10.1.0.0/24", "next_hop": "10.0.0.2",
                     "protocol": "ospf", "metric": 110, "interface": "Gi0/0"},
                    {"prefix": "10.2.0.0/24", "next_hop": "10.0.0.3",
                     "protocol": "bgp", "metric": 20, "admin_distance": 20,
                     "interface": "Gi0/1"},
                    {"prefix": "10.0.0.0/24", "next_hop": "0.0.0.0",
                     "protocol": "connected", "interface": "Gi0/0"},
                ]
            })
            ...
            d = api(driver, "POST", "/network/api/routing-entries", {
                ...
                "entries": [
                    ...
                    {"prefix": "10.1.0.0/24", "next_hop": "0.0.0.0",
                     "protocol": "connected", "interface": "Vlan100"},
                    {"prefix": "10.3.0.0/24", "next_hop": "10.0.0.4",
                     "protocol": "ospf", "interface": "Gi0/2"},
                ]
            })
            assert d.get("stored") == 3
        ...
        try:
            d = api(driver, "GET",
                    "/network/api/routing-entries?device_ip=10.0.0.1")
            assert len(d) >= 3
        ...
        try:
            d = api(driver, "POST",
                    "/network/api/routing-topology", {
                        ...
                        "name": "E2E Routing Topology"
                    })
            assert d.get("id")
            assert d.get("nodes") >= 3  # r1, sw1, + next-hops
            assert d.get("edges") >= 2
            topo_id = d["id"]
            results.ok("generate_routing_topology",
                        f"nodes={d['nodes']}, edges={d['edges']}, "
                        f"devices={d['devices_discovered']}")
        except Exception as e:
        ...
        if topo_id:
            try:
                t = api(driver, "GET",
                        f"/network/api/topologies/{topo_id}")
                gj = t.get("graph_json", "{}")
                ...
                results.ok("verify_routing_topology",
                            f"labels: {labels}")
            except Exception as e:
        ...
        try:
            ...
            t = api(driver, "POST", "/network/api/topologies", {
                "name": "Sync Test Topo",
                "graph_json": {"nodes": [
                    {"id": "n1", "label": "core-r1", "type": "router",
                     "x": 100, "y": 100, "config": {"ip": "10.0.0.1"}},
                ], "edges": []}
            })
            sync_topo = t.get("id")
            # Store a version config for this device
            api(driver, "POST", "/network/api/collected-configs", {
                "device_ip": "10.0.0.1",
                "hostname": "core-r1",
                "command_name": "version",
                "output_text": "Cisco IOS XE 17.9",
                "parsed_json": {"version": "17.9.4a",
                                "model": "ASR1001-X",
                                "serial": "FOC12345"},
            ...
            # Sync
            d = api(driver, "POST",
                    f"/network/api/config-to-canvas/{sync_topo}")
            assert d.get("updated_devices") >= 1, \
                f"Expected 1+ updated, got {d}"
            # Verify synced
            t2 = api(driver, "GET",
                     f"/network/api/topologies/{sync_topo}")
            ...
            results.ok("config_to_canvas_sync",
                        f"version={cfg.get('sw_version')}, "
