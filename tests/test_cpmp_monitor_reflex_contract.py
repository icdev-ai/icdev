...
2. Pass 3 read `nc.get("noncompliance")` and `finding["issue_type"]` /
   ...
   therefore always empty and no [SUBCON] card had ever been filed — 0 on the
   board against 14 [CPMP] cards from the separate pmo_ai_advisor path.

   This half is ALREADY FIXED on main by a separate change; the tests below are
   regression pins, not a repro. They are kept because both defects are contract
   bugs *between two modules* — the shape that re-breaks silently on the next
   rename, with no exception and no error entry to notice it by.
...
def test_flat_result_keys_survive_alongside_the_envelope(monitor, monkeypatch):
    """The envelope is ADDITIVE — three gated suites read this return flat.

    test_genesis_reflex_cpmp_monitor.py, _card_identity.py and _subcon_pass.py
    do `run()["cards_created"]` / `["subcon_alerts"]` directly. Returning only
    {success, metric_value, details} breaks 16 of their assertions with
    KeyError, so the results payload is spread alongside the envelope. This pins
    that, because the redundancy looks removable to anyone who did not hit it.
    """
    _patch_get_connection(monkeypatch, lambda *a, **k: _FakeConn([]))
    result = monitor.run()

    for key in ("contracts_scanned", "cards_created", "subcon_alerts", "errors"):
        assert key in result, f"{key!r} must stay readable flat off run()"
        assert result[key] == result["details"][key], "the two views must not drift"
...
def test_monitor_reads_findings_and_keys_dedup_on_category_and_sub_id(monitor):
    ...
    assert 'finding.get("category")' in src
