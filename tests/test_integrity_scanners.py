...
def test_known_safe_persistence_module_is_suppressed(staged_env, tmp_path, monkeypatch):
    """Regression (task-1b49742e56): ``known_safe_persistence_modules`` suppresses.

    The allowlist was added on 2026-07-28 (task-e74c6d806f) and its loader
    ``_load_safe_persistence_modules`` was written — but ``run_signature_scan``
    only ever consulted the dynamic_import allowlist, so every entry suppressed
    nothing and each 6h integrity_monitor sweep re-raised the same reviewed
    false positive as a fresh kanban card (assessment 274 re-reported
    ``cli/provision_db.py`` and ``network/seed_sops.py`` sixteen days later).

    A persistence hit in an allowlisted module is dropped; the same hit in an
    unlisted module, and a non-persistence hit in an allowlisted one, are kept.
    """
    aid = _new_assessment(staged_env)
    staged = tmp_path / "x"
    monkeypatch.setattr(
    ...
    # on Windows — which is why the matcher normalises with Path.as_posix().
    kept = {(Path(r["file_path"]).as_posix(), r["line"]) for r in _findings(aid)}
    ...
    # The allowlist is narrow: it excuses one category in one module, not the module.
...
def test_shared_checks_is_allowlisted_for_persistence():
    ...
    comment, two docstrings and the BLOCKED message of the write-containment
    guard that REFUSES writes to exactly that path. ``sipa-persistence-generic``
    is a ``languages: [generic]`` regex and cannot tell prose from code.
    ...
    as its root, so it reports ``hooks/shared_checks.py``.
    """
