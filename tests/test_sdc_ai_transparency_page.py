# CUI // SP-CTI
"""rmf-ui-15: /ai-transparency is GOVERNED -- it lives on the Security canvas.

The AI Transparency page (OMB M-25-21 / M-26-04 / NIST AI 600-1 / GAO-21-519SP
posture reporting) was one of ~15 compliance pages that were bare
...
NEW_PATH = "/security/ai-transparency"
OLD_PATH = "/ai-transparency"
...
TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "security_canvas" / "ai_transparency.html"
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "security_canvas" / "ai_transparency.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "ai_transparency.html"
OLD_TEMPLATE_MIRROR = REPO / "icdev" / "tools" / "dashboard" / "templates" / "ai_transparency.html"
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(sdc_app):
    ...
    assert "AI Transparency &amp; Accountability" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/ai-transparency/stats" in body
    assert "/api/ai-transparency/audit" in body
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("security_canvas/ai_transparency.html"' in src or (
        "render_template(\n" in src and '"security_canvas/ai_transparency.html"' in src
    ), "blueprint does not render security_canvas/ai_transparency.html"
