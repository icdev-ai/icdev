...
def _insert_egress_data(db_path, child_id="child-1", connections=None,
    ...
    conn.execute(
        """INSERT INTO child_telemetry
           ...
           VALUES (?, ?, 'egress', ?, ?, ?)""",
        (str(uuid.uuid4()), child_id, json.dumps(data),
...
class TestStoreEgress:
    def test_store_egress_data(self, monitor, monitor_db):
        ...
        count = conn.execute(
            "SELECT COUNT(*) FROM child_telemetry WHERE metric_type = 'egress'"
        ).fetchone()[0]
        conn.close()
        assert count == 1
    ...
    def test_store_sets_healthy_status(self, monitor, monitor_db):
        ...
        conn = sqlite3.connect(str(monitor_db))
        row = conn.execute(
            "SELECT health_status FROM child_telemetry WHERE child_id = 'c1'"
        ).fetchone()
        conn.close()
    ...
    def test_store_sets_degraded_on_error(self, monitor, monitor_db):
        ...
        conn = sqlite3.connect(str(monitor_db))
        row = conn.execute(
            "SELECT health_status FROM child_telemetry WHERE child_id = 'c2'"
        ).fetchone()
        conn.close()
...
class TestCLI:
    def test_cli_summary_json(self, monitor_db):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "tools.registry.egress_monitor",
             "--summary", "--child-id", "child-1",
             "--hours", "24", "--json",
             "--db-path", str(monitor_db)],
    ...
    def test_cli_evaluate_json(self, monitor_db):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "tools.registry.egress_monitor",
             "--evaluate", "--child-id", "child-1",
             "--json", "--db-path", str(monitor_db)],
            capture_output=True, text=True, timeout=30,
        )
        assert result.returncode == 0
        data = json.loads(result.stdout)
    ...
    def test_cli_collect_unreachable_json(self, monitor_db):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "tools.registry.egress_monitor",
             "--collect", "--child-id", "child-1",
             "--endpoint", "http://127.0.0.1:1/health/egress",
             "--json", "--db-path", str(monitor_db)],
            capture_output=True, text=True, timeout=30,
        )
        assert result.returncode == 0
