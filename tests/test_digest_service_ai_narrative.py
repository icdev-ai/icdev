...
# ---------------------------------------------------------------------------
# Pipeline integration tests (aiify-opp-5938): send_* functions
# ---------------------------------------------------------------------------
...
def test_aiify_weekly_digest_narrative_attached_when_llm_available(monkeypatch):
    """narrative key contains LLM text when ai_narrative=True and LLM succeeds."""
    scans = [
...
def test_aiify_weekly_digest_narrative_none_on_llm_failure(monkeypatch):
    """AI-ify digest ships even when LLM raises; narrative degrades to None."""
    scans = [{"scan_id": 41, "status": "complete", "overall_ai_readiness": 0.75, "created_at": "2026-06-02"}]
    top_opps = [{"function_name": "handle_event", "pattern_type": "event_driven_ml", "composite_score": 0.88}]
    pattern_summary = [{"pattern_type": "event_driven_ml", "cnt": 4}]
    _fake_digest_conn(monkeypatch, rows_by_call=[scans, top_opps, pattern_summary])
    _fake_router(monkeypatch, raises=RuntimeError("provider unavailable"))
...
def test_audit_trail_summary_narrative_attached_when_llm_available(monkeypatch):
    """narrative key contains LLM text when ai_narrative=True and LLM succeeds."""
    events = [
    ...
    actors = [{"actor": "sovanna", "cnt": 20}, {"actor": "scanner", "cnt": 6}]
    _fake_digest_conn(monkeypatch, rows_by_call=[events, actors])
...
def test_audit_trail_summary_narrative_none_on_llm_failure(monkeypatch):
    """Audit summary ships even when LLM raises; narrative degrades to None."""
...
def test_oracle_weekly_digest_no_narrative_by_default(monkeypatch):
    ...
    top_preds = [{"title": "Security breach imminent", "severity": "high", "confidence": 0.92}]
    lens_stats = [{"lens_id": "risk", "cnt": 4}]
...
def test_oracle_weekly_digest_narrative_attached_when_llm_available(monkeypatch):
    ...
    predictions = [{"severity": "critical", "cnt": 1, "avg_conf": 0.95}]
    top_preds = [{"title": "Privilege escalation path", "severity": "critical", "confidence": 0.95}]
    lens_stats = [{"lens_id": "risk", "cnt": 5}]
    _fake_digest_conn(monkeypatch, rows_by_call=[predictions, top_preds, lens_stats])
    _fake_router(monkeypatch, content="One critical prediction this week; address the privilege escalation path immediately.")

    result = digest_service.send_oracle_weekly_digest("ops@example.com", ai_narrative=True)

    assert result["status"] == "sent"
    assert result["prediction_groups"] == 1
    assert result["narrative"] == "One critical prediction this week; address the privilege escalation path immediately."


def test_oracle_weekly_digest_narrative_none_on_llm_failure(monkeypatch):
    """Oracle digest ships even when LLM raises; narrative degrades to None."""
...
# --- send_compliance_posture_report ---
...
def test_compliance_posture_report_no_narrative_by_default(monkeypatch):
    ...
    open_findings = [{"severity": "CAT1", "cnt": 2}]
...
def test_compliance_posture_report_narrative_attached_when_llm_available(monkeypatch):
    ...
    open_findings = [{"severity": "CAT1", "cnt": 1}]
    _fake_digest_conn(monkeypatch, rows_by_call=[canvases, overall, open_findings])
    _fake_router(monkeypatch, content="Overall posture at 78%; security canvas is the weakest link — remediate the CAT1 finding first.")
    ...
    assert result["narrative"] == "Overall posture at 78%; security canvas is the weakest link — remediate the CAT1 finding first."


def test_compliance_posture_report_narrative_none_on_llm_failure(monkeypatch):
    """Posture report ships even when LLM raises; narrative degrades to None."""
...
def test_genesis_phase_summary_narrative_attached_when_llm_available(monkeypatch):
    ...
    _fake_digest_conn(monkeypatch, rows_by_call=[phases, design, reflexes])
    _fake_router(monkeypatch, content="Modernization Engine has completed design and build; review is underway with two high-confidence reflexes fired.")

    result = digest_service.send_genesis_phase_summary("design-abc123", "team@example.com", ai_narrative=True)

    assert result["status"] == "sent"
    assert result["phases"] == 3
    assert result["narrative"] == "Modernization Engine has completed design and build; review is underway with two high-confidence reflexes fired."


def test_genesis_phase_summary_narrative_none_on_llm_failure(monkeypatch):
    """Genesis summary ships even when LLM raises; narrative degrades to None."""
...
def test_zig_maturity_report_no_narrative_by_default(monkeypatch):
    ...
    gaps = [{"pillar_slug": "devices", "title": "Device attestation", "phase": "1"}]
...
def test_zig_maturity_report_narrative_attached_when_llm_available(monkeypatch):
    ...
    gaps = [{"pillar_slug": "apps", "title": "SBOM integration", "phase": "1"}]
    _fake_digest_conn(monkeypatch, rows_by_call=[pillars, gaps])
...
def test_poam_due_soon_digest_narrative_none_on_llm_failure(monkeypatch):
    """POA&M digest ships even when LLM raises; narrative degrades to None."""
    due_items = [
