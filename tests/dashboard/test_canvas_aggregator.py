...
Uses in-memory SQLite databases injected via a patch on _get_canvas_conn so
that no real canvas DB files are required.  Each test class also exercises the
60-second result cache independently.
"""
...
import tools.dashboard.canvas_aggregator as agg
from tools.dashboard.canvas_aggregator import (
    get_canvas_activity_trend,
    get_canvas_compliance_summary,
    get_canvas_finding_counts,
    get_recent_canvas_findings,
...
class TestGetCanvasComplianceSummary:
    ...
    def test_unavailable_when_db_missing(self):
        conns = _build_all_conns(skip_canvases={"security", "network"})
        with _patch_conns(conns):
            result = get_canvas_compliance_summary()
        slugs = {r["slug"]: r for r in result}
        assert not slugs["security"]["available"]
        assert not slugs["network"]["available"]
        for slug, row in slugs.items():
            if slug not in ("security", "network"):
                assert row["available"], f"{slug} should be available"

    def test_json_canvas_score_grade_and_counts(self):
        findings = [
            {"rule_id": "B-01", "title": "Missing encryption", "severity": "CAT1", "status": "open", "affected_entity": "S3"},
            {"rule_id": "B-02", "title": "Weak password", "severity": "CAT2", "status": "remediated", "affected_entity": "EC2"},
        ]
        conns = _build_all_conns(
            extra_assessments={
                "boundary": [
        ...
        assert row["last_assessed"] == "2026-04-10T12:00:00"
    ...
    def test_direct_canvas_score_from_checks(self):
        ...
        assert row["open_findings"] == 1
        assert row["closed_findings"] == 1
        assert row["last_assessed"] == "2026-04-11T09:00:00"
    ...
    def test_result_is_cached_on_second_call(self):
        ...
        with patch.object(agg, "_get_canvas_conn", side_effect=counting_conn):
            ...
            get_canvas_compliance_summary()
            assert call_count[0] == first_count, "Second call should use cached result"
    ...
    def test_malformed_findings_json_is_skipped(self):
        ...
        # Must not raise; counts stay at zero
        assert row["open_findings"] == 0
        assert row["closed_findings"] == 0

    def test_accepted_risk_and_false_positive_count_as_closed(self):
        findings = [
        ...
        conns = _build_all_conns(
            extra_assessments={
                "data": [
                    {"id": "a1", "score": 60.0, "created_at": "2026-04-10", "findings_json": json.dumps(findings)},
                ]
            }
        )
        with _patch_conns(conns):
            result = get_canvas_compliance_summary()
        row = next(r for r in result if r["slug"] == "data")
        assert row["closed_findings"] == 2
        assert row["open_findings"] == 1
