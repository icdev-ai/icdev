...
class TestStubDetectorPipeline:
    ...
    def test_gate_evaluation_pass(self, real_python_file):
        ...
        results = verify_directory(
            real_python_file.parent, max_level="substantive"
        )
    ...
    def test_store_results(self, tmp_path, tmp_db, real_python_file):
        ...
        row = conn.execute(
            "SELECT * FROM stub_detection_results WHERE id = ?", (rid,)
        ).fetchone()
        conn.close()
...
class TestCLIIntegration:
    ...
    def test_stub_detector_file(self, real_python_file):
        ...
        result = subprocess.run(
            ["python", "tools/testing/stub_detector.py",
             "--file", str(real_python_file), "--json"],
    ...
    def test_context_pressure_health(self):
        import subprocess
        result = subprocess.run(
            ["python", "tools/agent/context_pressure.py",
             "--check", "health", "--json"],
            capture_output=True, text=True, timeout=30,
        )
        assert result.returncode == 0
    ...
    def test_deviation_rules_classify(self):
        ...
        result = subprocess.run(
            ["python", "tools/knowledge/deviation_rules.py",
             "--classify", failure],
            capture_output=True, text=True, timeout=30,
        )
        assert result.returncode == 0
