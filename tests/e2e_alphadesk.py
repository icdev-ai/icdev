...
class TestResult:
    ...
    def summary(self):
        ...
        return {
            ...
            "pass_rate": (
                f"{len(self.passed)/total*100:.1f}%" if total else "0%"
            ),
    ...
    for path, expected, slug in pages:
        try:
            ...
            assert expected in h1.text, (
                f"Expected '{expected}' in h1, got '{h1.text}'"
            )
    ...
    try:
        ...
        WebDriverWait(driver, 15).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, "#live-result .stat-grid")
            )
        )
...
def test_run_history(driver, results):
    try:
        ...
        rows = driver.find_elements(
            By.CSS_SELECTOR, "#runs-body tr"
        )
        # Filter out empty-state rows
        data_rows = [
            r for r in rows if "empty-state" not in r.get_attribute("innerHTML")
        ]

        assert len(data_rows) >= 1, (
            f"Expected >=1 run in history, got {len(data_rows)}"
        )
...
def test_signals_page(driver, results):
    try:
        ...
        assert total_val >= 1, (
            f"Expected >=1 total signal, got {total_val}"
        )

        # Check table has signal rows
        rows = driver.find_elements(
            By.CSS_SELECTOR, "#signals-body tr"
        )
        data_rows = [
            r for r in rows
            if "empty-state" not in r.get_attribute("innerHTML")
        ]
        assert len(data_rows) >= 1, (
            f"Expected >=1 signal row, got {len(data_rows)}"
        )

        # Check approve/reject buttons exist for pending signals
        approve_btns = driver.find_elements(
            By.CSS_SELECTOR, ".btn-approve"
        )
        screenshot(driver, "signals-pending")
        results.ok(
            "signals_page",
            f"{total_val} signals, {pending_val} pending, "
            f"{len(approve_btns)} approve buttons",
        )
    except Exception as exc:
...
def test_approve_signal(driver, results):
    try:
        ...
        if not approve_btns:
            results.ok(
                "approve_signal", "No pending signals to approve (skip)"
            )
            return
        ...
        approved_badges = driver.find_elements(
            By.CSS_SELECTOR, ".badge.pass"
        )
        ...
        results.ok(
            "approve_signal",
            f"Signal approved, badge count={len(approved_badges)}, "
            f"approved={approved_count.text}",
        )
    except Exception as exc:
...
def main():
    ...
    print(
        f"Result: {status} — {s['passed']}/{s['total']} "
        f"tests passed ({s['pass_rate']})"
    )
    if s["failures"]:
        print("Failures:")
        for f in s["failures"]:
