...
def _watcher(prs, merged):
    ...
    # `**_kw` because `_auto_merge` also takes the PR record now
    # (kpr-watch-04): the shared chokepoint answers the hold-label question
    # for BOTH callers, so a future one cannot route around the label.
...
def test_cli_is_read_only():
    ...
    # (`measure_behind_by`), task-det-295a9bb95e the third
    # (`fetch_required_checks`, a GET of the branch-protection rule), and
    # kpr-watch-16 the fourth (`_gh_rest`, the REST fallback taken when
    # `gh pr list` is refused on GraphQL budget) -- so this counts argvs rather
    # than asserting there is one, but each is checked below and a new one
    ...
    assert len(shells) == 4, (
        "expected four subprocess references, got %s"
    ...
    assert len(argvs) == 4, "expected four argv lists, got %d" % len(argvs)
    ...
    # TWO bare `api` argvs now: fetch_required_checks and kpr-watch-16's
    # `_gh_rest`. Both carry their path as a VARIABLE, so the literal list is
    # just ["api"] -- which is why the write-flag loop below, not this
    # enumeration, is what proves they stay GETs.
    assert literals.count(["api"]) == 2, literals
