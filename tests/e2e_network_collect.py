...
def run_tests():
    ...
    try:
        ...
        try:
            d = api(driver, "POST", "/network/api/connect-collect", {
                "device_ip": "10.99.0.1",
            ...
            results.ok("manual_paste_collect",
                        f"commands={d['commands_executed']}")
        ...
        try:
            ...
            if d.get("hint"):
                ...
                results.ok("ssh_collect_hint",
                            f"SSH error (expected): {str(d['error'])[:60]}")
            else:
        ...
        try:
            d = api(driver, "POST", "/network/api/import/extract-data", {
                "format": "drawio",
            ...
            results.ok("import_extract_data",
                        f"devices={ext['devices']}, "
                        f"ipam={ext.get('ipam_blocks', 0)}, "
                        f"circuits={ext.get('circuits', 0)}")
        except Exception as e:
        ...
        try:
            ...
            results.ok("profile_dropdown",
                        f"{len(options)} profiles")
        except Exception as e:
