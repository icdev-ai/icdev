...
class TestTransfer:
    ...
    def test_bandwidth_throttle(self, tmp_src, tmp_dst):
        ...
        t0 = time.monotonic()
        result = transfer_file(provider, tmp_src, provider, tmp_dst, "hello.txt",
                               verify=False, bandwidth_limit_kbps=1)
...
class TestSFTPProvider:
    ...
    def test_env_var_config(self):
        from tools.filesync.providers.sftp import SFTPSyncProvider
        with patch.dict(os.environ, {"ICDEV_SFTP_HOST": "test.example.com",
                                      "ICDEV_SFTP_PORT": "2222",
                                      "ICDEV_SFTP_USER": "testuser"}):
...
class TestFileWatcher:
    ...
    def test_polling_detects_new_file(self, tmp_path):
        ...
        watcher = FileWatcher(str(watch_dir), on_changes, debounce_seconds=0.1,
                              fallback_scan_interval=0.5)
        watcher.start()
        time.sleep(0.3)
        # Create a new file
