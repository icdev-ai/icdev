...
class TestGenomeManager:
    ...
    def test_create_version(self, evo_db):
        ...
        result = gm.create_version(
            genome_data={"capabilities": ["prompt_injection_defense", "ai_telemetry"]}, created_by="test"
        )
    ...
    def test_get_current(self, evo_db):
        ...
        assert (
            "capabilities" in json.loads(current["genome_data"])
            if isinstance(current["genome_data"], str)
            else current["genome_data"]
        )
...
class TestPropagationManager:
    ...
    def test_prepare_propagation(self, evo_db):
        ...
        conn.execute(
            "INSERT INTO child_app_registry (id, child_name, parent_project_id, status, created_at) VALUES (?, ?, ?, ?, ?)",  # noqa: E501
            ("child-1", "TestChild", "proj-1", "active", now),
        )
        conn.commit()
        conn.close()

        result = pm.prepare_propagation(capability_id="cap-1", target_children=["child-1"])
        assert result is not None
...
class TestMarketplaceGate8:
    ...
    def test_scan_clean_asset(self, tmp_path, evo_db):
        ...
        scan_id, result = scan_prompt_injection(str(asset_dir), "asset-1", "ver-1", db_path=evo_db)
    ...
    def test_scan_malicious_asset(self, tmp_path, evo_db):
        ...
        scan_id, result = scan_prompt_injection(str(asset_dir), "asset-2", "ver-2", db_path=evo_db)
...
class TestMarketplaceGate9:
    ...
    def test_scan_safe_asset(self, tmp_path, evo_db):
        ...
        scan_id, result = scan_behavioral_sandbox(str(asset_dir), "asset-3", "ver-3", db_path=evo_db)
        assert result["status"] == "pass"
    ...
    def test_detect_dangerous_patterns(self, tmp_path, evo_db):
        ...
        scan_id, result = scan_behavioral_sandbox(str(asset_dir), "asset-4", "ver-4", db_path=evo_db)
    ...
    def test_generate_for_csp_aws(self, tmp_path):
        ...
        try:
            files = generate_for_csp(
                str(tmp_path), {"project_name": "test", "environment": "dev", "db_name": "testdb"}, csp="aws"
            )
            assert len(files) > 0
        except Exception:
