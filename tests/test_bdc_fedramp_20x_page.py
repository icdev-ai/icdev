# CUI // SP-CTI
"""rmf-ui-06: /fedramp-20x is GOVERNED -- it lives on the Boundary canvas.

The FedRAMP 20x KSI Dashboard was one of ~15 compliance pages that were bare
...
ONE route per card, on purpose: a 15-route move is unreviewable and its failure
mode is a silently dropped page. This file is the rmf-ui-01 exemplar's shape
(tests/test_bdc_ato_compliance_page.py) cloned for the KSI page -- an RMF
ARTIFACT surface (KSI evidence, maturity levels, authorization package), an
orphan until rmf-inert-01 linked it. It pins exactly one migration end to end -- the governed home renders, the old URL redirects rather than 404s, both
base.html copies link the new path, the template is mirrored, and the page
lands on the ``bdc`` IQE canvas.
...
NEW_PATH = "/boundary/fedramp-20x"
OLD_PATH = "/fedramp-20x"
...
TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "boundary_canvas" / "fedramp_20x.html"
TEMPLATE_MIRROR = (
    REPO / "icdev" / "tools" / "dashboard" / "templates" / "boundary_canvas" / "fedramp_20x.html"
)
OLD_TEMPLATE = REPO / "tools" / "dashboard" / "templates" / "fedramp_20x.html"
OLD_TEMPLATE_MIRROR = REPO / "icdev" / "tools" / "dashboard" / "templates" / "fedramp_20x.html"
BASE_HTML_COPIES = (
...
def test_governed_page_renders_for_an_authenticated_session(bdc_app):
    ...
    assert "FedRAMP 20x" in body and "Key Security Indicators" in body
    # The page still drives its own API -- only the PAGE route moved.
    assert "/api/fedramp-20x/ksis" in body
...
def test_blueprint_route_references_the_canvas_template():
    src = _read(BLUEPRINT)
    assert 'render_template("boundary_canvas/fedramp_20x.html"' in src or (
        "render_template(\n" in src and '"boundary_canvas/fedramp_20x.html"' in src
    ), "blueprint does not render boundary_canvas/fedramp_20x.html"
