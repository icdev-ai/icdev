    ...
    try:
        ...
        conn.execute(
            """CREATE TABLE IF NOT EXISTS dic_section_annotations (
                   ann_id TEXT PRIMARY KEY, section_id TEXT NOT NULL,
                   doc_id TEXT NOT NULL, selected_text TEXT NOT NULL DEFAULT '',
                   category TEXT NOT NULL, comment TEXT NOT NULL,
...
def _comment(section_id: str, text: str, *, author: str = "alice",
             created_at: str = "2026-09-08T01:00:00+00:00") -> str:
    ann_id = f"ann_{uuid.uuid4().hex[:12]}"
    conn = get_connection()
    conn.execute(
        "INSERT INTO dic_section_annotations (ann_id, section_id, doc_id, category, "
        "comment, author, status, created_at) VALUES (%s,%s,'doc-1','improvement',"
        "%s,%s,'open',%s)",
        (ann_id, section_id, text, author, created_at),
    )
    conn.commit()
    conn.close()
    return ann_id
...
class TestThreadSelection:
    ...
    def test_a_resolved_comment_is_not_an_instruction(self):
        sug_id, _fid = _seed_change()
        _comment("sec-1", "still open")
        conn = get_connection()
        conn.execute("INSERT INTO dic_section_annotations (ann_id, section_id, doc_id, "
                     "category, comment, author, status, created_at) VALUES "
                     "('ann_done','sec-1','doc-1','improvement','already handled',"
                     "'bob','resolved','2026-09-08T00:00:00+00:00')")
        conn.commit()
        conn.close()
        sel = rd.thread_for_change(store.get_suggestion(sug_id))
        assert sel.total == 1
...
class TestSupersede:
    def test_a_supersede_names_its_successor(self):
        old, _ = _seed_change()
        new, _ = _seed_change()
        assert store.supersede_suggestion(old, new, "redraft requested by alice") is True
        row = store.get_suggestion(old)
        assert row["status"] == "superseded"
        assert row["superseded_by"] == new
        assert "alice" in row["superseded_reason"]

    def test_a_supersede_with_no_successor_is_refused(self):
        old, _ = _seed_change()
        with pytest.raises(ValueError):
            store.supersede_suggestion(old, "")
        with pytest.raises(ValueError):
            store.supersede_suggestion(old, old)
    ...
    def test_a_decided_change_is_never_superseded(self):
        ...
        assert store.supersede_suggestion(old, new) is False
        assert store.get_suggestion(old)["status"] == "rejected"

    def test_a_supersede_records_NO_decision(self):
        """A redraft request is not a review verdict, and
        dic_suggestion_decisions is what cef-ui-03 reads to answer 'was this
        reviewed?'."""
        old, _ = _seed_change()
        new, _ = _seed_change()
        store.supersede_suggestion(old, new)
        assert store.get_decisions_for_suggestion(old) == []
...
class TestRedraftEndToEnd:
    def test_comment_press_redraft_new_change_cited_old_superseded(self, monkeypatch):
        ...
        assert old["superseded_by"] == res.new_suggestion_id
