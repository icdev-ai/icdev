...
def run_tests():
    ...
    try:
        ...
        try:
            d = api(driver, "POST", "/network/api/import", {
                "format": "drawio", "content": SAMPLE_DRAWIO,
                "name": "E2E Semantic Import Test"
            })
            assert d.get("id"), f"No id: {d}"
            topo_ids.append(d["id"])
            assert d["nodes"] == 4
            assert d["edges"] == 2
            results.ok("semantic_import_drawio",
                        f"id={d['id']}, {d['nodes']}N/{d['edges']}E")
        ...
        if topo_ids:
            try:
                t = api(driver, "GET",
                        f"/network/api/topologies/{topo_ids[0]}")
                gj = t.get("graph_json", "{}")
                graph = json.loads(gj) if isinstance(gj, str) else gj
                types = {n["label"]: n["type"] for n in graph["nodes"]}
                assert types.get("Core-Router-01") == "router", \
                    f"Router: {types.get('Core-Router-01')}"
                assert types.get("Firewall-DMZ") == "firewall", \
                    f"FW: {types.get('Firewall-DMZ')}"
                assert types.get("Dist-Switch-L3") == "switch-l3", \
                    f"SW: {types.get('Dist-Switch-L3')}"
                assert types.get("App-Server-01") == "server", \
                    f"Srv: {types.get('App-Server-01')}"
                results.ok("classification_correct",
                            str(types))
            except Exception as e:
        ...
        # ── 3. Bulk import ────────────────────────────────────────────────
        try:
            d = api(driver, "POST", "/network/api/import/bulk", {
                "files": [
                    {"format": "drawio", "content": SAMPLE_DRAWIO,
                     "name": "Bulk Import A"},
                    {"format": "drawio", "content": SAMPLE_DRAWIO_2,
                     "name": "Bulk Import B"},
                ]
            })
        ...
        try:
            proj = api(driver, "POST", "/network/api/projects", {
                "name": "E2E Import Project", "status": "draft"
            })
            pid = proj.get("id")
            d = api(driver, "POST", "/network/api/import/bulk", {
                "project_id": pid,
            ...
            project_import = next(
                (t for t in topos if t["name"] == "Project Import"), None)
        ...
        try:
            ...
            d = api(driver, "POST", "/network/api/import/stitch", {
                "topology_ids": topo_ids[:2],
                "name": "E2E Stitched Topology"
            })
            assert d.get("id"), f"No stitch id: {d}"
            assert d.get("source_topologies") == 2
            assert d.get("nodes") >= 6  # 4 + 2 from both
            topo_ids.append(d["id"])
            results.ok("stitch_topologies",
                        f"nodes={d['nodes']}, edges={d['edges']}")
        except Exception as e:
        ...
        try:
            ...
            topo_ids.append(d["id"])
            results.ok("stitch_with_interconnects",
                        f"edges={d['edges']}")
        except Exception as e:
        ...
        # ── 7. Import-as-audit ────────────────────────────────────────────
        try:
            d = api(driver, "POST", "/network/api/import/audit", {
                "format": "drawio", "content": SAMPLE_DRAWIO,
                "name": "E2E Audit Import"
            })
            assert d.get("id")
            ...
            topo_ids.append(d["id"])
            results.ok("import_as_audit",
                        f"findings={d['compliance']['findings']}, "
                        f"types={d['classified_types']}")
        except Exception as e:
            results.fail("import_as_audit", e)

        # ── 8. Re-classify nodes ──────────────────────────────────────────
        try:
            d = api(driver, "POST", "/network/api/classify-nodes", {
                "topology_id": topo_ids[0]
            })
            assert "reclassified" in d
            results.ok("reclassify_nodes",
                        f"changed={d['reclassified']}")
        except Exception as e:
