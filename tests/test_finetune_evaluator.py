...
def _seed_dataset(db_path, ds_id="ds-test", name="Test Dataset", status="ready",
                  purpose="general"):
    conn = sqlite3.connect(str(db_path))
    conn.execute(
...
def _seed_example(db_path, ds_id="ds-test", user_input="Q", expected_output="A",
                  approved=1, created_at=None):
    conn = sqlite3.connect(str(db_path))
...
def _seed_model_version(db_path, mv_id="mv-test", job_id="job-test",
                        model_name="test-model", version=1, ollama_name="test:v1",
                        eval_bleu=0.5, eval_rouge_l=0.6, eval_perplexity=5.0):
    conn = sqlite3.connect(str(db_path))
    conn.execute(
        """INSERT INTO ft_model_versions
           ...
           VALUES (?, ?, ?, ?, 'qwen3:latest', ?, ?, ?, ?)""",
        (mv_id, job_id, model_name, version, ollama_name,
         eval_bleu, eval_rouge_l, eval_perplexity),
    )
    conn.commit()
    conn.close()
...
def _seed_training_job(db_path, job_id="job-test", ds_id="ds-test",
...
def _seed_active_model(db_path, function_name="code_generation", mv_id="mv-test",
                       ollama_name="test:v1"):
    conn = sqlite3.connect(str(db_path))
    conn.execute(
...
class TestComputeBleu:
    ...
    def test_brevity_penalty(self):
        ...
        full = compute_bleu("the cat sat on the big green mat",
                           "the cat sat on the big green mat")
        assert short < full
...
class TestEvaluateModel:
    ...
    def test_evaluate_stores_in_db(self, ft_db, tmp_path):
        ...
        row = conn.execute(
            "SELECT * FROM ft_evaluations WHERE id = ?", (result["eval_id"],)
        ).fetchone()
        conn.close()
...
class TestPromotionEligibility:
    ...
    def test_eligible_model(self, ft_db):
        _seed_model_version(
            ft_db, eval_bleu=0.5, eval_rouge_l=0.6, eval_perplexity=3.0,
            ollama_name="test:v1",
        )
        from tools.finetune.promotion_manager import check_promotion_eligibility
    ...
    def test_ineligible_low_bleu(self, ft_db):
        _seed_model_version(
            ft_db, eval_bleu=0.1, eval_rouge_l=0.6, eval_perplexity=3.0,
            ollama_name="test:v1",
        )
        from tools.finetune.promotion_manager import check_promotion_eligibility
        result = check_promotion_eligibility("mv-test", db_path=ft_db)
    ...
    def test_ineligible_low_rouge(self, ft_db):
        _seed_model_version(
            ft_db, eval_bleu=0.5, eval_rouge_l=0.2, eval_perplexity=3.0,
            ollama_name="test:v1",
        )
        from tools.finetune.promotion_manager import check_promotion_eligibility
        result = check_promotion_eligibility("mv-test", db_path=ft_db)
    ...
    def test_perplexity_improvement_check(self, ft_db):
        ...
        _seed_model_version(
            ft_db, mv_id="mv-current", model_name="current",
            version=1, ollama_name="current:v1",
            eval_bleu=0.3, eval_rouge_l=0.4, eval_perplexity=10.0,
        )
        _seed_active_model(ft_db, function_name="code_generation",
                          mv_id="mv-current", ollama_name="current:v1")

        # Seed candidate with much better (lower) perplexity
        _seed_model_version(
            ft_db, mv_id="mv-new", model_name="new",
            version=1, ollama_name="new:v1",
            eval_bleu=0.5, eval_rouge_l=0.6, eval_perplexity=5.0,
        )

        from tools.finetune.promotion_manager import check_promotion_eligibility
        result = check_promotion_eligibility(
            "mv-new", function_name="code_generation", db_path=ft_db,
        )
        assert result["success"]
        assert result["eligible"]
...
class TestAutoPromote:
    ...
    def test_auto_promote_ineligible(self, ft_db):
        _seed_model_version(
            ft_db, eval_bleu=0.1, eval_rouge_l=0.2, eval_perplexity=3.0,
            ollama_name="test:v1",
        )
...
class TestForcePromote:
    ...
    def test_force_promote_bypasses_thresholds(self, mock_promote, ft_db):
        mock_promote.return_value = {"success": True, "model_version_id": "mv-test"}
        _seed_model_version(
            ft_db, eval_bleu=0.1, eval_rouge_l=0.1, eval_perplexity=99.0,
            ollama_name="test:v1",
        )
        from tools.finetune.promotion_manager import force_promote
        result = force_promote(
            "mv-test", "code_generation",
            reason="Admin override", activated_by="admin",
            db_path=ft_db,
        )
        assert result["success"]
    ...
    def test_force_promote_logs_override(self, mock_promote, ft_db):
        ...
        result = force_promote(
            "mv-test", "code_generation",
            reason="Testing", activated_by="tester",
            db_path=ft_db,
        )
        assert result["success"]
        ...
        conn = sqlite3.connect(str(ft_db))
        log = conn.execute(
            "SELECT * FROM ft_promotion_log WHERE action = 'override_promoted'"
        ).fetchone()
        conn.close()
