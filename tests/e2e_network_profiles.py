...
def run_tests():
    ...
    try:
        ...
        try:
            ...
            results.ok("list_builtin_profiles",
                        f"{len(d)} profiles, vendors: {set(vendors)}")
        ...
        try:
            ...
            results.ok("get_cisco_ios_profile",
                        f"{len(cmds)} commands")
        except Exception as e:
        ...
        try:
            d = api(driver, "POST", "/network/api/device-profiles", {
                "vendor": "Huawei",
                "platform": "VRP",
                "description": "Huawei routers and switches",
                "commands": {
                    "running_config": {"command": "display current-configuration", "parser": "vrp_config", "timeout_sec": 30},
                    "routing_table_v4": {"command": "display ip routing-table", "parser": "vrp_route", "timeout_sec": 15},
                    "interfaces": {"command": "display ip interface brief", "parser": "vrp_interface", "timeout_sec": 10},
                    "version": {"command": "display version", "parser": "vrp_version", "timeout_sec": 10},
                }
            })
            user_prof_id = d.get("id")
            assert user_prof_id
            results.ok("create_user_profile",
                        f"id={user_prof_id}")
        except Exception as e:
            ...
            try:
                api(driver, "PUT",
                    ...
                    {"description": "Huawei VRP (Updated)",
                     "commands_json": {"routing_table_v4": {"command": "display ip routing-table verbose", "parser": "vrp_route_v", "timeout_sec": 20}}})
                d = api(driver, "GET",
                        f"/network/api/device-profiles/{user_prof_id}")
        ...
        try:
            d = api(driver, "DELETE",
                    "/network/api/device-profiles/prof-cisco-ios")
            assert d.get("error")
            results.ok("cannot_delete_builtin",
                        d.get("error", ""))
        except Exception as e:
        ...
        try:
            api(driver, "PUT",
                "/network/api/device-profiles/prof-cisco-ios",
                {"commands": {"custom_check": {"command": "show processes cpu", "parser": "ios_cpu", "timeout_sec": 10}}})
            d = api(driver, "GET",
                    "/network/api/device-profiles/prof-cisco-ios")
        ...
        try:
            d = api(driver, "POST", "/network/api/discovery-configs", {
                "name": "E2E Test Scan",
            ...
            disc_id = d.get("id")
            assert disc_id
            results.ok("create_discovery_config",
                        f"id={disc_id}")
        except Exception as e:
        ...
        try:
            configs = api(driver, "GET",
                          "/network/api/discovery-configs")
            our_config = next(
                (c for c in configs if c.get("id") == disc_id), None)
            assert our_config
            assert our_config.get("read_only") == 1, \
                f"read_only should be 1, got {our_config.get('read_only')}"
        ...
        try:
            ...
            config_id = d.get("id")
            assert config_id
            results.ok("store_collected_config",
                        f"id={config_id}")
        except Exception as e:
        ...
        if config_id:
            try:
                d = api(driver, "GET",
                        f"/network/api/collected-configs/{config_id}")
        ...
        if disc_id:
            try:
                api(driver, "DELETE",
                    f"/network/api/discovery-configs/{disc_id}")
            except Exception:
                pass
        driver.quit()
