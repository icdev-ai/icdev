...
class TestA2ACallback:
    ...
    @patch.dict("os.environ", {"ICDEV_PARENT_CALLBACK_URL": "https://parent:8443/a2a"})
    def test_json_rpc_payload(self, mock_urlopen):
        """call_parent should send valid JSON-RPC 2.0 payload."""
        mock_response = MagicMock()
        mock_response.status = 200
        mock_response.read.return_value = json.dumps({"jsonrpc": "2.0", "id": "test", "result": {"ok": True}}).encode()
