...
def get_sop_by_id(sop_id):
    ...
    with _get_conn() as conn:
        row = conn.execute("SELECT * FROM sdc_sops WHERE id=?", (sop_id,)).fetchone()
...
def delete_sop(sop_id):
    ...
    with _get_conn() as conn:
        cur = conn.execute("DELETE FROM sdc_sops WHERE id=?", (sop_id,))
...
def submit_for_review(sop_id):
    ...
    with _get_conn() as conn:
        conn.execute(
            "UPDATE sdc_sops SET approval_status='pending_review', updated_at=? WHERE id=?",
            (now, sop_id),
        )
