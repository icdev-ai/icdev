...
"""ClawHub DataBridge Connector — live skill discovery from clawhub.ai.

Read-only connector for the ClawHub skill registry API. Enables:
...
Usage (via DataBridge registry):
    ...
    conn = get_connector_instance("clawhub")
    conn.connect({})
    result = conn.read(ConnectorRequest(query="self-improvement"))

Usage (standalone CLI):
    python tools/databridge/connectors/clawhub_connector.py --search "code review" --json
...
# ---------------------------------------------------------------------------
@register_connector
class ClawHubConnector(DataConnector):
    """Read-only connector for the ClawHub skill registry API.
    ...
    def connect(self, config: Dict[str, Any]) -> bool:
        """Initialize connection to ClawHub API.
    ...
    def health_check(self) -> Dict[str, Any]:
        """Check if ClawHub API is reachable."""
        try:
    ...
    def _api_get(self, path: str) -> Optional[Dict]:
        """Make a GET request to the ClawHub API.
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="ClawHub DataBridge Connector CLI")
    ...
    try:
        if args.search:
            ...
            out = args.output or ".tmp/clawhub_downloads"
