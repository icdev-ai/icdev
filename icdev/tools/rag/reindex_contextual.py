...
Design invariants:
  * Resumable / idempotent — chunks that already carry a ``context_prefix`` in
    metadata are skipped unless ``--force``. Re-running is safe.
...
class LiveChunkStore:
    ...
    def iter_chunks(self, source_type: str = "", source_id: str = "") -> List[VectorChunk]:
...
def _reindex_cli(args) -> Dict[str, Any]:
    ...
    if args.execute:
        ...
        chunks = store.iter_chunks(source_type=args.source or "")
    ...
    return reindexer.reindex(chunks, store_update=None, dry_run=True, force=args.force)
        ...
        if "reindex" in result:
            ...
            if "error" in r:
                ...
                print(f"Re-index ({'dry-run' if r['dry_run'] else 'live'}): "
                      f"{r['planned']} planned, {r['reindexed']} written, "
