...
def summary(surface: Optional[str] = None, limit: int = 20,
            since: Optional[str] = None) -> Sequence[Dict[str, Any]]:
    """Per-name rollup: calls, errors, avg/max duration. Read-only.

    Thin wrapper over :meth:`tools.observability.invocation_store.InvocationStore.by_name`
    — the SQL lives there so ``icdev runtime top`` and the dashboard panel read
    the table exactly the way this does, and there is one place to fix when the
    rollup is wrong. Kept as a function because callers already import it.

    Args:
        surface: restrict to one of :data:`SURFACES`.
        limit: rows to return.
        since: ISO-8601 UTC timestamp; only invocations started at or after it.
    """
    from tools.observability.invocation_store import InvocationStore

    return InvocationStore().by_name(surface=surface, since=since, limit=limit)
