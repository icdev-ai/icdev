...
def xc_orders_adapter(query: str, conn=None, **_) -> list[dict]:
    _conn, owned = _ccc_conn(conn)
    try:
        q = query.lower()
        if "in-flight" in q or "in flight" in q or "pipeline" in q or "active order" in q:
            sql = (
                "SELECT id,xc_number,facility,order_status,estimated_delivery,"
                "carrier_ticket_id,ordered_by FROM ccc_cross_connects "
                "WHERE order_status IN ('quoted','ordered','loa_submitted','loa_approved','installing','testing') "
                "ORDER BY estimated_delivery ASC"
            )
        elif "partner" in q:
            sql = (
                "SELECT id,xc_number,facility,order_status,partner_id,estimated_delivery "
                "FROM ccc_cross_connects WHERE order_status != 'not_ordered' "
                "ORDER BY partner_id, estimated_delivery ASC"
            )
        elif "delay" in q or "overdue" in q:
            sql = (
                "SELECT id,xc_number,facility,order_status,estimated_delivery,carrier_ticket_id "
                "FROM ccc_cross_connects "
                "WHERE order_status NOT IN ('active','cancelled','not_ordered') "
                "AND estimated_delivery != '' AND estimated_delivery < date('now') "
                "ORDER BY estimated_delivery ASC"
            )
        elif "cancel" in q:
            sql = (
                "SELECT id,xc_number,facility,order_status,carrier_ticket_id "
                "FROM ccc_cross_connects WHERE order_status='cancelled' ORDER BY xc_number"
            )
        else:
            sql = (
                "SELECT id,xc_number,facility,order_status,estimated_delivery,"
                "carrier_ticket_id,monthly_cost_usd FROM ccc_cross_connects "
                "WHERE order_status != 'not_ordered' ORDER BY order_status, xc_number LIMIT 50"
            )
        return _safe_fetch(_conn, sql)
    finally:
        if owned:
...
_COLLECTION_MAP = {
    ...
    "ccc.xc_orders":      xc_orders_adapter,
}
...
def handle_query(conn, query: str, collection: str = "") -> dict:
    ...
    q = query.lower()
    if not collection:
        if any(k in q for k in ("order", "in-flight", "pipeline", "overdue", "ordered", "quoted")):
            collection = "ccc.xc_orders"
        elif any(k in q for k in ("cross-connect", "cross connect", "xc ", "patch")):
