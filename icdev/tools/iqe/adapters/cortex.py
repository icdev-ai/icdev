...
**A capped scan is not a certainty** (ctx-trust-04). The row cap below is
applied by the SQL, but the executor applies the query's ``where`` clauses in
PYTHON afterwards, so a question whose window matches more rows than the cap is
answered from the newest ``_ROW_CAP`` rows only. ``cortex.ask("how many blocked
calls in the last 30 days?")`` therefore counted within the cap and
``analyst._label_rows_result`` stamped the undercount ``confidence_score: 1.0`` —
a confident falsehood on the exact surface an operator uses to audit Cortex.

The fix here is NOT predicate pushdown. Pushing the ``where`` clauses into the
adapter SQL would fix the count for these three collections, but it changes the
adapter contract for every canvas (~40 of them) and re-implements the executor's
predicate evaluation in two dialects — and it would still leave the cap silently
biting on any predicate that did not push down. Instead the cap is *reported*:
``_fetch`` asks for one row more than the cap, and when that extra row comes
back it returns a ``RowSet`` marked ``reason="row_cap"``. That report survives
``union``/``join`` and reaches ``analyst``, which labels the answer
``confidence: "flag"`` and names the truncation in the text. The wrong number
is still wrong — but it can no longer be presented as certain, and the default
cap of 10000 keeps the honest path the common one.
"""
...
#: Raised from 500 (ctx-trust-04). Overridable per query via the IQE call form,
#: e.g. ``cortex.audit(20000)``.
_ROW_CAP = 10000
