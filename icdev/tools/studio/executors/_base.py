...
Canvas is resolved in priority order:
  1. --canvas CLI arg
  2. The `canvas:` key declared by the run's workflow template (authoritative)
  3. Inferred from Generate IaC artifact paths in the run
  4. Workflow name prefix  (e.g. 'DDC Test Workflow' → 'ddc')
  5. CanvasResolutionError
...
def resolve_canvas(run_id: str = "", hint: str = "") -> str:
    ...
    # 1. The template's own `canvas:` declaration — authoritative.
    declared = _canvas_from_template(run_id)
    if declared:
        return declared

    # 2. Infer from Generate IaC artifact paths.
    ...
    # 3. Infer from the workflow name.
