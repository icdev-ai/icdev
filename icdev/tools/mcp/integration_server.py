...
def handle_configure_jira(args: dict) -> dict:
    ...
    configure = _import_tool("icdev.tools.integration.jira_connector", "configure")
    if not configure:
...
def handle_sync_jira(args: dict) -> dict:
    ...
    if direction == "push":
        fn = _import_tool("icdev.tools.integration.jira_connector", "push_to_jira")
    elif direction == "pull":
        fn = _import_tool("icdev.tools.integration.jira_connector", "pull_from_jira")
    else:
...
def handle_configure_servicenow(args: dict) -> dict:
    ...
    configure = _import_tool("icdev.tools.integration.servicenow_connector", "configure")
    if not configure:
...
def handle_sync_servicenow(args: dict) -> dict:
    ...
    direction = args.get("direction", "push")
    if direction == "push":
        fn = _import_tool("icdev.tools.integration.servicenow_connector", "push_to_servicenow")
    elif direction == "pull":
        fn = _import_tool("icdev.tools.integration.servicenow_connector", "pull_from_servicenow")
    else:
        return {"error": f"Invalid direction: {direction}. Use 'push' or 'pull'"}
...
def handle_configure_gitlab(args: dict) -> dict:
    ...
    configure = _import_tool("icdev.tools.integration.gitlab_connector", "configure")
    if not configure:
...
def handle_sync_gitlab(args: dict) -> dict:
    ...
    direction = args.get("direction", "push")
    if direction == "push":
        fn = _import_tool("icdev.tools.integration.gitlab_connector", "push_to_gitlab")
    elif direction == "pull":
        fn = _import_tool("icdev.tools.integration.gitlab_connector", "pull_from_gitlab")
    else:
        return {"error": f"Invalid direction: {direction}. Use 'push' or 'pull'"}
...
def handle_export_reqif(args: dict) -> dict:
    ...
    export_reqif = _import_tool("icdev.tools.integration.doors_exporter", "export_reqif")
    if not export_reqif:
...
def handle_submit_approval(args: dict) -> dict:
    ...
    submit_for_approval = _import_tool("icdev.tools.integration.approval_manager", "submit_for_approval")
...
def handle_review_approval(args: dict) -> dict:
    ...
    review_approval = _import_tool("icdev.tools.integration.approval_manager", "review_approval")
...
def handle_build_traceability(args: dict) -> dict:
    ...
    build_rtm = _import_tool("icdev.tools.requirements.traceability_builder", "build_rtm")
    if not build_rtm:
