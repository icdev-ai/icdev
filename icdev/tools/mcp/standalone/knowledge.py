#!/usr/bin/env python3
# CUI // SP-CTI
"""ICDEV Standalone MCP Server -- Knowledge.

Wrapper script that resolves the ICDEV installation directory, sets up
sys.path and environment, then starts the Knowledge MCP server.
...
logger = logging.getLogger("icdev.mcp.standalone.knowledge")
...
def _resolve_base_dir():
    ...
    if env_dir and Path(env_dir).is_dir():
        return Path(env_dir)
    # Infer from package location (tools/mcp/standalone/knowledge.py -> 4 levels up)
...
def main():
    ...
    try:
        from icdev.tools.mcp.knowledge_server import create_server
        server = create_server()
        logger.info("Starting ICDEV Knowledge MCP server (base_dir=%s)", base_dir)
        server.run()
        ...
        try:
            ...
            logger.error("Failed to start Knowledge MCP server: %s", exc)
            sys.exit(1)
    except Exception as exc:
        logger.error("Failed to start Knowledge MCP server: %s", exc)
        sys.exit(1)
