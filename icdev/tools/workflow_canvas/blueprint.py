...
{text[:8000]}"""
    ...
    def api_step_coach(workflow_id: str, step_index: int):
        ...
        try:
            from tools.workflow_canvas.process_coach import coach_from_db
            result = coach_from_db(workflow_id, step_index)
            return jsonify(result)
        except Exception as exc:
            return jsonify({"error": str(exc)}), 500
    ...
    def my_tasks_page():
        ...
        # Collect unique roles from task descriptions for the filter dropdown
        all_tasks = conn.execute(
            """SELECT description FROM kanban_tasks
               WHERE id LIKE 'processify-%' AND status != 'done'"""
        ).fetchall()
        conn.close()
        roles: list[str] = []
        ...
        import re as _re
        for t in all_tasks:
            desc = t["description"] or "" if hasattr(t, "keys") else (t[0] or "")
            for match in _re.findall(r'"assignee_role":\s*"([^"]+)"', desc):
                if match and match not in seen_roles:
                    seen_roles.add(match)
                    roles.append(match)
        roles.sort()
        return render_template(
            "workflow_canvas/my_tasks.html",
