...
class LLMRouter:
    ...
    def _get_provider(self, provider_name: str) -> Optional[LLMProvider]:
        ...
        try:
            if ptype == "bedrock":
                from icdev.tools.llm.bedrock_provider import BedrockLLMProvider
                region = _expand_env(provider_cfg.get("region", "us-gov-west-1"))
                instance = BedrockLLMProvider(region=region)

            elif ptype == "anthropic":
                from icdev.tools.llm.anthropic_provider import AnthropicLLMProvider
                ...
                from icdev.tools.llm.ollama_provider import OllamaProvider
                ...
                from icdev.tools.llm.gemini_provider import GeminiProvider
                ...
                from icdev.tools.llm.openai_provider import OpenAICompatibleProvider
                api_key = provider_cfg.get("api_key", "")
                if not api_key:
                ...
                from icdev.tools.llm.azure_openai_provider import AzureOpenAIProvider
                ...
                from icdev.tools.llm.vertex_ai_provider import VertexAIProvider
                ...
                from icdev.tools.llm.oci_genai_provider import OCIGenAIProvider
        ...
        for model_name in chain:
            ...
            try:
                ...
                if ptype in ("openai", "openai_compatible"):
                    from icdev.tools.llm.embedding_provider import OpenAIEmbeddingProvider
                    ...
                    from icdev.tools.llm.embedding_provider import BedrockEmbeddingProvider
                    pcfg = self._config.get("providers", {}).get(provider_name, {})
                    ...
                    from icdev.tools.llm.embedding_provider import GeminiEmbeddingProvider
                    pcfg = self._config.get("providers", {}).get(provider_name, {})
                    api_key = pcfg.get("api_key", "")
                    if not api_key:
                    ...
                    from icdev.tools.llm.embedding_provider import AzureEmbeddingProvider
                    pcfg = self._config.get("providers", {}).get(provider_name, {})
                    ...
                    from icdev.tools.llm.embedding_provider import OCIEmbeddingProvider
                    pcfg = self._config.get("providers", {}).get(provider_name, {})
                    ...
                    from icdev.tools.llm.embedding_provider import IBMWatsonxEmbeddingProvider
                    pcfg = self._config.get("providers", {}).get(provider_name, {})
                    api_key = _expand_env(pcfg.get("api_key", ""))
                    if not api_key:
