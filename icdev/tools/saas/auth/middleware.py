...
def register_auth_middleware(app):
    ...
    def authenticate_request():
        ...
        # G-15: Device trust check (ZTA Pillar 2 — Device)
        if os.environ.get("ICDEV_DEVICE_TRUST_REQUIRED", "false").lower() == "true":
            try:
                ...
                trust_result = verify_device_posture(device_fp)
                if not trust_result.trusted:
                    logger.warning(
                        "Device trust check failed: user=%s reason=%s",
                        auth_info["user_id"], trust_result.reason,
                    )
                    return jsonify(
                        {"error": "Device posture check failed", "code": "DEVICE_UNTRUSTED",
                         "reason": trust_result.reason}
                    ), 403
                g.device_trust = trust_result._asdict() if hasattr(trust_result, "_asdict") else vars(trust_result)
            except Exception as exc:
                logger.debug("Device trust check error: %s", exc)
