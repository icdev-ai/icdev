...
with architectural decisions D7/D25 — zero deps, air-gap safe).
...
def _build_reqif_document(requirements, session_id, include_trace=True, trace_links=None):
    ...
    ET.SubElement(req_if_header, "COMMENT").text = f"Generated by ICDEV™ RICOAS for session {session_id}"
    ...
    ET.SubElement(req_if_header, "TITLE").text = f"Requirements Export - Session {session_id}"
    ...
    for idx, val in enumerate(["draft", "clarified", "validated", "approved", "rejected", "decomposed", "deferred"]):
        ev = ET.SubElement(
            sv_status,
            "ENUM-VALUE",
            {
...
def export_reqif(session_id, output_path, include_trace=True, db_path=None):
    ...
    try:
        ...
        if not rows:
            return {"error": f"No requirements found for session {session_id}", "requirements_exported": 0}
        ...
        if include_trace:
            ...
            if req_ids:
                ...
                trace_links = [
                    (dict(row)["source_id"], dict(row)["target_id"], dict(row)["link_type"]) for row in links
                ]
...
def main():
    parser = argparse.ArgumentParser(description="ReqIF 1.2 exporter for DOORS NG integration")
    parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Actions
    parser.add_argument("--export-reqif", action="store_true", help="Export requirements as ReqIF")
    parser.add_argument("--validate", action="store_true", help="Validate a ReqIF file")

    # Export args
    parser.add_argument("--session-id", help="Intake session ID")
    parser.add_argument("--output-path", help="Output file path for ReqIF")
    parser.add_argument("--no-trace", action="store_true", help="Exclude traceability relations")
