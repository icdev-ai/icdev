...
logger = get_logger("icdev.innovation.solution_generator")
...
FORGE_LAYER_INFO = {
    "goal": {
        ...
        "proposed_template": (
            "Create a new goal workflow in `goals/` that defines:\n"
    ...
    "tool": {
        ...
        "proposed_template": (
            "Create a new deterministic Python script in `tools/` that:\n"
        ...
        "effort": "L",
        "test_unit": (
            "pytest unit tests: happy path, error handling, DB ops, JSON validation, "
    ...
    "arg": {
        ...
        "proposed_template": (
            "Create a new YAML configuration file in `args/` that:\n"
    ...
    "context": {
        ...
        "proposed_template": (
            "Create a new context reference file in `context/` that:\n"
    ...
    "hardprompt": {
        ...
        "proposed_template": (
            "Create a new hardprompt template in `hardprompts/` that:\n"
...
CATEGORY_LAYER_MAP = {
    "security_vulnerability": "tool",
    "compliance_gap": "goal",
    "developer_experience": "tool",
    "performance": "tool",
    "modernization": "goal",
    "supply_chain": "tool",
    "infrastructure": "arg",
    "testing": "tool",
    "ai_tooling": "hardprompt",
}
...
def _build_acceptance_criteria(gotcha_layer, signal):
    ...
    if category == "security_vulnerability":
        ...
        criteria.append(
            "- [ ] Given crosswalk engine queried, When controls implemented, Then FedRAMP/CMMC auto-populated"
        )
...
def _build_implementation_blueprint(gotcha_layer, signal, metadata):
    ...
    if gotcha_layer == "tool":
        ...
        config_section = f"- `args/{tool_name}_config.yaml` — Primary deliverable; document all ICDEV_ overrides"
        ...
        config_section = f"- Add provenance metadata header to `context/{tool_name}.json`"
    else:  # hardprompt
        new_files_section = (
        ...
        config_section = "- `args/llm_config.yaml` — Add routing entry if new LLM function introduced"
    ...
    if url and url not in ("N/A", "", "None"):
        source_section = (
            ...
            "4. Map external concepts to FORGE layer artifacts"
        )
...
# =========================================================================
# OPT-IN DIVERGENCE STEP (dvg-wire-02)
# =========================================================================
# _build_implementation_blueprint is heuristic dispatch producing exactly ONE
# blueprint per signal, so a bad gotcha-layer resolution silently determines the
# whole solution with no alternatives considered. This OFF-BY-DEFAULT step runs
# BEFORE blueprint construction: it generates candidate approaches under multiple
# frames (isolated invoke_divergence), runs the separate critic pass, and either
# picks the highest-scoring non-trap approach or surfaces the top clusters for a
# human. The valuable output is the REJECTED alternatives, recorded for review.
DIVERGENCE_FUNCTION = "innovation_ideation"


def _build_signal_problem(signal):
    """Compose the generative problem statement fed to the divergence branches."""
    title = (signal.get("title") or "Unnamed signal").strip()
    desc = (signal.get("description") or "").strip()
    category = (signal.get("category") or "uncategorized").strip()
    parts = [f"Capability signal: {title}", f"Category: {category}"]
    if desc and desc.lower() != title.lower():
        parts.append(f"Detail: {desc}")
    parts.append(
        "Propose distinct candidate APPROACHES to build this capability into the "
        "platform. Approaches only -- different implementation strategies, not one "
        "refined answer."
    )
    return "\n".join(parts)


def _render_divergence_section(trace_id, chosen, rejected):
    """Render chosen + rejected approaches as a Markdown section for the spec.

    Trap-flagged approaches are visibly marked wherever the solution is reviewed
    -- the seductive-but-broken directions are shown, not hidden behind the winner.
    """
    lines = [
        "## Candidate Approaches (Divergence)",
        "",
        f"_Generated by isolated divergent ideation (trace `{trace_id}`). The "
        "blueprint above reflects the CHOSEN approach; rejected alternatives are "
        "retained below for review. Trap-flagged approaches are marked._",
        "",
    ]

    def _fmt(c, tag):
        trap = "  **[TRAP-FLAGGED]**" if c.get("has_trap") else ""
        header = f"### {tag}: {c.get('label') or 'unlabeled'} (composite {c.get('best_composite', 0.0):.3f}){trap}"
        block = [header]
        if c.get("sketch"):
            block.append(c["sketch"])
        members = c.get("members") or []
        if members:
            block.append("")
            block.append("Ideas: " + "; ".join(str(m.get("idea", ""))[:200] for m in members[:3]))
        if c.get("risks"):
            block.append("Risks: " + "; ".join(str(r) for r in c["risks"][:5]))
        return "\n".join(block)

    if chosen:
        lines.append(_fmt(chosen, "CHOSEN"))
        lines.append("")
    else:
        lines.append("_No non-trap approach was auto-selected; all surviving clusters are surfaced for human choice._")
        lines.append("")
    for c in rejected:
        lines.append(_fmt(c, "REJECTED"))
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"


def _run_divergence_step(signal, div_cfg, *, orchestrator=None, router=None, persist=True):
    """Generate -> critique -> cluster candidate approaches for one signal.

    Returns ``{ran, trace_id, reason, clusters, chosen, rejected,
    section_markdown}``. ``chosen`` is the highest-scoring NON-TRAP cluster
    (clusters are composite-descending); if every surviving cluster is a trap,
    ``chosen`` is None (surface-for-human mode) and all clusters are rejected.
    Never raises: a disabled/excluded chain, unavailable LLM, empty pool, or
    unparseable critic response yields ``ran=False`` with a reason, and the
    caller proceeds with the unchanged single-blueprint path.
    """
    function = str(div_cfg.get("function") or DIVERGENCE_FUNCTION)
    out = {
        "ran": False, "trace_id": None, "reason": "", "clusters": [],
        "chosen": None, "rejected": [], "section_markdown": "",
    }
    try:
        ...
        request = LLMRequest(messages=[{"role": "user", "content": _build_signal_problem(signal)}])
        chain = orchestrator.invoke_divergence(function, request)
    except RuntimeError as exc:
        out["reason"] = f"divergence_unavailable: {exc}"
        return out
    except Exception as exc:  # noqa: BLE001 -- degrade like any unavailable-LLM path
        logger.warning("Divergence step generate failed for '%s': %s", function, exc)
        out["reason"] = f"generate_failed: {exc}"
        return out

    if not getattr(chain, "content", ""):
        out["reason"] = f"empty_pool: {getattr(chain, 'stop_reason', 'unknown')}"
        return out
    ...
    try:
        ...
        scored = score_idea_pool(chain.content, function=function, trace_id=trace_id, router=router, persist=persist)
        if not scored.ordered:
            out["reason"] = f"critic_no_scores: {scored.stop_reason}"
            return out
        deepened = cluster_and_deepen(scored, function=function, router=router)
    except Exception as exc:  # noqa: BLE001
        logger.warning("Divergence step critic/cluster failed: %s", exc)
        out["reason"] = f"critic_failed: {exc}"
        return out

    clusters = deepened.get("clusters", [])
    if not clusters:
        out["reason"] = "no_clusters"
        return out

    # Pick highest-scoring non-trap approach; clusters are composite-descending.
    chosen = next((c for c in clusters if not c.get("has_trap")), None)
    rejected = [c for c in clusters if c is not chosen]
    out.update(
        ran=True,
        reason="completed",
        clusters=clusters,
        chosen=chosen,
        rejected=rejected,
        section_markdown=_render_divergence_section(trace_id or "", chosen, rejected),
    )
    return out
...
def generate_solution_spec(signal_id, db_path=None):
    ...
    try:
        ...
        if signal.get("status") != "approved":
            return {
                "error": f"Signal {signal_id} not approved (status: {signal.get('status')})",
        ...
        if existing:
            return {
                "error": f"Solution already exists for signal {signal_id}",
                "solution_id": existing["id"],
                "solution_status": existing["status"],
            }
        ...
        try:
            ...
            pass
        compliance_notes = metadata.get("compliance_notes") or BOUNDARY_NOTES.get(
            boundary_tier, BOUNDARY_NOTES["GREEN"]
        )
        innovation_score = (
            signal.get("innovation_score")
            if signal.get("innovation_score") is not None
            else (signal.get("community_score") or 0.0)
        )
        asset_type = layer_info.get("asset_type", "tool")

        # Opt-in divergence step (dvg-wire-02) BEFORE blueprint construction:
        # consider candidate approaches instead of committing to one heuristic
        # blueprint. OFF by default -- when disabled this is a no-op and the
        # function behaves exactly as before. Degrades cleanly on LLM failure.
        divergence_result = None
        div_cfg = _load_config().get("divergence", {})
        if div_cfg.get("enabled", False):
            divergence_result = _run_divergence_step(signal, div_cfg)
        ...
        spec_content = _SPEC_TEMPLATE.format(
            ...
            effort_days=effort_details["days"],
            effort_points=effort_details["story_points"],
            implementation_blueprint=implementation_blueprint,
        )

        # Carry surviving candidate approaches (chosen + rejected) into the spec.
        if divergence_result and divergence_result.get("ran") and divergence_result.get("section_markdown"):
            spec_content = spec_content + "\n\n" + divergence_result["section_markdown"]
        ...
        # Record the divergence decision: the rejected alternatives are the
        # valuable part for later review (dvg-wire-02).
        if divergence_result and divergence_result.get("ran"):
            chosen = divergence_result.get("chosen") or {}
            audit_details["divergence"] = {
                "trace_id": divergence_result.get("trace_id"),
                "chosen": {
                    "label": chosen.get("label"),
                    "best_composite": chosen.get("best_composite"),
                } if chosen else None,
                "rejected": [
                    {
                        "label": c.get("label"),
                        "best_composite": c.get("best_composite"),
                        "has_trap": c.get("has_trap", False),
                    }
                    for c in divergence_result.get("rejected", [])
                ],
            }
        elif divergence_result:
            audit_details["divergence"] = {"ran": False, "reason": divergence_result.get("reason")}

        _audit(
            "innovation.solution_generated",
            "innovation-agent",
        ...
        return {
            ...
            "divergence": audit_details.get("divergence"),
        }
    finally:
        conn.close()
...
def generate_all_approved(db_path=None):
    ...
    if not signals:
        return {
            ...
            "errors": 0,
            "skipped": 0,
            "message": "No approved signals pending solution generation.",
            "results": [],
        }
...
def list_solutions(status=None, limit=20, db_path=None):
    ...
    try:
        query = (
            "SELECT sol.id, sol.signal_id, sol.gotcha_layer, sol.asset_type, "
        ...
        solutions = [
            {
                ...
                "signal_title": r["signal_title"] or "",
                "signal_source": r["signal_source"] or "",
                "signal_category": r["signal_category"] or "",
                "gotcha_layer": r["gotcha_layer"],
                "asset_type": r["asset_type"],
                "estimated_effort": r["estimated_effort"],
                "status": r["status"],
                "spec_quality_score": r["spec_quality_score"],
        ...
        counts = {
            r["status"]: r["cnt"]
            for r in conn.execute("SELECT status, COUNT(*) as cnt FROM innovation_solutions GROUP BY status").fetchall()
        }
        return {
            "solutions": solutions,
...
def _print_human(args, result):
    ...
    if args.generate:
        ...
        print(
            f"Solution generated: {result.get('solution_id')}\n"
        ...
        print(
            f"Batch: {result.get('generated', 0)} generated, {result.get('errors', 0)} errors, "
        ...
        q = result.get("spec_quality_score")
        print(
            f"Solution: {result.get('solution_id')}\n"
        ...
        print(
            f"Solutions ({result.get('returned', 0)} of {result.get('total', 0)})"
            f"  Status: {', '.join(f'{k}={v}' for k, v in counts.items())}"
        )
        for s in result.get("solutions", []):
            q = s.get("spec_quality_score")
            print(
                f"  {s['solution_id']} [{s['status']:10s}] {s['gotcha_layer']:10s} "
...
def main():
    ...
    parser.add_argument(
        "--status-filter", type=str, dest="filter_status", choices=VALID_STATUSES, help="Filter by status (with --list)"
    )
