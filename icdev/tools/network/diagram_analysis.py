...
def run_analysis(upload_id: str, industry: str, conn) -> dict:
    ...
    """
    # Use get_canvas_connection (no RLS) because nc_diagram_uploads has no
    # classification/tenant_id columns — get_connection() injects an RLS WHERE
    # predicate that raises UndefinedColumn inside Flask request context.
    from tools.db.storage import get_canvas_connection as _gcc
    with _gcc() as _rconn:
...
def generate_ndc_topology(tabs: dict, cloud_context: dict) -> dict:
    """Build an NDC-native graph_json from analysis tabs.
    ...
    for finding in security:
        ...
        if comp and comp != "all":
            sev = finding.get("severity", "info")
            if _sev_rank(sev) > _sev_rank(sev_map.get(comp, "info")):
                sev_map[comp] = sev

    def _get_sev(label: str) -> str:
        lbl = label.lower()
        for comp, sev in sev_map.items():
            if comp[:14] in lbl or lbl[:14] in comp:
                return sev
        return "info"
