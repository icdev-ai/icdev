...
- ``external`` — authorized EXTERNAL sources, reached through the DataBridge
  agent broker (``tools/databridge/broker.py``). The only backend whose evidence
  comes from outside the boundary; it inherits the broker's authorization model
  whole (air-gap interlock, connector+table allowlist, per-agent grants,
  classification ceiling, read-only, fail-closed outbound redaction, egress
  guard, row caps, one audit row per outcome) and adds a service-key scope check
  in front of it. Pinned fail-closed regardless of ``governance.fail_closed``.
...
# external is deliberately ABSENT: an ambiguous query must not egress. Fan-out
# is what an unclassifiable query falls back to, so putting the external rung in
# it would make "Cortex could not tell what you meant" the trigger for an
# outbound call to a third party. It is reached by an explicit strategy, or by a
# deployment that adds it to search.fan_out.backends having decided that.
