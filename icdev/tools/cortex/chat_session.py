...
deliberately *thin* session model over three canvas-owned tables:
    ...
    cortex_sessions   — one row per session (mode/domain/title/status)
    ...
    cortex_audit      — append-only facade outcome trail
...
def ensure_session(
    ...
    *,
    ...
    try:
        ...
        try:
            row = conn.execute(
                "SELECT session_id FROM cortex_sessions WHERE session_id = %s",
                (session_id,),
            ).fetchone()
            if row:
                conn.execute(
                    "UPDATE cortex_sessions SET mode = %s, domain = %s, updated_at = %s "
                    "WHERE session_id = %s",
    ...
    try:
        ...
        try:
            row = conn.execute(
                ...
                "created_at, updated_at FROM cortex_sessions WHERE session_id = %s",
                (session_id,),
            ).fetchone()
        finally:
            conn.close()
...
def record_audit(
    ...
    *,
    ...
    """Append a facade-outcome row to the append-only cortex_audit trail."""
    try:
        conn = _conn()
        try:
            conn.execute(
                ...
                (uuid.uuid4().hex, session_id, facade, outcome,
                 _to_bool_int(blocked), detail[:500], classification, tenant_id, _now()),
            )
            conn.commit()
        finally:
            conn.close()
    except Exception as exc:  # noqa: BLE001
        logger.debug("cortex.session: record_audit skipped: %s", exc)
