...
from icdev.tools.testing.data_types import (
    TestResult,
    E2ETestResult,
    GateResult,
    GateEvaluation,
...
from icdev.tools.testing.utils import (
    make_run_id,
    setup_logger,
...
def _detect_e2e_mode() -> str:
    ...
    """
    from icdev.tools.testing.e2e_runner import check_playwright_installed, discover_native_tests
...
def discover_e2e_tests() -> List[str]:
    ...
    from icdev.tools.testing.e2e_runner import discover_e2e_tests as _discover
...
def run_e2e_tests(
    ...
    if mode == "native":
        from icdev.tools.testing.e2e_runner import run_playwright_native
        results = run_playwright_native(run_id, logger, project="chromium")
        return results
    else:
        # MCP / validation fallback
        from icdev.tools.testing.e2e_runner import discover_mcp_tests, execute_e2e_test
...
def evaluate_security_gate(project_dir: str, logger) -> GateEvaluation:
    ...
    # SAST check
    try:
        from icdev.tools.security.sast_runner import run_sast
    ...
    try:
        from icdev.tools.security.secret_detector import scan_directory
...
def evaluate_compliance_gate(project_id: str, project_dir: str, logger) -> GateEvaluation:
    ...
    # CUI marking check
    try:
        from icdev.tools.compliance.cui_marker import verify_directory
...
def main():
    ...
    # Step 1: Health check
    logger.info("\n=== Step 1: Health Check ===")
    from icdev.tools.testing.health_check import run_health_check
    ...
    if not args.skip_acceptance:
        logger.info("\n=== Step 7: Acceptance Criteria Validation (V&V) ===")
        try:
            from icdev.tools.testing.acceptance_validator import validate_acceptance
