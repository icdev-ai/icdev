...
def session_results(session_id: int):
    ...
    for team in teams:
        ...
        receipt_total = conn.execute(
            "SELECT COALESCE(SUM(receipt_count), 0) FROM ttx_scores WHERE team_id = ?", (tid,)
        ).fetchone()[0] or 0
        tools_used = conn.execute(
            "SELECT DISTINCT tool_slug FROM ttx_api_log WHERE team_id = ?", (tid,)
        ).fetchall()
        ai_stats[tid] = {
...
def api_simulate_state(session_id: int):
    ...
    for t in teams_raw:
        tid = t["team_id"]
        mc = conn.execute(
            "SELECT COUNT(*) FROM ttx_team_members WHERE team_id = ?", (tid,)
        ).fetchone()[0] or 0
        rc = conn.execute(
            "SELECT COUNT(*) FROM ttx_responses WHERE team_id = ?", (tid,)
        ).fetchone()[0] or 0
        rt = conn.execute(
            "SELECT COALESCE(SUM(receipt_count),0) FROM ttx_scores WHERE team_id = ?", (tid,)
        ).fetchone()[0] or 0
