...
- **A module that standalone apps VENDOR must stay a subset of its copies — declare the copies in `args/vendor_parity.yaml`.** `tools/cortex/client.py` is deliberately stdlib-only so standalone apps in SEPARATE git repos can copy it verbatim into their own `tools/integrations/cortex_client.py` (ctx-expose-06). Nothing tied the copies to canonical, and the drift is LATENT — the copies were missing methods no consumer called *yet*, so nothing broke and nobody noticed (measured 2026-08-02: canonical 24 public methods, compass 22, idea_lab 19). Adding a public method here means re-vendoring the file into each consumer repo. Enforced by `coherence_checker.py:check_vendor_parity`, which compares the PUBLIC API (classes/functions/method parameter names) rather than bytes — a vendored copy legitimately differs by its provenance header and by line endings (canonical is LF, the consumers are CRLF under `core.autocrlf=true`). A copy that is temporarily AHEAD of canonical is fine; a consumer repo not checked out on this machine is SKIPPED, never failed, so CI stays green. Adding a vendor target needs no code change — add an entry to the YAML.
