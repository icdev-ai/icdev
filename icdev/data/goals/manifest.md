...
| Marketplace | goals/marketplace.md | Federated FORGE asset marketplace: publish, install, search, review, sync skills/goals/hardprompts/context/args/compliance across tenant orgs with 7-gate security pipeline (Phase 22) **(parent-only)** |
...
| Remote Command Gateway | goals/remote_command_gateway.md | Remote Command Gateway: messaging channel integration, 8-gate security chain, IL-aware response filtering, user binding, air-gapped/connected mode (Phase 28) **(parent-only)** |
| Cross-Language Translation | goals/cross_language_translation.md | LLM-assisted cross-language code translation: 5-phase hybrid pipeline (Extract→Type-Check→Translate→Assemble→Validate+Repair), 30 directional pairs, pass@k candidates, mock-and-continue, compliance bridge, Dashboard+Portal visibility (Phase 43, D242-D256) **(parent-only)** |
...
| Creative Engine | goals/creative_engine.md | Customer-centric feature opportunity discovery: auto-discover competitors, scan review sites/forums/GitHub, extract pain points, 3-dimension scoring, trend detection, template-based spec generation, Innovation Engine bridge (Phase 58, D351-D360) **(parent-only)** |
...
| OSINT Streaming Ingestion | goals/osint_streaming_ingestion.md | Real-time data ingestion from social media (X API v2), satellite imagery (Copernicus), and open-source news (NewsAPI/GDELT) into a temporary staging directory. StreamBufferService coordinates fetchers, deduplicates via SHA-256, and persists cursor state. Integrated into IngestionPipelineService as osint_stream, satellite_stream, and news_stream source types. |
| Procurement Intel Agent | goals/procurement_intel.md | SAM.gov capture pipeline: scan solicitations, extract shall-statement requirements, map to ICDEV capability catalog (L/M/N grading), identify gaps, generate response outline — composes tools/govcon/ with no new Python (pattern='procurement-intel') |
| GovChain Blockchain Workflow | goals/blockchain_workflow.md | Hyperledger Fabric anchor lifecycle: Merkle batching, Fabric submission, air-gap queue, pending-ops flush, asset tokenization, verification reports (D-GC-1 through D-GC-11) |
| Autonomous Capability Foundry (ACF) | goals/autonomous_capability_foundry.md | 0→1 product factory at /foundry (prefix acf-): harvest→synthesize→novelty-gate→score→CoD go/no-go→spec→task-graph→seed→learn loop bridging Innovation/Creative/Research/Genesis/telemetry into vetted autonomous-build kanban epics; novelty-gated, SIPA self-vetted, rate-limited, dual circuit breaker (foundry_cycle reflex, 12h, flag ICDEV_FOUNDRY_ENABLED) |
| ACE Co-Worker | goals/ace_coworker.md | Dynamic agentic co-worker teams: delegation, creator-verifier, negotiation, broadcast — triggered by chat/kanban/API; Oracle problem classification → role assembly → ThreadPoolExecutor step loop → HITL gates → /coworker/ canvas + chat result surfacing |
