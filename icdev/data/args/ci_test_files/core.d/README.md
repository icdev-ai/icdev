# `core.d` — per-PR fragments for `core.txt`

Add a test to CI by dropping **one new file here**, named for your task:

```
args/ci_test_files/core.d/<task-id>.txt
```

One target per line; `#` comments and blank lines are ignored, exactly as in
`core.txt`. Files are read after `core.txt` in filename order, so the run
order is deterministic on every machine.

## Why a directory

`core.txt` was the largest merge-collision surface in the repository. 82.8% of
...
`core.txt` is unchanged and still authoritative for everything already in it —
