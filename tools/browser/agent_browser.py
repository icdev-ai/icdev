...
Scope: ``navigate()`` refuses any scheme outside
``agent_browser.allowed_schemes`` and any http(s) host outside
``agent_browser.allowed_domains`` (localhost/127.0.0.1 by default), then hands
the URL to ``tools.browser.scope.check_navigation`` — the canonical browser
scope control from oss-browse-02, which owns the denylist, the non-local
switches, and ``egress_guard``. Both must allow, so this module can narrow the
canonical policy but never widen it. Credential brokering, action budgets, and
the per-action audit trail live in ``scope.GuardedDriver``.
...
class AgentBrowser:
    ...
    def _check_scope(self, url: str) -> None:
        ...
        Two allowlists, both of which must permit the target, so this surface
        can only ever be *narrower* than the canonical policy:

        1. ``agent_browser.allowed_schemes`` / ``allowed_domains`` from
           ``args/agent_browser.yaml`` — this module's own, and the only check
           that applies to hostless schemes (``file``/``about``/``data``).
        2. ``scope.check_navigation`` from oss-browse-02 — the canonical
           browser scope control (denylist, locality, ``egress_guard``). Every
           http(s) target goes through it; a missing scope module fails closed.
        """
        parsed = urlparse(url)
        ...
        try:
            from tools.browser.scope import check_navigation
        except Exception as exc:  # fail closed — no canonical policy, no navigation
            raise BrowserScopeError(
                f"tools.browser.scope is unavailable, refusing to navigate: {exc}"
            ) from exc
        decision = check_navigation(url)
        if not decision.allowed:
            raise BrowserScopeError(
                f"browser_scope policy denied '{url}' ({decision.reason}). "
                "See args/browser_scope.yaml."
            )
