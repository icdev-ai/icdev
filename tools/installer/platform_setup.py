...
CLI::
    ...
    python tools/installer/platform_setup.py --generate docker --modules core,llm,builder,dashboard --output docker-compose.yml  # noqa: E501
    ...
    python tools/installer/platform_setup.py --generate helm-values --modules core,llm,builder --output deploy/helm/values-custom.yaml  # noqa: E501
    python tools/installer/platform_setup.py --generate install-bash --modules core,llm --profile isv_startup --output install.sh  # noqa: E501
    python tools/installer/platform_setup.py --generate install-ps1 --modules core,llm --profile isv_startup --output install.ps1  # noqa: E501
...
class PlatformSetup:
    ...
    def generate_docker_compose(self, modules: List[str], profile_name: str = "") -> str:
        ...
        for name, port, dockerfile, tier, desc in services:
            ...
            lines.append("      - ICDEV_DB_PATH=/app/data/icdev.db")
            lines.append("      - ICDEV_PROJECT_ROOT=/app")
            lines.append(f"      - PORT={port}")
            lines.append("    env_file:")
            lines.append("      - .env")
            lines.append("    volumes:")
            lines.append("      - icdev-data:/app/data")
    ...
    def generate_install_script_bash(
        ...
        return textwrap.dedent(f"""\
            ...
            if command -v kubectl &>/dev/null; then
                info "kubectl: $(kubectl version --client --short 2>/dev/null || kubectl version --client 2>/dev/null | head -1)"  # noqa: E501
            else
