...
def run(dry_run: bool = False, clean: bool = False) -> dict:
    """Full prebuild sync: tools + data + orchestration bootstrap."""
    result = {
    ...
    print("[1/3] Syncing tools/ -> icdev/tools/ ...", file=sys.stderr)
    result["tools_sync"] = sync_tools(dry_run=dry_run, clean=clean)

    print("[2/3] Syncing FORGE data layers -> icdev/data/ ...", file=sys.stderr)
    result["data_sync"] = sync_data(dry_run=dry_run)

    # Run prebuild_bootstrap.py (CLAUDE.md + .claude/ + skills)
    if not dry_run:
        print("[3/3] Running prebuild_bootstrap.py ...", file=sys.stderr)
        try:
            bs = subprocess.run(
    ...
    result["totals"] = {
        ...
        "grand_total": total_tools + total_data + total_bs,
    }
    return result
