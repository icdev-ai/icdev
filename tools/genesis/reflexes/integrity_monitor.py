...
def _card_description(info: Dict[str, Any], assessment_id: int, source_ref: str, verdict: str) -> str:
    ...
    parts = [
        ...
        f"Capability:      {info['capability_type']}",
        f"File:            {info['rel_path']}"
    ...
    parts.extend(
        [
            ...
            "  2. Confirm whether a requirement authorizes it (RTM) — if yes, add it; "
            "if no, remediate.",
            "  3. Re-run: python -c \"from tools.integrity import engine; "
            "print(engine.assess('tools', mode='provenance_aware')['verdict'])\"",
            "  4. POST to /api/kanban/tasks/<task_id>/move {\"status\":\"done\"} when cleared.",
        ]
    )
    return "\n".join(parts)
