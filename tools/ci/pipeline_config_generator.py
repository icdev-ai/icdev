...
CHECK_REGISTRY = {
    ...
    "ironbank_generate": {
        ...
        "command": 'python tools/infra/ironbank_metadata_generator.py --project-id "$PROJECT_ID" --generate --output-dir .tmp/ironbank --json',  # noqa: E501
        "stage": "artifacts",
        "artifact": ".tmp/ironbank/hardening_manifest.yaml",
    },
    "ironbank_validate": {
        "name": "Iron Bank Manifest Validation",
        "command": 'python tools/infra/ironbank_metadata_generator.py --project-id "$PROJECT_ID" --validate --manifest-path .tmp/ironbank/hardening_manifest.yaml --json',  # noqa: E501
        "stage": "artifacts",
        "artifact": None,
    },
}
...
def generate_pipeline(
    ...
    if platform == "github":
        content = _generate_github_workflow(project_name, project_id, on_pr, on_merge, on_schedule, gates, config)
        output_path = cwd / ".github" / "workflows" / "icdev.yml"
    else:
        content = _generate_gitlab_ci(project_name, project_id, on_pr, on_merge, on_schedule, gates, config)
...
def _generate_github_workflow(
    ...
    if on_merge:
        ...
        lines.append(
            "    if: github.event_name == 'push' && (github.ref == 'refs/heads/main' || github.ref == 'refs/heads/master')"  # noqa: E501
        )
        lines.append("    runs-on: ubuntu-latest")
        lines.append("    steps:")
        lines.append("      - uses: actions/checkout@v4")
...
def main():
    parser = argparse.ArgumentParser(description="Generate CI/CD pipeline configs from icdev.yaml (D192)")
    parser.add_argument("--dir", help="Project directory containing icdev.yaml")
    parser.add_argument(
        "--platform", choices=["auto", "github", "gitlab"], default="auto", help="Target CI/CD platform"
    )
    parser.add_argument("--write", action="store_true", help="Write config file to disk")
    parser.add_argument("--dry-run", action="store_true", help="Generate but don't write (preview)")
