...
DIRECTORY_TREE = [
    ...
    "tools/llm",
    "tools/compat",
    "tools/cli",  # D-CHILD-9: fundamental infra
    "args",
    "context/agentic",
    "context/compliance",
    "context/languages",
    ...
    for adaptation in adaptations:
        if adaptation == "db_rename":
            ...
            if classification == "SECRET":
                ...
                content = content.replace("CUI Category: CTI", "Classification: SECRET")
...
def _build_fallback_manifest(blueprint: dict) -> list:
    ...
    capabilities = blueprint.get("capabilities", {})
    entries = []
    default_adaptations = ["db_rename", "port_remap", "classification_replace", "app_name_replace"]
...
def query_parent_rag(query: str, top_k: int = 5) -> dict:
    ...
    if "error" not in result and result.get("results"):
        try:
            conn = sqlite3.connect(cache_db)
            conn.execute(
                "INSERT OR REPLACE INTO rag_parent_cache (query_hash, results, expires_at) VALUES (?, ?, datetime('now', '+1 hour'))",  # noqa: E501
