...
def generate_init_script(blueprint: Dict[str, Any]) -> str:
    ...
    parts.append("")
    parts.append("# " + "=" * 60)
    parts.append("# BACKEND RESOLVER -- pgp-ca-02-d2")
    parts.append("# PG is the primary backend for child apps; SQLite is INIT-ONLY fallback.")
    parts.append("# The resolver is consulted ONLY at init time. Runtime must not")
    parts.append("# silently switch backends -- if a child app code path later needs a")
    parts.append("# connection, it must use the same _BACKEND value the init used.")
    parts.append("# " + "=" * 60)
    parts.append("try:")
    parts.append("    from tools.db.storage import get_connection as _parent_get_connection  # type: ignore")
    parts.append("    _HAS_PARENT_STORAGE = True")
    parts.append("except Exception:  # pragma: no cover -- parent layer unavailable")
    parts.append("    _parent_get_connection = None")
    parts.append("    _HAS_PARENT_STORAGE = False")
    parts.append("")
    parts.append("")
    parts.append("def _resolve_backend():")
    parts.append('    """Return the configured ICDEV_STORAGE_BACKEND (lowercased).')
    parts.append("")
    parts.append("    PG is the primary backend; SQLite is the init-only fallback.")
    parts.append('    Callers should treat anything other than "postgresql" as SQLite.')
    parts.append('    """')
    parts.append('    return os.environ.get("ICDEV_STORAGE_BACKEND", "sqlite").strip().lower()')
    parts.append("")
    parts.append("")
    parts.append("def _get_db_connection(db_path):")
    parts.append('    """Return a DB connection honoring ICDEV_STORAGE_BACKEND.')
    parts.append("")
    parts.append("    Rules (pgp-ca-02-d2):")
    parts.append("      * PG is PRIMARY. If ICDEV_STORAGE_BACKEND=postgresql and a parent")
    parts.append("        storage layer is available, use it (RLS is disabled because")
    parts.append("        child app tables have no tenant_id/classification columns).")
    parts.append("      * SQLite is the FALLBACK when PG is unavailable AT INIT TIME.")
    parts.append("        The parent storage layer handles the PG-unreachable probe;")
    parts.append("        if it raises, we fall back to sqlite3 so init can complete.")
    parts.append("      * Once a backend is chosen, RUNTIME MUST NOT silently switch.")
    parts.append("        Any post-init code path should re-use this function (which")
    parts.append("        honors the same env var) rather than picking a new backend.")
    parts.append("")
    parts.append("    Args:")
    parts.append("        db_path: Fallback SQLite path (used only when PG is unavailable")
    parts.append('                 or when ICDEV_STORAGE_BACKEND is unset/sqlite.')
    parts.append("")
    parts.append("    Returns:")
    parts.append("        A sqlite3.Connection (RLS context disabled) wrapping the")
    parts.append("        parent PG connection when available, otherwise a plain")
    parts.append("        sqlite3.connect(str(db_path)) connection.")
    parts.append('    """')
    parts.append("    backend = _resolve_backend()")
    parts.append('    if backend == "postgresql" and _HAS_PARENT_STORAGE:')
    parts.append("        try:")
    parts.append("            conn = _parent_get_connection()")
    parts.append("            # Child app tables have no tenant_id/classification columns,")
    parts.append("            # so disable RLS at the connection boundary.")
    parts.append("            try:")
    parts.append("                conn.set_security_context(None)  # type: ignore[attr-defined]")
    parts.append("            except Exception:  # pragma: no cover -- backend without RLS API")
    parts.append("                pass")
    parts.append("            return conn")
    parts.append("        except Exception:")
    parts.append("            # PG primary unreachable AT INIT TIME -> SQLite fallback.")
    parts.append("            # This branch runs only during initialization; runtime code")
    parts.append("            # is expected to re-resolve and surface a clear error rather")
    parts.append("            # than silently falling back.")
    parts.append("            pass")
    parts.append("    return sqlite3.connect(str(db_path))")
    parts.append("")
    parts.append("")
    parts.append("")
    ...
    parts.append("")
    parts.append("    # Resolves ICDEV_STORAGE_BACKEND at init time only.")
