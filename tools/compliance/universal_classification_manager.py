        ...
        result.append(
            {
                ...
                "sensitivity_rank": SENSITIVITY_ORDER.index(cat_id) if cat_id in SENSITIVITY_ORDER else 99,
            }
        )
...
def get_composite_banner(
    ...
    normalized.sort(key=lambda c: SENSITIVITY_ORDER.index(c) if c in SENSITIVITY_ORDER else 99)
...
def get_project_marking(
    ...
    subcats = {c["data_category"]: c.get("subcategory", "") for c in categories_data if c.get("subcategory")}
...
def detect_data_categories(
    ...
    try:
        ...
        result = {
            ...
            "advisory_note": "Detection is advisory (ADR D110). Confirm categories before applying markings.",
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
...
def validate_project_markings(
    ...
    try:
        ...
        if not categories:
            issues.append("No data categories assigned. Run --detect to identify applicable categories.")
            recommendations.append("Add at least one data category with --add-category.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Universal Data Classification & Marking Manager")
    parser.add_argument(
        "--list-categories",
        action="store_true",
        ...
        if args.detect:
            ...
            if args.json:
                ...
                for d in result["detected_categories"]:
                    print(f"  [{d['confidence']:.0%}] {d['category']}/{d['subcategory']} -- {d['name']}")
