...
def _parse_ts(ts_str: Optional[str]) -> Optional[datetime]:
    ...
    for fmt in (
        ...
        try:
            dt = datetime.strptime(ts_str.replace("+00:00", "").replace("Z", ""), fmt.replace("+00:00", "").replace("Z", ""))
    ...
    try:
        ...
        if _table_exists(conn, "pc_audit"):
            ...
            for row in conn.execute(sql, params).fetchall():
                controls, family, ev_type = _map_action_keyword(
                    row["action"], _PDC_ACTION_MAP
                )
                events.append({
                    "source": "pdc",
                    "event_id": str(row["id"]),
...
def build_evidence_chain(
    ...
    # Gate assessment
    gate_pass, gate_findings = _evaluate_gate(
        pdc_events, ndc_events, sdc_events, icdev_events
    )
...
def _evaluate_gate(
    ...
    if not pdc_events:
        findings.append(
            "WARN: No PDC (Pipeline Design Canvas) audit events found — "
            "CM/SA/SR control evidence missing"
        )
    if not ndc_events:
        findings.append(
            "WARN: No NDC (Network Design Canvas) audit events found — "
            "SC/AC control evidence missing"
        )
    if not sdc_events:
        findings.append(
            "WARN: No SDC (Security Design Canvas) audit events found — "
            "RA/CA control evidence missing"
        )
    if not icdev_events:
        findings.append(
            "WARN: No ICDEV compliance audit events found — "
            "AU/CA continuous monitoring evidence missing"
        )
    ...
    if not assessment_events:
        findings.append(
            "FAIL: No assessment-type evidence found — "
            "CA-2 (Security Assessments) requires periodic testing"
        )
    ...
    if len(unresolved) > 10:
        findings.append(
            f"FAIL: {len(unresolved)} unresolved compliance findings — "
            "CA-5 (POA&M) requires remediation tracking"
        )
...
def export_oscal_assessment_results(chain: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for ev in chain["timeline"]["events"]:
        ...
        obs = {
            ...
            "description": (
                f"[{ev['source'].upper()}] {ev['event_type']} — "
                f"actor: {ev.get('actor', 'unknown')}"
            ),
        ...
        "assessment-results": {
            ...
            "results": [{
                ...
                "reviewed-controls": {
                    ...
                    "control-selections": [{
                        "include-controls": [{"control-id": c} for c in reviewed_ids]
                    }],
                },
...
def _build_risks(chain: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for finding in chain["gate"]["findings"]:
        ...
        risks.append({
            ...
            "status": "open",
            "characterizations": [{
                "origin": {"actors": [{"type": "tool", "actor-uuid": str(uuid.uuid5(uuid.NAMESPACE_DNS, "evidence-chain"))}]},
...
def run_cli() -> None:
    parser = argparse.ArgumentParser(
        description="Continuous Compliance Evidence Chain (PDC/NDC/SDC → OSCAL)"
    )
