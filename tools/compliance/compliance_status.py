...
def _get_ivv_status(conn, project_id):
    ...
    try:
        critical_rows = conn.execute(
            "SELECT COUNT(*) as cnt FROM ivv_findings WHERE project_id = ? AND severity = 'critical' AND status = 'open'",  # noqa: E501
            (project_id,),
        ).fetchone()
    ...
    try:
        cert_row = conn.execute(
            "SELECT status, verification_score, validation_score, overall_score FROM ivv_certifications WHERE project_id = ?",  # noqa: E501
            (project_id,),
        ).fetchone()
        if cert_row:
