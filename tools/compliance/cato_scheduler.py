...
def run_scheduled_collections(project_id, project_dir=None, db_path=None):
    ...
    if collect_evidence is None:
        raise RuntimeError("cato_monitor.collect_evidence not available. Ensure cato_monitor.py is importable.")
    ...
    try:
        _log_audit_event(
            conn,
            project_id,
            "Scheduled collections executed",
            {
    ...
    print(f"cATO scheduled run: {result['total_due']} due, {result['collected']} collected, {result['failed']} failed")
...
def generate_collection_calendar(project_id, db_path=None):
    ...
    for w in weeks:
        if w["collection_count"] > 0:
            print(
                f"  Week {w['week_number']} ({w['start_date']} - {w['end_date']}): {w['collection_count']} collections"
            )
...
def _format_calendar_report(calendar):
    ...
    for week in calendar.get("weeks", []):
        ...
        lines.append(f"  Week {week['week_number']:>2}  {week['start_date']} - {week['end_date']}  {marker}")

        if count > 0:
            for coll in week["collections"][:5]:
                lines.append(f"           {coll['control_id']:<10} {coll['evidence_type']:<16} [{coll['frequency']}]")
            if count > 5:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="cATO evidence collection scheduler")
    ...
    parser.add_argument(
        "--project-dir", type=Path, default=None, help="Project directory for file-based evidence collection"
    )

    # Action flags (mutually exclusive)
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--run-due", action="store_true", help="Execute all collections that are currently due")
    group.add_argument("--upcoming", action="store_true", help="List collections due in the next N days")
    group.add_argument("--overdue", action="store_true", help="List overdue collections")
    group.add_argument("--calendar", action="store_true", help="Generate 12-week collection calendar")
    group.add_argument("--schedule", action="store_true", help="Show full collection schedule")

    # Optional parameters
    parser.add_argument("--days", type=int, default=30, help="Number of days to look ahead for --upcoming (default 30)")
        ...
        if args.run_due:
            ...
            if args.json:
                ...
                if not result:
                    ...
                    print(f"{'Control':<10} {'Type':<16} {'Source':<20} {'Freq':<12} {'Next Due':<22} {'Status'}")
                    print("-" * 90)
                    for item in result:
