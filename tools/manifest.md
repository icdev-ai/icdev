...
| Agent Adapter Registry (OPT-71) | tools/agents/ | Unified agent adapter protocol + registry (optio/packages/agent-adapters MIT). AgentAdapter Protocol + AgentSession/AgentResult dataclasses + pick_default() with env/config/fallback resolution. Ships adapters: claude_cli, local_llm_router, codex_cli (stub), copilot_cli (stub). Additive — no kanban.py refactor. | pick_default(task_type), get_adapter(name), detect_available() | AgentAdapter / list[str] |
