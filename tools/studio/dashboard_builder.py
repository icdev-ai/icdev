...
ROLE_DEFAULTS: dict[str, dict[str, Any]] = {
    ...
    "isso": {
        ...
        "widgets": [
            ...
            {"widget_type": "case_board_mini", "x": 0, "y": 2, "w": 6, "h": 5,
             "config": {"title": "Finding Tracker"}},
            {"widget_type": "compliance_gauge", "x": 6, "y": 2, "w": 3, "h": 3,
             "config": {"title": "CMMC Score", "framework": "cmmc"}},
            {"widget_type": "alert_feed", "x": 9, "y": 2, "w": 3, "h": 5,
    ...
    "developer": {
        ...
        "widgets": [
            ...
            {"widget_type": "workflow_status", "x": 0, "y": 2, "w": 6, "h": 4,
             "config": {"title": "Recent Builds"}},
            {"widget_type": "agent_grid", "x": 6, "y": 2, "w": 6, "h": 4,
    ...
    try:
        conn.execute(
            """INSERT INTO studio_dashboards
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (dash_id, name, layout_json, role_default, created_by,
             _now_iso(), _now_iso(), 1 if shared else 0),
        )
        conn.commit()
...
def get_dashboard(dash_id: str) -> dict | None:
    ...
    try:
        row = conn.execute(
            "SELECT * FROM studio_dashboards WHERE dashboard_id = ?", (dash_id,)
        ).fetchone()
        if not row:
            return None
        d = dict(row)
...
def delete_dashboard(dash_id: str) -> dict:
    ...
    try:
        cur = conn.execute(
            "DELETE FROM studio_dashboards WHERE dashboard_id = ?", (dash_id,)
        )
        conn.commit()
