...
def _load_mosa_config():
    ...
    defaults = {
        "allowed_protocols": ["REST", "gRPC", "AMQP", "Kafka", "SFTP", "JDBC", "GraphQL", "WebSocket"],
        "allowed_data_formats": ["JSON", "XML", "Protobuf", "CSV", "Avro", "Parquet"],
    }
    ...
    try:
        ...
        return {
            "allowed_protocols": icd.get("allowed_protocols", defaults["allowed_protocols"]),
    ...
    for path_str, methods in spec.get("paths", {}).items():
        for verb in ("get", "post", "put", "patch", "delete"):
            if verb in methods:
                op = methods[verb]
                endpoints.append(
                    f"| `{verb.upper()}` | `{path_str}` | {op.get('summary', op.get('operationId', ''))} |"
                )
    ep_table = (
        ("| Method | Path | Description |\n|--------|------|-------------|\n" + "\n".join(endpoints))
        if endpoints
        else "*No endpoints discovered.*"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Generate Interface Control Documents (ICDs) for MOSA compliance")
    parser.add_argument("--project-id", required=True, help="Project ID")
    parser.add_argument("--interface-id", help="Specific interface to generate ICD for")
    parser.add_argument(
        "--all", action="store_true", dest="gen_all", help="Generate ICDs for all discovered interfaces"
    )
    ...
    try:
        ...
        if args.gen_all:
            ...
            if not interfaces:
                msg = (
                    f"No OpenAPI/Swagger specs found in "
