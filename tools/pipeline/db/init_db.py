...
_PC_BACKEND = os.environ.get("PC_STORAGE_BACKEND", "sqlite").lower()
...
def get_connection():
    ...
    if _PC_BACKEND == "postgresql":
        try:
            ...
            conn = _icdev_conn(
                db_path=os.environ.get("PC_PG_DATABASE", "pipeline_canvas")
            )
            return conn
        except ImportError:
            pass
...
TEMPLATES = [
    ...
    },
    {
        ...
        "graph_json": json.dumps({
            "nodes": [
                _node("src", "GitLab (self-hosted)", "scm-gitlab", 50, 200, {"config": {"airgap": True, "fips": True}}),
    ...
    },
    {
        ...
        "graph_json": json.dumps({
            "nodes": [
                ...
                _node("ndc", "NDC Topology", "ndc-topology", 50, 50, {"config": {"ndc_topology_id": "", "ndc_topology_name": "Link your NDC topology"}}),
    ...
    },
    {
        ...
        "tags": json.dumps(["sre", "slo", "incident", "chaos", "dora", "runbook", "prometheus", "grafana", "backstage", "litmus"]),
        "graph_json": json.dumps({
            "nodes": [
    ...
    },
    {
        ...
        "graph_json": json.dumps({
            ...
            "edges": [
                _edge("src", "ci"),
                _edge("ci", "sast"), _edge("ci", "sca"),
                _edge("sast", "build"), _edge("sca", "build"),
                _edge("build", "scan"), _edge("build", "sbom"),
                _edge("scan", "sign"), _edge("sign", "harbor"),
                _edge("harbor", "gate"), _edge("gate", "argo"),
                _edge("argo", "k8s"),
                # SRE wiring
                _edge("k8s", "prom", "metrics"),
    ...
    },
    {
        ...
        "graph_json": json.dumps({
            ...
            "edges": [
                ...
                _edge("src", "ci"),
                _edge("ci", "sast"), _edge("ci", "sca"), _edge("ci", "iac"),
                _edge("sast", "sbom"), _edge("sca", "sbom"),
                _edge("sbom", "sign"),
                _edge("sign", "harbor"),
    ...
    },
    {
        ...
        "graph_json": json.dumps({
            "nodes": [
                ...
                _node("src", "GitLab (SIPR)", "scm-gitlab", 250, 200, {"config": {"network": "SIPR", "fips": True}}),
...
SRE_SNIPPETS = [
    ...
    },
    {
        ...
        "graph_json": json.dumps({
            "nodes": [
                _node("ci", "CI/CD Engine", "cicd-gitlab", 50, 150, {"config": {"note": "Source of deploy events"}}),
    ...
    },
    {
        ...
        "graph_json": json.dumps({
            "nodes": [
                _node("cb", "Circuit Breaker", "sre-circuit-breaker", 50, 100, {"config": {"failure_threshold": 5, "recovery_timeout": 30}}),
                _node("retry", "Retry + Backoff", "sre-retry", 50, 250, {"config": {"max_retries": 3, "base_delay": 1.0}}),
...
CROSS_DOMAIN_SNIPPETS = [
    {
        ...
        "graph_json": json.dumps({
            "nodes": [
                # Commercial side
                _node("src-comm", "GitLab (Commercial)", "scm-gitlab", 50, 200, {"config": {"boundary": "commercial"}}),
                ...
                # GovCloud side
                _node("reg-gov", "Harbor (GovCloud)", "registry-harbor", 850, 200, {"config": {"boundary": "govcloud"}}),
        ...
        if existing == 0:
            ...
            for snip in all_snippets:
                conn.execute(
                    ...
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (snip["id"], snip["name"], snip["category"], snip["description"],
                     snip["classification_level"], snip["impact_level"],
                     snip["slsa_level"], snip["ssdf_practices"],
                     snip["graph_json"], snip["tags"]),
                )
            conn.commit()
