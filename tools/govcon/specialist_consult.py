...
def _redact(question: str, context_summary: str) -> tuple[str, str]:
    """Sanitize both strings via GovConSanitizer before they leave this
    process. Falls back to the original text unmodified if the redaction
    module can't be imported -- this must never block the pipeline, but
    every effort is still made to sanitize first."""
    try:
        ...
        return question, context_summary
...
def request_council_consult(
    ...
    safe_question, safe_context = _redact(question, context_summary)
    base_url = _base_url()
