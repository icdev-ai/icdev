...
def _nlp_extract_stig_refs(text: str, task: str) -> Optional[dict]:
    ...
    system_msg = (
        "You are a STIG compliance analyst. "
        'Output ONLY a JSON object: {"found": <bool>, "refs": [<str>, ...], "confidence": <float 0-1>}. '
        "refs contains V-IDs (e.g. V-220938), CAT severity labels, or STIG document names. "
        "Set found to false and refs to [] when nothing is detected. No prose or markdown fences."
    )
    user_msg = f"{task}.\n\nText:\n{sample}"
    try:
        ...
        # Prefer provider-parsed structured output when available
        if response.structured_output and isinstance(response.structured_output, dict):
            return response.structured_output
        # Direct JSON parse — LLM instructed to emit JSON only; no regex on external input
        result_text = response.content.strip()
        # Strip markdown code fences via string ops (avoids regex on LLM output)
        if result_text.startswith("```"):
            newline_pos = result_text.find("\n")
            result_text = result_text[newline_pos + 1:] if newline_pos != -1 else result_text[3:]
