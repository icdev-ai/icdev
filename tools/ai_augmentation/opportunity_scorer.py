...
# ============================================================================
# Anomaly detection on score results
# ============================================================================


def detect_score_anomalies(scores: dict, config_path=None) -> list:
    """Detect anomalies in a scored opportunity using config-driven thresholds.

    Replaces hardcoded numeric guards with values loaded from
    args/aac_config.yaml ``anomaly_detection`` section.

    Checks:
      - |value_score - feasibility_score| > value_feasibility_max_delta
      - any component in score_detail.components below component_outlier_floor
      - any component in score_detail.components above component_outlier_ceiling

    Args:
        scores:      Return value of score_opportunity().
        config_path: Optional override path to the config YAML file.

    Returns:
        List of dicts, each with keys ``check``, ``detail``, and ``value``.
        Empty list if no anomalies detected.
    """
    cfg = _load_config(config_path)
    ad_cfg = cfg.get("anomaly_detection", _FALLBACK_CONFIG["anomaly_detection"])

    max_delta = float(ad_cfg.get("value_feasibility_max_delta", 0.50))
    floor = float(ad_cfg.get("component_outlier_floor", 0.05))
    ceiling = float(ad_cfg.get("component_outlier_ceiling", 0.95))

    anomalies = []

    value = scores.get("value_score", 0.0)
    feasibility = scores.get("feasibility_score", 0.0)
    delta = abs(value - feasibility)
    if delta > max_delta:
        anomalies.append({
            "check": "value_feasibility_delta",
            "detail": f"|value({value:.4f}) - feasibility({feasibility:.4f})| = {delta:.4f} exceeds {max_delta}",
            "value": delta,
        })

    components = scores.get("score_detail", {}).get("components", {})
    for name, val in components.items():
        if val < floor:
            anomalies.append({
                "check": "component_outlier_low",
                "detail": f"component '{name}' = {val:.4f} below floor {floor}",
                "value": val,
            })
        elif val > ceiling:
            anomalies.append({
                "check": "component_outlier_high",
                "detail": f"component '{name}' = {val:.4f} above ceiling {ceiling}",
                "value": val,
            })
...
def main():
    ...
    anomalies = detect_score_anomalies(result)
    if anomalies:
        result["score_anomalies"] = anomalies
