...
TIER_EMAIL = "EMAIL"
TIER_REMEDIATION = "REMEDIATION-DIAGRAM"
...
def build_context(
    ...
    diagram_findings: list[dict[str, Any]] = []
    remediation_diagrams: list[str] = []  # Mermaid blocks from each diagram analysis
    ...
    for analysis in (analyses or []):
        ...
        if raw_json:
            try:
                ...
                # Pick up the remediation diagram generated by diagram_analysis.py
    ...
    # 6. Assemble supplemental_notes (tier-ordered: OPERATOR first, then EMAIL, REMEDIATION, SOURCE)
    ...
    # Remediation diagrams — include AFTER email (high confidence) so they inform document structure
    for idx, rdiag in enumerate(remediation_diagrams, 1):
        label = f"Diagram {idx}" if len(remediation_diagrams) > 1 else "Diagram"
        notes_parts.append(
            f"[{TIER_REMEDIATION}] Remediated network state ({label}):\n{rdiag}"
        )
        log.info("IDR context: included remediation diagram %d/%d", idx, len(remediation_diagrams))
