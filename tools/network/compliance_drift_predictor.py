...
# STIG check definitions: (check_id, description, is_critical, pass_fn)
# pass_fn(config_text) -> True if passing
_STIG_CHECKS = [
    # critical
    ("V-220518", "SSH v2 required",           True,  lambda c: "ip ssh version 2" in c),
    ("V-220519", "No telnet",                 True,  lambda c: "service telnet" not in c),
    ("V-220521", "No unsecure HTTP server",   True,  lambda c: "ip http server" not in c or "ip http secure-server" in c),
    # non-critical
    ("V-220520", "Password encryption enabled", False, lambda c: "service password-encryption" in c),
    ("V-220522", "Logging configured",          False, lambda c: "logging buffered" in c or "logging host" in c),
    ("V-220523", "NTP configured",              False, lambda c: "ntp server" in c),
    ("V-220524", "Exec timeout configured",     False, lambda c: "exec-timeout" in c),
    ("V-220525", "MOTD banner configured",      False, lambda c: "banner motd" in c or "banner login" in c),
    ("V-220526", "AAA configured",              False, lambda c: "aaa new-model" in c),
    ("V-220527", "SNMPv3 only",                 False, lambda c: "snmp-server group" in c and "v3" in c),
]
...
def _get_previous_score(conn, device_name: str) -> Optional[float]:
    try:
        cur = conn.execute(
...
def _compute_drift_risk(current_score: float, drift_delta: float, critical_failing: int) -> tuple:
    ...
    drift_penalty = min(0.20, max(0.0, -drift_delta * 5))
...
def predict_compliance_drift(device_name=None, network_id=None):
    ...
    with get_connection() as conn:
        for dev in devices:
            ...
            drift_delta = 0.0 if last_score is None else round(current_score - last_score, 4)
