  ...
  policy = jsonencode({{
    ...
    Statement = [
      ...
      {{
        ...
        Action   = "s3:PutObject"
        Resource = "${{aws_s3_bucket.config_logs.arn}}/AWSLogs/${{data.aws_caller_identity.current.account_id}}/Config/*"  # noqa: E501
        Condition = {{
          StringEquals = {{
...
resource "aws_flow_log" "cloudwatch" {{
  ...
  log_format = "${{version}} ${{account-id}} ${{interface-id}} ${{srcaddr}} ${{dstaddr}} ${{srcport}} ${{dstport}} ${{protocol}} ${{packets}} ${{bytes}} ${{windowstart}} ${{windowend}} ${{action}} ${{log-status}} ${{vpc-id}} ${{subnet-id}} ${{instance-id}} ${{tcp-flags}} ${{type}} ${{pkt-srcaddr}} ${{pkt-dstaddr}} ${{region}} ${{az-id}} ${{sublocation-type}} ${{sublocation-id}} ${{pkt-src-aws-service}} ${{pkt-dst-aws-service}} ${{flow-direction}} ${{traffic-path}}"  # noqa: E501
...
resource "aws_cloudwatch_metric_filter" "rejected_traffic" {{
  ...
  pattern        = "[version, account_id, interface_id, srcaddr, dstaddr, srcport, dstport, protocol, packets, bytes, windowstart, windowend, action = REJECT, log_status]"  # noqa: E501
...
def main():
    ...
    parser.add_argument(
        "--human", action="store_true", help="Output human-readable summary (default when --json not set)"
    )
