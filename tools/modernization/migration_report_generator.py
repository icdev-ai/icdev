...
def generate_assessment_report(app_id, output_dir=None, db_path=None):
    ...
    try:
        ...
        assess_row = conn.execute(
            "SELECT * FROM migration_assessments WHERE legacy_app_id = ? ORDER BY rowid DESC LIMIT 1", (app_id,)
        ).fetchone()
    ...
    lines.append("")
    lines.append(
        f"Showing top {len(components)} components by cyclomatic complexity (out of {total_components} total)."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        f"**Recommended Strategy:** **{recommended.upper()}** "
    ...
    lines.append("")
    lines.append(
        "Risk score combines strategy inherent risk (40%), application "
    ...
    if maint is not None:
        ...
        lines.append(
            f"- **Application Health Risk:** {health_risk:.2f} (maintainability index: {_format_number(maint)})"
        )
...
def generate_progress_report(plan_id, pi_number=None, output_dir=None, db_path=None):
    ...
    try:
        ...
        if pi_number:
            progress_rows = conn.execute(
                "SELECT * FROM migration_progress WHERE plan_id = ? AND pi_number = ? ORDER BY rowid DESC",
                (plan_id, pi_number),
            ).fetchall()
        else:
            progress_rows = conn.execute(
    ...
    lines.append(f"**Overall Completion:** {completion_pct:.1f}% ({completed}/{total_tasks} tasks)")
    lines.append("")
    ...
    if pi_number and progress:
        ...
        lines.append("")
        lines.append(
            "| PI | Tasks Total | Completed | In Progress | Blocked | Components Migrated | Test Coverage | Hours |"
        )
        lines.append(
            "|-----|-------------|-----------|-------------|---------|--------------------|---------------|-------|"
        )
...
def generate_ato_impact_report(plan_id, output_dir=None, db_path=None):
    ...
    lines.append("")
    if controls:
        lines.append(f"The **{strategy.upper()}** strategy impacts **{len(controls)}** NIST 800-53 control families:")
        lines.append("")
    ...
    if compliance_links:
        ...
        lines.append("*No existing compliance mapping data available. Manual compliance assessment recommended.*")
        lines.append("")
        lines.append(
            f"Based on the **{strategy.upper()}** strategy, the following "
    ...
    lines.append("")
    lines.append(
        f"The **{strategy.upper()}** migration strategy adds an estimated "
...
def generate_executive_summary(app_id, output_dir=None, db_path=None):
    ...
    for sid, sname in strategy_names:
        ...
        lines.append(
            f"| {marker}{sname}{end_marker} | {score_val:.4f} | {_format_number(s_cost)} | {s_time} | {s_risk} |"
        )
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    if ato_impact in ("high", "critical"):
        ...
        findings.append(
            "ATO impact is **NONE**. The existing authorization boundary is unaffected by this migration strategy."
        )
    ...
    if strengths:
        lines.append(
            f"The application demonstrates strong fitness in: {', '.join(s.replace('_', ' ') for s in strengths)}."
        )
    if weaknesses:
        lines.append(f"Areas requiring attention: {', '.join(w.replace('_', ' ') for w in weaknesses)}.")
    lines.append("")
...
def generate_all_reports(app_id, plan_id=None, pi_number=None, output_dir=".", db_path=None):
    ...
    try:
        results["assessment"] = generate_assessment_report(app_id, output_dir=str(out_dir), db_path=db_path)
    except Exception as exc:
        errors["assessment"] = str(exc)

    try:
        results["executive"] = generate_executive_summary(app_id, output_dir=str(out_dir), db_path=db_path)
    except Exception as exc:
    ...
    if plan_id:
        try:
            results["progress"] = generate_progress_report(
                plan_id, pi_number=pi_number, output_dir=str(out_dir), db_path=db_path
            )
        except Exception as exc:
            errors["progress"] = str(exc)

        try:
            results["ato_impact"] = generate_ato_impact_report(plan_id, output_dir=str(out_dir), db_path=db_path)
        except Exception as exc:
...
def main():
    ...
    try:
        if args.report_type == "assessment":
            filepath = generate_assessment_report(args.app_id, output_dir=args.output_dir, db_path=args.db_path)
            if args.json_output:
                print(
                    json.dumps(
                        {
            ...
            filepath = generate_progress_report(
                args.plan_id, pi_number=args.pi_number, output_dir=args.output_dir, db_path=args.db_path
            )
            if args.json_output:
                print(
                    json.dumps(
            ...
            filepath = generate_ato_impact_report(args.plan_id, output_dir=args.output_dir, db_path=args.db_path)
            if args.json_output:
                print(
                    json.dumps(
                        {
            ...
            filepath = generate_executive_summary(args.app_id, output_dir=args.output_dir, db_path=args.db_path)
            if args.json_output:
                print(
                    json.dumps(
                        {
