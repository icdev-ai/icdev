...
def extract_call_graph(app_id: str) -> dict:
    ...
    try:
        ...
        for r in edge_rows:
            ...
            cids.add(r["target_component_id"])
        nodes = []
        if cids:
...
def extract_component_diagram(app_id: str) -> dict:
    ...
    try:
        ...
        deps = conn.execute(
            "SELECT source_component_id,target_component_id FROM legacy_dependencies WHERE legacy_app_id=? AND target_component_id IS NOT NULL",  # noqa: E501
            (app_id,),
        ).fetchall()
    ...
    try:
        ...
        for api in apis:
            ...
            while queue:
                ...
                if c["component_type"] in DB_LAYER_TYPES:
                    rdb = True
...
def detect_service_boundaries(app_id: str) -> dict:
    ...
    try:
        ...
        deps = conn.execute(
            "SELECT source_component_id,target_component_id,weight FROM legacy_dependencies WHERE legacy_app_id=? AND target_component_id IS NOT NULL",  # noqa: E501
            (app_id,),
        ).fetchall()
        if not comps:
        ...
        for d in deps:
            ...
            if s not in nidx or t not in nidx:
                continue
            i, j = nidx[s], nidx[t]
            adj[i][j] += w
            adj[j][i] += w
    ...
    for pf in src.rglob("*.py"):
        ...
        for ln in content.splitlines():
            ...
            if mm and ct:
                old = ct
                ct = mm.group(1)
                if old in tbls and old != ct:
                    tbls[ct] = tbls.pop(old)
            if ct:
    ...
    for jf in list(src.rglob("*.java")) + list(src.rglob("*.kt")):
        ...
        if not tbls[tn]["columns"]:
            for fm in _RE["hb_fld"].finditer(content):
                if fm.group(1) in ("class", "interface", "void", "return", "static"):
                    continue
        ...
        for p in props:
            ...
            for fp, fn in fkas:
                if abs(fp - p.start()) < 100 and fp < p.start():
                    ...
                    rels.append({"from_table": tn, "from_col": pn, "to_table": fn.lower(), "to_col": "Id"})
                    break
    ...
    for sf in src.rglob("*.sql"):
        ...
        for stmt in re.split(r";\s*\n", content):
            ...
            if ct:
                ...
                tbls.setdefault(tn, {"columns": []})
                pks = set()
                for m in _RE["sql_pki"].finditer(s):
                    pks.add(m.group(1).lower())
...
def extract_database_schema(app_id: str, source_path: str) -> dict:
    ...
    try:
        ...
        if lang == "python":
            ...
            _parse_hibernate(src, tbls, rels)
        elif lang in ("csharp", "c#"):
            _parse_ef(src, tbls, rels)
        ...
        for tn, ti in tbls.items():
            for col in ti.get("columns", []):
                try:
                    conn.execute(
                        "INSERT OR IGNORE INTO legacy_db_schemas (id,legacy_app_id,db_type,schema_name,table_name,column_name,data_type,is_primary_key,is_foreign_key,foreign_table,foreign_column) VALUES (?,?,?,'public',?,?,?,?,?,?,?)",  # noqa: E501
                        (
                            str(uuid.uuid4()),
                            app_id,
                            dbt,
...
def _detect_style(tc: collections.Counter, total: int) -> str:
    ...
    if has("controller") and has("service") and has("repository"):
        return "layered"
    if has("service") and has("api_endpoint") and tc.get("service", 0) >= 3:
        return "microservice-candidate"
    if has("controller") and (has("model") or has("entity")):
        return "mvc"
    if has("service") and has("repository"):
        return "layered"
    return "monolith"
...
def generate_architecture_summary(app_id: str) -> dict:
    ...
    try:
        ar = conn.execute(
            "SELECT name,primary_language,framework,loc_total,complexity_score,maintainability_index FROM legacy_applications WHERE id=?",  # noqa: E501
            (app_id,),
        ).fetchone()
        if not ar:
            raise ValueError(f"Application {app_id} not found in database.")
        ...
        hs = [
            ...
            for h in conn.execute(
                "SELECT name,coupling_score FROM legacy_components WHERE legacy_app_id=? ORDER BY coupling_score DESC LIMIT 5",  # noqa: E501
                (app_id,),
            ).fetchall()
        ]
    finally:
    ...
    cd = extract_component_diagram(app_id)
    sb = detect_service_boundaries(app_id)
    cxd = {"low": 0, "medium": 0, "high": 0, "very_high": 0}
    cvs = []
...
def _fmt_cg(d):
    ...
    if d["edges"]:
        ...
        for e in d["edges"]:
            by_s[e["source"]].append(e)
        ls += ["  DEPENDENCIES:", "  " + "-" * 66]
        for sid, es in sorted(by_s.items(), key=lambda x: nm.get(x[0], "")):
            for e in es:
                ar = "-->" if e["type"] == "method_call" else "..>"
                ls.append(
                    f"  [{nm.get(sid, sid[:10])[:20]}] {ar} [{nm.get(e['target'], e['target'][:10])[:20]}]  ({e['type']}, w={e['weight']:.1f})"  # noqa: E501
                )
        ls.append("")
    ls += ["=" * 70, "CUI // SP-CTI"]
    return "\n".join(ls)
...
def _fmt_df(d):
    ...
    for f in d["flows"]:
        ...
        if f["chain"]:
            ...
            ls.append("    (no component chain traced)")
        ls.append("")
    ls += ["=" * 70, "CUI // SP-CTI"]
    return "\n".join(ls)
...
def _fmt_ds(d):
    ...
    if d["relationships"]:
        ...
        for r in d["relationships"]:
            ls.append(f"  {r['from_table']}.{r['from_col']} --> {r['to_table']}.{r['to_col']}")
        ls.append("")
    ls += ["=" * 70, "CUI // SP-CTI"]
    return "\n".join(ls)
