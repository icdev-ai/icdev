            ...
            try:
                conn.execute(
                    ...
                    (
                        ...
                        f"Strangler fig cutover: component migrated to modern implementation",
                    ),
                )
            except sqlite3.IntegrityError:
...
def _generate_python_acl(class_name, boundary, components, api_by_comp, now):
    ...
    lines = [
        ...
        f"Maps legacy interfaces to modern domain model.",
        f"",
        ...
        "",
        f"    def __init__(self, legacy_client, modern_client):",
        f'        """Initialize with both legacy and modern service clients.',
        f"",
        f"        Args:",
        f"            legacy_client: Client for the legacy system.",
        f"            modern_client: Client for the modern system.",
        f'        """',
        f"        self._legacy = legacy_client",
        f"        self._modern = modern_client",
        "",
    ]
    ...
    for comp in components:
        ...
        if comp_apis:
            lines.append(f"        Legacy endpoints:")
            for api in comp_apis:
                lines.append(f"            {api['method']} {api['path']}")
        lines.append(f"")
        lines.append(f"        Args:")
        lines.append(f"            request_data: dict with legacy request payload.")
        lines.append(f"")
        lines.append(f"        Returns:")
        lines.append(f"            dict with translated modern response.")
        lines.append(f'        """')
        lines.append(f"        # TODO: Implement legacy-to-modern data mapping")
        lines.append(f"        modern_request = self._map_request(request_data)")
        lines.append(f"        response = self._modern.call(modern_request)")
        lines.append(f"        return self._map_response(response)")
        lines.append("")
