...
# sits beside icdev/data/args/ and icdev/data/context/, not icdev/args/.
CATALOG_CANDIDATES = (
    BASE_DIR / "args" / "benchmark_subsystems.yaml",
    BASE_DIR / "data" / "args" / "benchmark_subsystems.yaml",
)
...
def _first_existing(candidates: Sequence[Path]) -> Path:
    """First candidate that exists, else the first — so errors name the
    checkout path a reader expects rather than the packaged fallback."""
...
DEFAULT_CATALOG = _first_existing(CATALOG_CANDIDATES)
DEFAULT_WATCHLIST = _first_existing(WATCHLIST_CANDIDATES)
