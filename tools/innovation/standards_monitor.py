...
FRAMEWORK_KEYWORD_MAP = {
    "nist_800_53": ["800-53", "SP 800-53"],
    "fedramp": ["FedRAMP", "cloud authorization"],
    "cmmc": ["CMMC", "cybersecurity maturity model"],
    "nist_800_171": ["800-171", "CUI protection"],
    "fips_199": ["FIPS 199", "security categorization"],
    "fips_200": ["FIPS 200"],
    "fips_140": ["FIPS 140", "cryptographic module"],
    "nist_800_207": ["800-207", "zero trust"],
    "cisa_sbd": ["secure by design", "memory safe"],
    "cato": ["cATO", "continuous ATO"],
    "mosa": ["MOSA", "modular open systems"],
    "devsecops": ["DevSecOps", "pipeline security"],
    "nist_csf": ["cybersecurity framework", "NIST CSF"],
    "iso_27001": ["ISO 27001", "27001:2022"],
    "hipaa": ["HIPAA"],
    "pci_dss": ["PCI DSS"],
    "cjis": ["CJIS", "criminal justice"],
    "sbom": ["SBOM", "software bill of materials"],
    "nist_800_161": ["800-161", "C-SCRM"],
}
CATALOG_KEYWORDS = [
    "revision",
    "final",
...
def _ensure_table(db_path=None):
    ...
    try:
        ...
        for col in ("body", "status", "created_at"):
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_isu_{col} ON innovation_standards_updates ({col})")
        conn.commit()
    finally:
        conn.close()
...
def _parse_feed(xml_bytes):
    ...
    try:
        ...
        return []
    items = (
        root.findall("atom:entry", ATOM_NS) or root.findall("entry") or (root.find("channel") or root).findall("item")
    )
    entries = []
    for item in items:
        ...
        pub_type = (
            "SP"
            if any(k in tu for k in ("SP ", "SP-"))
            else "FIPS"
            if "FIPS" in tu
            else "IR"
            if any(k in tu for k in ("IR ", "NISTIR"))
            else "directive"
            if any(k in tu for k in ("DIRECTIVE", "BOD"))
            else "advisory"
            if "ADVISORY" in tu
            else "memo"
            if "MEMO" in tu
...
def check_nist_updates(config, db_path=None):
    ...
    return _check_feed_body("nist", config, db_path, url_key="publications_url", rss_key="rss_url", filter_fn=_f)
...
BODY_CHECKERS = {
    "nist": check_nist_updates,
    "cisa": check_cisa_updates,
    "dod": check_dod_updates,
    "fedramp": check_fedramp_updates,
...
def check_all_bodies(db_path=None):
    ...
    totals = {
        "updates_found": sum(r.get("updates", 0) for r in results.values()),
        "new_stored": sum(r.get("stored", 0) for r in results.values()),
    }
    _audit("standards.check_all", "standards-monitor", f"Checked {len(results)} bodies", totals)
    return {"check_time": _now(), "bodies_checked": len(results), "results": results, "totals": totals}
...
def assess_impact(update_id, db_path=None):
    ...
    try:
        ...
        affected = [fw for fw, kws in FRAMEWORK_KEYWORD_MAP.items() if any(k.lower() in text for k in kws)]
        ...
        priority = round(
            min(
                {"fips": 0.40, "directive": 0.35, "sp": 0.25, "advisory": 0.20, "ir": 0.15, "memo": 0.15}.get(pt, 0.10)
                + {"nist": 0.15, "cisa": 0.15, "dod": 0.10, "fedramp": 0.10, "iso": 0.05}.get(body, 0.05)
                + min(len(affected) * 0.05, 0.25)
                + (0.15 if catalog_needed else 0),
                1.0,
            ),
            2,
        )
        ...
        if crosswalk_hit:
            actions.append(
                {"action": "review_crosswalk", "urgency": "high", "description": "Check dual-hub crosswalk mappings"}
            )
        for fw in affected:
            actions.append(
                {
        ...
        if not actions:
            actions.append(
                {"action": "monitor", "urgency": "low", "description": "No immediate ICDEV impact; continue monitoring"}
            )
        assessment = {
            "affected_frameworks": affected,
            "catalog_update_needed": catalog_needed,
            "crosswalk_impact": crosswalk_hit,
            "priority": priority,
...
def main():
    ...
    try:
        ...
        print(json.dumps({"error": str(e)}, indent=2) if args.json else f"ERROR: {e}", file=sys.stderr)
        sys.exit(1)
...
def _print_human(args, result):
    ...
    if args.check and args.all:
        t = result.get("totals", {})
        print(f"Checked {result['bodies_checked']} bodies  |  Found: {t['updates_found']}  New: {t['new_stored']}")
        for n, r in result.get("results", {}).items():
            err = r.get("error", r.get("skipped", ""))
            print(f"  {n}: {r.get('updates', 0)} updates, {r.get('stored', 0)} new" + (f"  ({err})" if err else ""))
    elif args.check:
        print(f"{result['body']}: {result.get('updates', 0)} found, {result.get('stored', 0)} new")
    elif args.report:
        print(
            f"Standards Report ({result['period_days']}d)  Total: {result['total_updates']}  "
        ...
        for item in result.get("assessed_updates", [])[:10]:
            ...
            print(f"  [{item['priority']:.2f}] {item['body']}: {item['title'][:55]}" + (f"  -> {fws}" if fws else ""))
    elif args.assess:
