...
GAP_HANDLER_TOOLS = {
    ...
    "translate_code": {
        ...
        "description": "Run full cross-language translation pipeline (Extract, Type-Check, Translate, Assemble, Validate+Repair).",  # noqa: E501
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "evaluate_capability": {
        ...
        "description": "Evaluate a capability across 6 dimensions (universality, compliance_safety, risk, evidence, novelty, cost).",  # noqa: E501
        "input_schema": {
            "type": "object",
...
def extract_from_server(category: str, module_path: str) -> tuple:
    ...
    for uri, res_info in server._resources.items():
        ...
        handler_name = (
            handler_fn.__name__
            if handler_fn and hasattr(handler_fn, "__name__")
            else f"handle_resource_{uri.split('://')[-1].replace('/', '_')}"
        )
...
def main():
    ...
    for cat_name in categories:
        ...
        for tool_name, tool_info in cat_tools.items():
            ...
            schema_str = (
                schema_str.replace(": true", ": True").replace(": false", ": False").replace(": null", ": None")
            )
