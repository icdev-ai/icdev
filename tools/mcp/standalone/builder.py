...
def _resolve_base_dir():
    ...
    if env_dir and Path(env_dir).is_dir():
        return Path(env_dir)
    # Infer from package location (tools/mcp/standalone/builder.py -> 4 levels up)
...
def main():
    ...
    try:
        from tools.mcp.builder_server import create_server
        ...
        try:
            ...
            logger.error("Failed to start Builder MCP server: %s", exc)
            sys.exit(1)
    except Exception as exc:
        logger.error("Failed to start Builder MCP server: %s", exc)
        sys.exit(1)
