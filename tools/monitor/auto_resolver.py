...
def _update_status(
    ...
    try:
        ...
        conn.execute(f"UPDATE auto_resolution_log SET {', '.join(sets)} WHERE id = ?", params)
        conn.commit()
    finally:
        conn.close()
...
def _notify_resolution(resolution_id: str, decision: str, details: dict) -> None:
    ...
    try:
        ...
        req = urllib.request.Request("http://localhost:5000/api/events/ingest", data=data, method="POST")
