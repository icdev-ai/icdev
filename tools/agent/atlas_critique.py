...
class AtlasCritique:
    ...
    def get_session_status(self, session_id: str) -> dict:
        ...
        try:
            ...
            findings = conn.execute(
                "SELECT * FROM atlas_critique_findings WHERE session_id = ? ORDER BY round_number, severity",
                (session_id,),
            ).fetchall()
    ...
    def get_history(self, project_id: str, limit: int = 20) -> dict:
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM atlas_critique_sessions WHERE project_id = ? ORDER BY created_at DESC LIMIT ?",
                (project_id, limit),
            ).fetchall()
...
def main():
    ...
    if args.json_output:
        ...
        try:
            from tools.cli.output_formatter import format_banner, format_table  # noqa: F401
