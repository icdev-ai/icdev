...
Registers three collections on the module-level Executor:
  cortex.sessions       — canvas chat/agent sessions (cortex_sessions)
...
def sessions_adapter(conn: Any) -> list[dict]:
    ...
    try:
        cur = c.execute(
            ...
            "FROM cortex_sessions ORDER BY created_at DESC LIMIT 500"
        )
...
register_collection("cortex.sessions", sessions_adapter)
