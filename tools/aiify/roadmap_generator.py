# CUI // SP-CTI
"""AI-ify Canvas — Roadmap Generator.
...
Persists result to aiify_roadmaps via get_connection().
"""
...
from tools.aiify.db.init_db import get_connection

_ICDEV_ROOT = Path(__file__).resolve().parents[2]
_CATALOG_PATH = _ICDEV_ROOT / "context" / "aiify" / "pattern_catalog.json"
...
def generate_roadmap(scan_id: str, opportunities: list[dict], scores: list[dict]) -> dict:
    ...
    Args:
        ...
        opportunities: List of opportunity dicts from aiify_opportunities.
        scores:        List of score dicts from aiify_scores.
    ...
    title = f"AI-ify Roadmap — Scan {scan_id}"
    created_at = _now()
