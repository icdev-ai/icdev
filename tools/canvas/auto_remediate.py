...
def _load_findings_from_db(canvas_key: str, design_id: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        row = conn.execute(
            f"SELECT findings_json FROM {table_name} WHERE design_id = %s ORDER BY created_at DESC LIMIT 1",  # noqa: S608  # nosec B608 — table_name from registry constants, not user input
            (design_id,),
        ).fetchone()
        conn.close()
