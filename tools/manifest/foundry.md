...
| DB Schema | tools/foundry/db/init_db.py | Creates `foundry_signals` (append-only, RLS-aware: tenant_id + classification). Dual PG/SQLite schema. UNIQUE(dedup_hash) enforces cross-source dedup at the DB layer. | init_db(force=False) (alias `init_foundry_db` for legacy callers) | foundry_signals table |
