...
| Agent Executor | tools/studio/executors/agent_executor.py | Runs an agent loop as a Studio workflow step (`node_type: agent`, hgx-agent-01). Invokes `icdev.tools.llm.agent_loop.run_agent_loop` with the hgx-exec-01 worktree toolset intersected with the step's `agent_tools` bundles (default-deny via `toolsets.resolve_bundles`), every call gated by `agent_runtime.approval_gate.build_approval_hook`. Routes by `llm_function`, never a model id. Exit 0 on a degraded step (`AgentLoopUnsupported`) so the run continues. Writes the payload to run memory under `step:<step_id>` and declares written files as `artifacts`. | `--prompt <text> --agent-tools <bundles> [--llm-function --work-dir --system-prompt --max-iterations --max-tokens --effort --approval-mode --run-id --step-id --project-id --json]` | JSON `{status, step_id, llm_function, agent_tools, tools_offered, degraded, turns, artifacts, memory_written}` |
