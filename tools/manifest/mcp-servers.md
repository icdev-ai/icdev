...
| MCP Dispatch Audit (server-side) | `tools/mcp/dispatch_audit.py` | xrv-cost-05. One `studio_mcp_dispatch_audit` row per `tools/call`, whoever called. `studio_mcp_dispatch_audit` had exactly two writers, both Studio-internal, so `capability_consumption --class mcp_dispatch_tool` read 468 of 472 tools `inert` while Claude Code used them daily -- a measurement of ONE caller wearing the name of all callers. Attached at the ONE choke point every MCP server dispatches through (`base_server.py::_handle_tools_call`), on all three of its exits. Writes through the EXISTING `mcp_executor.record_dispatch_audit` -- no second `INSERT` into an append-only table -- with `caller_source = 'mcp_server:<name>'`, which is what `capability_consumption`'s `extra.by_caller_source` splits on. Arguments are digested, never stored. Bounded queue + one daemon thread so an unreachable database (measured: 20 s per synchronous call) cannot block a tool result; overflow is counted, never silent. `ICDEV_MCP_DISPATCH_AUDIT=0` stands it down and `initialize` says so. Library, no CLI: `record_tool_call`, `audit_status`, `stats`, `flush`. |
