...
| Tool | File | Description | Input | Output |
|------|------|-------------|-------|--------|
| Migration Versions | tools/db/migration_versions.py | Detects duplicate migration version numbers. `MigrationRunner` keeps only the FIRST migration per version, so same-version siblings are skipped permanently and silently — 71 migrations are currently shadowed this way, which is the root cause of a large share of the tables missing from live databases. Grandfather list: `args/migration_duplicate_versions.yaml`. The scan reads the filesystem, so a renamed migration leaves its old directory behind in every pre-existing checkout holding only `__pycache__`; such directories track no file in git and are excluded from the version scan and reported separately as stale local artifacts, never as collisions. Public API: `find_duplicates()`, `shadowed_migrations()`, `check()`, `load_allowlist()`, `stale_directories()`, `stale_directory_message()`. | `--json`, `--gate`, `--shadowed` | JSON `{duplicate_versions, new_violations, shadowed_count, stale_directories, passed}` |
