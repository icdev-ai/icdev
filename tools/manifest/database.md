...
| Tool | File | Description | Input | Output |
|------|------|-------------|-------|--------|
| Migration Versions | tools/db/migration_versions.py | Detects duplicate migration version numbers. `MigrationRunner` keeps only the FIRST migration per version, so same-version siblings are skipped permanently and silently — 60 migrations are currently shadowed this way. Grandfather list: `args/migration_duplicate_versions.yaml`, where **every entry must carry a reason** (`- <name>: "<why safe>"`); an unexplained entry fails the gate. Public API: `find_duplicates()`, `shadowed_migrations()`, `check()`, `load_allowlist()`, `load_allowlist_reasons()`, `unexplained_entries()`. | `--json`, `--gate`, `--shadowed` | JSON `{duplicate_versions, new_violations, unexplained_entries, shadowed_count, passed}` |
