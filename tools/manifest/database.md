...
| Migrate AlphaDESK | tools/db/migrate_alphadesk.py | Idempotent migration of all fathomdesk.db (SQLite) tables into the main ICDEV database (SQLite or PostgreSQL) via INSERT OR IGNORE; supports --source/--dest overrides | --json, --gate, --source, --dest | Migration results; exit 1 on --gate + failures |
