...
| CDP Session | tools/browser/cdp/session.py | cdp-port-03. Request/response correlation + event demux ABOVE the frame codec (§4.3): `send(method, params)` assigns an id, reads until the matching reply, **buffers/dispatches interleaving events** (never mistaken for the response), raises `CDPError` on a protocol error. Single-flight per page target. | (import) | `CDPSession.send()` → result dict; `add_listener`/`drain_events` |
