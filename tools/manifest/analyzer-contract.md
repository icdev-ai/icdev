...
| Binary Triage | tools/analyzers/binary_triage.py | Triages a compiled artifact (`binary` observable, `xrv-bin-01`) — format from magic bytes, architecture, SHA-256, PE/ELF section table with per-section Shannon entropy, imports, printable strings (ASCII **and** UTF-16LE) and version hints. Format, sections and entropy are pure-Python `struct`, so they answer on an air-gapped host; imports alone need `pefile`/`pyelftools` and read `None` with `library_unavailable` NAMED, never `[]`. Four statuses (`ok`/`truncated`/`unsupported_format`/`source_unreadable`) and `packed_hint` is `None` — never `False` — when no entropy was measured. Nothing is executed; posture `sandboxed` (sandbox-coverage Gap 71). | `<path>` [--json] [--max-bytes N] [--max-strings N] [--min-string-length N] | Triage report (format / sha256 + scope / sections / imports / strings / packed_hint / limits) |
| Ghidra Headless | tools/analyzers/ghidra_headless.py | Decompiles a `binary` observable with NSA Ghidra headless (`ghidra_decompile`, `xrv-bin-02`) — functions, imports, defined strings and the **entry point's C**, via a `-postScript` that writes JSON (`ghidra_scripts/icdev_export.py`, Jython, no third-party import). **OPTIONAL BACKEND**: Ghidra needs a JDK 21+ and a ~400 MB install, is NOT in `requirements.txt`, and its absence reports `unavailable` NAMING `ICDEV_GHIDRA_HOME` — lists read `None`, never `[]`, so "nobody looked" can never read as "the binary has no functions". Five statuses (`ok`/`truncated`/`unavailable`/`timeout`/`error`). Ghidra's `-analysisTimeoutPerFile` is set BELOW our wall budget so an over-run yields a labellable partial instead of a killed process; on the wall budget the **process tree** is killed via `reflexes/kanban._kill_process_tree` (imported, never re-implemented). Posture `sandboxed` (sandbox-coverage Gap 71) — and that image needs a JDK and Ghidra too. | `<path>` [--json] [--timeout N] [--max-cpu N] [--ghidra-home DIR] [--max-functions N] [--max-strings N] | Decompilation report (status / functions / imports / strings / entry_decompiled / ghidra_version / truncation / limits) |
