...
| Artifact Corroboration | tools/devsecops/artifact_corroboration.py | Does the ARTIFACT corroborate what was attested about it (`xrv-bin-03`)? Two axes never merged -- the artifact's own SHA-256 against the attestation SUBJECT's digest, and what the triage OBSERVED against what the SBOM DECLARES -- each `agrees`/`disagrees`/`unmeasurable`. Reported BESIDE a signature or SLSA-level verdict and never folded into it: `attach()` returns a NEW mapping with every existing key unchanged and REFUSES to overwrite. Only a version contradiction on a name BOTH sides carry reaches `disagrees`; a component the binary never mentions and a string the SBOM never declares are context. Consumed by `attestation_verify --artifact` and `slsa_verify --artifact`. | --artifact, --attestation, --sbom, --json | Corroboration block (digest axis + sbom axis + contradictions) |
