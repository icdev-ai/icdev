...
| CLI Bridge Indicator Include | tools/dashboard/templates/includes/cli_bridge_indicator.html | Navbar status pill — shows live CLI-bridge state (active/missing/off), env override, and last-served provider/model by polling `/api/cli-bridge/status`. Fixed bottom-left; ARIA-labeled; honors the per-page `icdev_cli_bridge` cookie | (Jinja2 include) | Rendered status pill |
| CLI Bridge Prompt Panel Include | tools/dashboard/templates/includes/cli_bridge_panel.html | Slide-out interactive prompt panel — text input (capped at 8000 chars), submit posts to `/api/cli-bridge/prompt` with per-request `force_bridge` from the page cookie, renders provider/model + duration; cancel-on-outside-click + Escape | (Jinja2 include) | Rendered slide-out panel |
