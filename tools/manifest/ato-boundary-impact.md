...
| Fabric Registry | tools/fabric/registry.py | N named fabrics, each carrying a classification LABEL from args/classification_profiles.yaml (a banner such as `CUI // SP-CTI` is REFUSED with the label it should have been), an impact level the profile admits, an authoritative inventory source (netbox/csv/discovery; a fixture or a drawing is never authoritative) and discovery adapters. The in-repo `args/fabric_registry.yaml` is schema + SYNTHETIC fixture and names no real fabric; real fabrics live in a private overlay OUTSIDE the repo named by `ICDEV_FABRIC_REGISTRY_PATH`, overriding per fabric key. `load_registry()` (the posture seam) EXCLUDES synthetic fabrics unless asked and reports how many. Cross-fabric traversal is declared separately; direction derives from icdev.core.sensitivity and a downward path without a named guard is refused. Required controls reuse crosswalk_engine.get_controls_for_impact_level (IL2 reads None, never 0). | `--json`, `--include-synthetic`, `--overlay <path>`, `--fabric <key>`, `--check`; library `load()`, `load_registry(include_synthetic=False)`, `required_controls(fabric)` | {fabrics[], traversals[], source, synthetic_excluded, reason} |
