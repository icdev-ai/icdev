...
| Test Reflex | tools/genesis/reflexes/test.py | Find untested modules → generate test stubs → run → keep passing. YELLOW tier. Writes to `tests/genesis_auto/`. **GENERATED-TEST POLICY (tsg-gen-02): a generated test asserts BEHAVIOUR, never the existence of a private name.** `assert hasattr(mod, "_THRESHOLD")` cannot fail in a way that names a defect and DOES fail on any legal rename; because the emitted guard catches `ImportError` but not `AssertionError` the failure is a hard error, not a skip — that is how 96 such assertions across 26 files became a standing source of red (removed in #1591 / tsg-gen-01, which touched 0 production files and kept all 77 assertions on PUBLIC constants). `_is_public_name` filters constants BEFORE the `_MAX_CONSTANTS_ASSERTED` cap so private names cannot eat the public budget; `tests/test_genesis_test_reflex_policy.py` pins it, without which the next generation run silently re-adds all 96. Widening the guard to `except (ImportError, AttributeError)` is INERT — `assert hasattr(...)` raises `AssertionError` and `hasattr` never raises. The kept-and-repaired counter-example is exa-refine-04's evidence gate in `test_reflexion_agent.py` (a refinement with no `lesson_learned` rows is written `rejected_no_evidence`, never `pending`; the code was right, the generated test was wrong). Mirror any change to `icdev/tools/genesis/reflexes/test.py`. Full rationale: `tests/genesis_auto/README.md` | config dict | Test results |
...
| Cache Warm Reflex | tools/genesis/reflexes/cache_warm.py | Refills the LLM response cache after it VANISHES. `llm_response_cache` is UNLOGGED on PostgreSQL, so a crash or unclean shutdown empties it and the savings tile reads zero until live router traffic refills it; `tools/cache_savings/warmer.py` existed to pre-populate it and nothing ever invoked it. Cost-disciplined: asks `get_savings_stats()` first and warms ONLY on a `cold` cache (zero rows) — a `populated` or `expired` cache is a no-op, because entries aging out through their TTL is the cache working. GREEN tier. Always returns a dict carrying `success`, since the daemon scores a missing key a permanent failure. (Registered here retroactively — the tool shipped without a manifest row.) | config dict | {success, warmed, state, seeds_used} |
