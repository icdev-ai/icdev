...
| `refinement_evidence` | `tools/workflow/refinement_evidence.py` | exa-refine-04: joins `lesson_learned` to proposed refinements. Collects the lesson rows for a proposal's trajectory (trace.task_id -> the `lesson_learned` memory entry) plus the per-pattern recurrence score from `lesson_learned.get_recurrence`, and writes the whole `refinement_evidence/v1` bundle into `agent_improvement_artifacts.evidence_traces` (previously a bare trace-id list, or the literal `'[]'` from SELA). `evaluate_evidence()` is the gate: a proposal with no supporting lesson rows is persisted with `status='rejected_no_evidence'` so it is never selected by GEPA nor shown in a review queue. `parse_evidence()` also reads the two legacy shapes. Config: `args/refinement_evidence.yaml`. | --task-type, --artifact-id, --skill, --window-days, --json |
...
| `pattern_confidence` | `tools/nova/skill_generator.py` | Confidence 0..1 on every learned pattern (xrv-shield-02), copying `tools/registry/learning_collector.py`'s `child_learned_behaviors.confidence REAL CHECK(0..1)` shape. ONE declared formula over occurrence count (saturating), DISTINCT `memory_entries.session_ref` values and recency (`args/memory_config.yaml` half-lives), then the collector's injection rule — a hit at `block_confidence` rejects to 0.0, at `demote_confidence` caps at 0.5. A pattern measured in exactly ONE session is capped below the propose bar: repetition is not corroboration. `confidence` is null (never 0.0) when nothing could be measured and `sessions` is null (never 0) when attribution is unavailable. `score_candidate()` is the screening seam `skills_lifecycle.screen_candidates` calls; `propose_min_confidence()` is the bar. Weights: `args/nova_config.yaml`. | --analyze (now prints confidence + basis), --no-injection-scan, --json | {patterns[].confidence, mean_confidence (null over an empty denominator), unmeasurable, propose_min_confidence, above_propose_threshold} |
