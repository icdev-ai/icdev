...
| OIDC Integration | tools/auth/oidc.py | OpenID Connect (OIDC) enterprise SSO integration using authlib. Manages per-provider OAuth2Session registry, generates OIDC login URLs with CSRF state (`get_oidc_login_url`), exchanges authorization codes for tokens and decodes id_token claims (`exchange_oidc_code`), applies configurable claims_mapping, and persists sessions to `sso_sessions`. | provider_id (str), code, state, redirect_uri | (authorization_url, state) tuple; user_info dict with mapped claims |
