...
| RMF Stage Recorder | tools/compliance/rmf_stage_recorder.py | The ONE writer of rmf_workflow_stages. Records an RMF step as a CONSEQUENCE of an artifact being produced (ssp/poam/stig/oscal/cATO evidence), so the lifecycle board cannot drift from the artifacts. Carries the clock boundary: started_at -> submitted_at is ours, submitted_at -> completed_at is the AO's | --project-id, --actor, --artifact / --submit / --decision, --evidence | Upserted stage row |
| RMF Cycle Time | tools/compliance/rmf_cycle_time.py | Reports automation_time (ours -- the 72h claim) and decision_latency (the AO's queue) with SEPARATE denominators and NO blended figure. Either is None when unmeasured; a lower-bound sample can never satisfy the target. baseline_source carries the declared baseline and an independently measured_here one, and refuses the comparison when the baseline includes the AO's queue | --json, --window-days, --project-id, --db | Two clocks + baseline provenance |
