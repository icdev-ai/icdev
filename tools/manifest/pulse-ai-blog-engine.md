...
| Publish Gate | tools/pulse/publish_gate.py | LLM-judge publish hard gate (nav-intel-09): RED verdict or judge-not-run blocks publish (fail closed); audited admin force override into append-only pulse_publish_audit (migration 281) | (library) — `check_publish_gate(post_id)`, `log_publish_override(post_id, actor, reason, verdict)` | Gate decision dict / audit row id |
