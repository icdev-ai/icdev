...
| `tools/document_intelligence/workspace.py` | dwr-ws-02. The server half of the two-pane workspace at `/document-intelligence/workspace/<doc_id>` -- the FIRST consumer of dwr-ws-01's `GET /api/change-set`, which shipped with none. `document_body(doc_id)` decides the left pane's rendering mode BY MEASUREMENT, never by preference: `positioned` (dic_sections rendered verbatim, so `content[start:end]` on screen is the span `verify_anchor` checks and accept splices -- the only mode in which a mark is truthful), `chunks` (NO dic_sections row exists; the text survives only as `rag_chunks`, and since `api_suggestion_accept` writes `dic_sections.content` and nothing else, the view is labelled NOT ADDRESSABLE -- measured 2026-09-08, `dic_doc_28e2ee4d984f3f35` carries 47 of the board's 58 pending proposals and zero sections, so a sections-only page would show an EMPTY document beside 47 proposals), `no_body` (both stores READ, neither holds anything) and `unmeasured` (a read failed) -- the last two never merged, with counts `None` and never `0`. `document_findings(version_id)` runs the two DOCUMENT-level publish gates through `consistency_checker.check_version_consistency` / `check_version_citations` (imported, never re-derived) because `placeholder_guard` and `citation_guard` anchor to no claim and a span-only page renders a blocked draft as clean -- the delta_review precedent. A GATE OVER AN EMPTY DENOMINATOR IS `unmeasured`, NEVER `clean`: the first live run drew two green ticks for a version with zero sections, so `section_count`/`ai_section_count` now guard it. `unplaced_summary()` is this page's own document-level finding -- proposals naming NO section, which no span-anchored rail can draw and which on this board is 58 of 58. `workspace_candidates()` backs the picker; `unmeasured` is never an empty list. Reads; writes nothing, decides nothing, applies nothing (pinned by an AST test) -- every act on the page goes through a door that already existed. | (import) | `dict{basis, sections[], chunks[], addressable, modes}` / `dict{placeholder, citation, numeric}` |
| `tools/document_intelligence/docx_revisions.py` | dwr-word-01. A .docx whose tracked changes are REAL Word revisions. python-docx 1.2.0 exposes NO revision API, so `w:ins` / `w:del` / `w:comment` are direct OOXML over its XML tree plus two package parts it does not know about (`comments.xml`, `commentsExtended.xml`). Reached as the `docx_tracked` format through `exporter.export_version`, so the placeholder -> citation -> WriteGuard gate still runs in order and still fails closed. WHAT WORD CALLS A REVISION IS AN *UNDECIDED* PROPOSAL AND THAT IS THE WHOLE MAPPING: only a `pending` suggestion becomes a `w:ins`/`w:del`; an `accepted` one is already spliced into `dic_sections.content`, so re-proposing it would offer a Word reviewer a Reject button over a decision already on the append-only chain, and it is reported as provenance instead. Positions are `review_rail`'s and are NEVER re-derived -- only a verified anchor is placed, and an item with no position is neither drawn at offset 0 nor dropped: it goes to a labelled appendix carrying its reason (`no_verified_anchor`, `not_pending`, `anchor_spans_paragraphs`, `anchor_overlap`, `anchor_text_mismatch`, `diff_not_round_trip`, `unplaced`). On the live board that is 58 of 58 suggestions. Revisions are WORD-level via `word_diff.diff_words` (FIPS 140-2 -> 140-3 deletes `2` and inserts `3`, not the clause), and a diff that fails its round trip is deferred rather than rendered lossily. `paragraph_spans` partitions a section on `\n` keeping ABSOLUTE offsets so `"\n".join(texts) == content` for every input -- asserted per section, because an anchor offset indexes the section content and this format therefore does NOT re-render markdown (offset fidelity and markdown reflow cannot both be had). Counts are `None` and never `0` for an `unmeasurable` rail. | (import) | `dict{path, rail_state, tracked_changes, revision_elements, comments, deferred[]}` |
| `tools/document_intelligence/docx_word_probe.py` | dwr-word-01. Asks WORD what is in the file, not the XML. A `w:del` wrapping a `w:t`, or a `commentsExtended` declaring `w15` as `.../office/2012/wordml` instead of `.../office/word/2012/wordml`, is well-formed XML that passes every schema-shaped assertion and that Word refuses outright -- both happened while `docx_revisions` was being written -- so the acceptance evidence is `Document.Revisions.Count`, `Document.Comments.Count` and `Comment.Ancestor` (Word's own word for "this is a reply in a thread", the only proof `commentsExtended` linked). THREE verdicts: `verified` \| `refused` (Word was there and said no -- the finding) \| `unmeasurable` (no Word, no COM bridge, file absent -- NOT a clean bill of health). `Document.Repaired` is not exposed by Word 16.0, so `repaired` is `None` with `repaired_basis: not_exposed_by_object_model` and that None means "cannot tell", never "was not repaired"; the silent-repair signature is a `verified` probe reporting ZERO against a build that placed revisions, so the two reports are read together. Windows-only by construction and deliberately not in CI (a runner has no Word); `tests/document_intelligence/test_docx_revisions.py` is the CI half. Report only, opens READ-ONLY, writes nothing. Exit 0 verified / 1 refused / 2 could not measure. | `python -m tools.document_intelligence.docx_word_probe <file.docx> [--json]` | `dict{verdict, revisions, comments, replies, detail[]}` |
| `tools/db/migrations/20260909004511_dic_artifacts_format_check_from_constant/up.py` | dwr-word-01: rebuilds `dic_artifacts.format`'s CHECK from `exporter.EXPORT_FORMATS`, which now names `docx_tracked`. 20260903194350 rendered an INLINE four-name CHECK from the constant AT THAT TIME, so a fifth format makes it the stale copy and a tracked export would render, pass every gate, and then be REFUSED on the INSERT -- an artifact on disk the record does not know about. The clause is GENERATED (the 20260903100336 precedent), never respelled. BOTH backends: PG drops and re-adds a named constraint; SQLite, which cannot ALTER a CHECK and unlike the precedent HAS the constraint wherever the original migration ran, rebuilds the table -- but ONLY when the live DDL is measurably stale, so a table with no CHECK (what `exporter._ensure_schema` creates) is left alone. The new DDL is derived from `exporter._SCHEMA` with the CHECK injected, so the columns cannot drift from the runtime CREATE. |
| `tools/document_intelligence/docx_review_import.py` | dwr-word-02. Reads a reviewer's Word revisions back in and RECONCILES them against the ICDEV change set -- the return leg of dwr-word-01. Parses `w:ins` / `w:del` / `w:comment` (stdlib zip + ElementTree; no python-docx, which exposes no revision API either way), folding each paragraph into `before` (equal + delete, i.e. the paragraph with every revision REJECTED -- what was in the .docx when it left here, and the only coordinate space an ICDEV anchor means anything in) and `after` (equal + insert). A `w:del` NESTED in a `w:ins` is in NEITHER side. THREE VERDICTS AND NO FOURTH: `matched` (located to exactly one span of one section and uncontested; `origin` says `own_proposal` or `reviewer_edit` -- both matched, never merged), `conflicting` (`base_paragraph_changed` \| `competing_proposal` \| `decided_since_export` -- surfaced with BOTH sides carried whole, never auto-merged and never won by one side), `unmatched` (reported BY NAME with its reason, never dropped). ABSENCE IS NOT A DECISION: an ICDEV item with no counterpart lands under `absent_from_upload`, because one absence is consistent with accept, reject, never-reached, paragraph-deleted and wrong-document, and four of those are not decisions. Location uses `docx_revisions.paragraph_spans` IMPORTED and the identity of one of our own redlines is re-derived through `word_diff.diff_words` -- the exporter emits a WORD-level diff, so comparing a returned revision against a suggestion's own columns matches nothing and every one of our redlines reports as a rival to itself. An `unmeasurable` rail makes the WHOLE report unmeasurable with `None` counts, because `matched` is a claim about the rail. Untrusted input is bounded: a declared DOCTYPE is refused unparsed, a part over `ICDEV_DOCX_IMPORT_MAX_PART_BYTES` is refused on its DECLARED size before decompression, only three parts are ever read, and a hit bound reports `truncated`. Report only -- no INSERT/UPDATE/DELETE and no store writer, pinned by AST. Round trip measured through Word 16.0: `docs/audits/dwr-word-02-round-trip.md`. | `python -m tools.document_intelligence.docx_review_import --file <docx> --version <version_id> [--json]` / `--parse-only` | `dict{state, revisions[], comments[], absent_from_upload[], counts}` |
| `tools/document_intelligence/collab.py` | dwr-collab-01. The document-scoped change-state feed behind `GET /document-intelligence/api/workspace/poll` -- what lets a SECOND reviewer's open workspace learn what a first reviewer decided, without a reload and without an SSE stream holding a synchronous WSGI worker open per reader (D103's transport; deliberately NOT `/api/events/poll`, which is bound to `hook_events` and has no notion of a document). `poll_document(doc_id, cursor, me=)` returns deltas since *cursor* plus who else is present. THE CURSOR IS A UTC ISO-8601 STAMP COMPARED AS TEXT, sound only because every writer on this canvas stamps `datetime.now(timezone.utc).isoformat()` (pinned by test); a cursor with any other offset is REFUSED (`cursor_not_utc_iso8601`) rather than compared, because `09:00+05:00` sorts after `09:00+00:00` while being four hours earlier. It is INCLUSIVE (`>=`): a strict `>` loses a row stamped in the cursor's own microsecond, and re-delivering costs a suppressed redraw while missing costs a decision nobody learns about. THE CURSOR NEVER ADVANCES ON A FAILED READ -- an `unmeasured` poll echoes it back unchanged, or the window nobody looked at is skipped for good and the next poll reports a quiet document. FOUR states: `baseline` (no cursor -- a window was never defined, so nothing is asserted about one) | `no_changes` (the measurement) | `changes` | `unmeasured`, never folded together; counts `None`, never `0`, when unmeasured. TWO FEEDS WITH THEIR OWN VERDICTS: `dic_suggestions.updated_at` for proposals and `dic_edit_history.edited_at` for content -- NOT `dic_sections`, which carries no `updated_at` at all and whose writers stamp `created_at` when they change content. A truncated page sets the cursor to the LAST DELIVERED ROW, never to now. Presence comes from `presence_registry.get_present_users` IMPORTED (AST-pinned), and `alone` is a MEASURED verdict an unreadable registry may never report. Reads; writes nothing, decides nothing, and does NOT heartbeat -- the refusal of a stale decision lives at the door (`suggestion_store.decide_outcome`'s conditional UPDATE), because a poll can only shorten the window, never close it. | (import) / `GET /api/workspace/poll?doc_id=&cursor=` | `dict{state, cursor, changes[], sections[], presence}` |
