...
| `tools/document_intelligence/modernization_routes.py` | Document Modernization routes (docmod-ux-02), attached to `dic_bp` via a one-import line at the bottom of `blueprint.py`. `POST /api/ingest/batch` accepts multipart `files` (repeated), saves to a per-job temp dir, runs `ingest_orchestrator.ingest_batch` on a daemon thread, and streams per-file progress (`{stage:"batch_file", file, of, filename, status, pct}`) over the existing single-ingest SSE endpoint `/api/ingest/<job_id>/stream`. Drives the bulk-upload UI on the DIC index + collections pages and the post-ingest "Run a freshness scan" CTA. |
