...
| Tool | Purpose |
|------|---------|
| `tools/document_intelligence/extractors.py` | Built-in air-gap-safe file extractors. Returns `Extraction(text, provider, content_type, page_count, title, metadata, warnings)`. Supports PDF (pypdf), DOCX (python-docx), XLSX (openpyxl), PPTX (python-pptx), PNG (pytesseract/easyocr), HTML (strip-html), TXT (plain read). All formats degrade gracefully — missing library yields `text=""` + warning rather than raising. Called by `ingest_orchestrator.py` before chunking. Optional **extraction-quality anomaly detection** (`detect_extraction_anomalies` / `extract_batch_with_quality`, aiify-opp-6059 hardcoded_threshold→anomaly_detection) lifts the implicit "min text length" cutoff a record validator would hardcode into named per-page text-yield bands (`_classify_yield` rich/sparse/empty) and adds a yield-distribution outlier pass over a batch that flags a silent extraction/OCR failure (a structurally-valid record — real page_count, no library error — that yielded near-empty text) or a yield cliff (near-empty tail of low statistical outliers), with optional best-effort LLM severity grading via `dic_extraction_anomaly_severity`; the statistical detector is authoritative/air-gap-safe and never changes what was extracted. Optional **OCR confidence anomaly detection** (`detect_ocr_confidence_anomalies` / `ocr_image_with_quality`, aiify-opp-6105 hardcoded_threshold→anomaly_detection) lifts the implicit "min word confidence" cutoff a tesseract parser would hardcode into named per-word confidence bands (`_classify_ocr_confidence` confident/uncertain/garbled) and adds a confidence-distribution outlier pass over a page's words that flags a confidence cliff (a low-confidence/garbled pocket of statistical outliers the absolute cutoff never surfaces relative to the rest of the page); `_extract_word_confidences` captures per-word confidence via `pytesseract.image_to_data` in the same pass that reconstructs the text, with optional best-effort LLM severity grading via `dic_ocr_confidence_anomaly_severity`; the statistical detector is authoritative/air-gap-safe and never changes which words were recognised. |
| `tools/document_intelligence/ingest_orchestrator.py` | Route a file → provider (by extension) → extract → REUSE `icdev.tools.rag.chunker.chunk_content` + `IngestionManager.ingest_source` to chunk/embed/upsert into the vector store → bridge each chunk into the KG via `rag_to_kg_ingester.ingest_chunk_to_kg` → write `dic_documents` + initial `dic_versions(origin='human_authored', status='approved')` + `dic_chunk_links` (rag chunk → doc + page/section). Stamps `tenant_id`/`classification` from the caller's security context on every row. Best-effort, HITL-gated AI enrichments are surfaced on `IngestOutcome.metadata` and never silently persisted: title/abstract (6098), OCR cleanup (6118), structured metadata (6086), identifiers (5988), taxonomy classification (6043), correspondence fields (6100), and **date-anomaly detection** (`assess_document_dates`, aiify-opp-6048 hardcoded_threshold→anomaly_detection — deterministically parses every calendar date and flags future-dated / implausibly-old / cluster-outlier dates under named thresholds, with optional LLM severity grading via `dic_date_anomaly_assessment`). |
...
| Tool | Purpose |
|------|---------|
| `tools/document_intelligence/search_engine.py` | DIC Grounded Search Engine. Default mode: BM25 + KG traversal (NO LLM, air-gap safe). Optional hybrid mode adds vector similarity + RRF fusion + cross-encoder rerank. Every result carries a mandatory citation pack; results with no traceable source are suppressed. `DICSearchEngine.search(query, collection_id, top_k, mode)` returns `list[DICSearchResult]` each with a `Citation` (doc_id, title, version_id, page, section, chunk_id). Falls back to pure SQL BM25 (`rag_chunks`) when the vector store is unavailable. Optional **search-relevance anomaly detection** (`detect_search_anomalies` / `DICSearchEngine.search_with_quality`, aiify-opp-6052 hardcoded_threshold→anomaly_detection) lifts the implicit relevance cutoff into named bands (`_classify_relevance` strong/moderate/weak) and adds a score-distribution outlier pass that flags a low-confidence retrieval (top hit itself weak) or a relevance cliff (noise tail of low statistical outliers), with optional best-effort LLM severity grading via `dic_search_anomaly_severity`; the statistical detector is authoritative/air-gap-safe and never changes which results `search()` returns. |
