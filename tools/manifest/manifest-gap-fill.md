...
| AlphaDesk Snapshot Builder | tools/trading/market_intel/snapshot_builder.py | AlphaDesk — materializes `ad_market_snapshot` and `ad_ticker_performance` tables so `/api/market/latest` and `/api/market/performance` read from cache instead of recomputing. Refreshed by the `snapshot_refresher` reflex. | `--build-snapshot`, `--build-performance`, `--json` | Row counts + timing |
