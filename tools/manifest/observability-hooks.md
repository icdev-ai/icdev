...
| Runtime Trace CLI | tools/cli/runtime.py::_trace | `icdev runtime trace <correlation-id>` — every span of ONE agent run, oldest first: the `agent.turn` spans the loop emits and the `gen_ai.invoke` spans the router emits beneath them. Reads through `agent_trace.spans_for_correlation`; authors no SQL of its own. | correlation_id, --limit, --json | Aligned span table or JSON |
| Agent Trace | tools/observability/agent_trace.py | The SAG run seam (hgx-obs-01): `correlation_scope`/`current_correlation_id` (contextvar the recorder reads), `submit_with_context` (carries it into pool workers, which start with an EMPTY context), `TurnTracer` (a span per turn with an explicit begin/finish lifecycle), and `spans_for_correlation` (the join). Never raises. | correlation_id, turn, executor | Spans in `otel_spans`; span dicts on read |
...
| agent | `tools/agent_runtime/dispatch.py::make_handler` | every SAG tool call — built-in, decorated and MCP-registry alike. `execute_agent` covers a whole agent run; this covers the individual tool calls inside a standalone-agent-runtime loop, which never touch the MCP server. Blocked and failed calls are recorded too, with status `error`. |
...
| Table | Purpose |
|-------|---------|
| runtime_invocations | What actually ran: surface, name, duration, status, error class, arg keys, and `correlation_id` (migration 20260808161052) joining a row to the run that issued it. Telemetry, NOT append-only audit evidence — deliberately absent from APPEND_ONLY_TABLES. Its `arg_values`/`result_preview` columns stay NULL unless `ICDEV_OBS_REPLAY` is explicitly enabled; the default stores KEY NAMES only. |
