            ...
            violations.append(
                {
                    ...
                    "severity": "high",
                    "action": "flag",
                    "description": f"Output size ({len(output_text.encode('utf-8')):,} bytes) exceeds limit ({self._max_size:,} bytes)",  # noqa: E501
                }
            )
    ...
    def evaluate_gate(self, project_id: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            for severity in ("critical", "high", "medium"):
                row = conn.execute(
                    f"SELECT COUNT(*) FROM agent_output_violations WHERE {where} AND severity = ? AND created_at >= ?",
                ...
                if severity == "critical":
                    ...
                    if count > 0:
                        ...
                        result["blocking"].append(
                            f"{count} critical output violation(s) in last 24h (classification leak or credential exposure)"  # noqa: E501
                        )
                elif severity == "high":
                    if count > 5:
