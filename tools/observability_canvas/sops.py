...
def get_all_sops(sop_type=None, approval_status=None):
    ...
    if sop_type and approval_status:
        sql = "SELECT * FROM odc_sops WHERE sop_type = ? AND approval_status = ? ORDER BY updated_at DESC"
        params = [sop_type, approval_status]
    elif sop_type:
        sql = "SELECT * FROM odc_sops WHERE sop_type = ? ORDER BY updated_at DESC"
        params = [sop_type]
    elif approval_status:
        sql = "SELECT * FROM odc_sops WHERE approval_status = ? ORDER BY updated_at DESC"
...
def get_sop_by_id(sop_id):
    ...
    with _get_conn() as conn:
        row = conn.execute("SELECT * FROM odc_sops WHERE id=?", (sop_id,)).fetchone()
...
def delete_sop(sop_id):
    ...
    with _get_conn() as conn:
        cur = conn.execute("DELETE FROM odc_sops WHERE id=?", (sop_id,))
        conn.commit()
...
def submit_for_review(sop_id):
    ...
    with _get_conn() as conn:
        conn.execute(
            "UPDATE odc_sops SET approval_status='pending_review', updated_at=? WHERE id=?",
            (now, sop_id),
        )
        conn.commit()
