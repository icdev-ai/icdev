...
# Thread selection
# ---------------------------------------------------------------------------
def _annotation_columns(conn) -> set:
    """What ``dic_section_annotations`` actually carries on THIS database.

    Probed from the catalogue rather than discovered by running a SELECT and
    catching the failure: a swallowed query error is indistinguishable from an
    empty thread, and an empty thread is a refusal this module makes by name.
    dwr-cmt-01 adds the anchor offsets; until it lands the probe simply reports
    they are absent and the selection says `section_scope`.
    """
    ...
    try:
        ...
        logger.warning("redraft: annotation catalogue unreadable (%s)", exc)
        return set()
...
def thread_for_change(suggestion: dict, conn=None, cfg: dict | None = None) -> ThreadSelection:
    """The comment thread that belongs to ONE change.

    Selection is by anchor overlap where the annotations carry a span
    (dwr-cmt-01) and by section otherwise, and the basis is RECORDED either
    way. A caller must be able to tell "these are the comments on this exact
    span" from "these are the comments anywhere in this section", because only
    the first is what the button's label promises.
    """
    ...
    try:
        ...
        cols = _annotation_columns(conn)
        if not cols:
            # No such table on this database -- nobody can have commented.
            return ThreadSelection(basis="section_scope")

        try:
            cur = conn.execute(
                "SELECT * FROM dic_section_annotations "
                "WHERE section_id = %s AND status = %s ORDER BY created_at ASC",
                (section_id, "open"),
            )
            names = [d[0] for d in cur.description]
            rows = [dict(zip(names, r)) for r in cur.fetchall()]
        except Exception as exc:  # noqa: BLE001
            logger.warning("redraft: annotation read failed (%s)", exc)
            return ThreadSelection(basis="section_scope")

        basis = "section_scope"
        anchored = {"anchor_start", "anchor_end"} <= cols
        if anchored and suggestion.get("anchor_start") is not None:
            scoped = [
                r for r in rows
                if _overlaps(r.get("anchor_start"), r.get("anchor_end"),
                             suggestion.get("anchor_start"), suggestion.get("anchor_end"))
            ]
            # An annotation with no span of its own is a comment on the SECTION
            # and is kept: dropping it would make a reviewer's instruction
            # vanish because of when they wrote it.
            unspanned = [r for r in rows if r.get("anchor_start") is None]
            rows = sorted(scoped + unspanned, key=lambda r: str(r.get("created_at") or ""))
            basis = "anchor_overlap"
        ...
        for r in rows:
            ...
            label = f"{author}" + (f" ({category})" if category else "")
...
def redraft_change(suggestion_id: str, actor: str, *,
    ...
    try:
        superseded = supersede_suggestion(
            suggestion_id, drafted.suggestion_id,
            superseded_reason=f"redraft requested by {actor}",
        )
    except Exception as exc:  # noqa: BLE001
