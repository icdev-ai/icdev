...
def run_integration_tests(
    ...
    checks.append({
        ...
        "passed": connect_ok,
        "detail": f"Connect: {connect_event.get('status', 'not attempted')}" if connect_event else "Connect not attempted",
    })
    ...
    checks.append({
        ...
        "passed": health_ok,
        "detail": f"Health: {health_event.get('status', 'not attempted')}" if health_event else "Health not attempted",
    })
    ...
    checks.append({
        ...
        "passed": tables_ok,
        "detail": f"Tables: {tables_event.get('count', '?')} found" if tables_event else "list_tables not attempted",
    })
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO db_forge_validations
               ...
               VALUES (?, 'integration', ?, ?, ?)""",
            (connector_id, 1 if passed else 0, details[:2000],
