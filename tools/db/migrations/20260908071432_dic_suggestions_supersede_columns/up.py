...
----------------------
``successor_suggestion_id``  the ``suggestion_id`` that REPLACED this row, when
there is one. dwr-anchor-05 supersedes a row whose ANCHOR went stale and there
is no successor in that case, so the column is NULLABLE and NULL means NOT
RECORDED -- a default would make a row nothing ever replaced read as one that
was. A redraft does produce a successor, and ``superseded`` with no successor
is a dead end a reader cannot follow: it makes a retired change read exactly
like a lost one.

It is a COLUMN rather than more prose in the decision row's ``note`` because a
reader FOLLOWING the chain needs an id, not a sentence. It is NOT named
``superseded_by``: ``supersede_suggestion`` already takes a ``superseded_by``
naming the MECHANISM that retired the row, which lands in
...
THE DECISION ROW IS STILL WRITTEN, on dwr-anchor-05's terms: ``decision`` stays
``superseded`` -- a value ``decide_suggestion`` refuses -- and ``decided_by``
names the mechanism, so a retirement can never be read as a person's
accept-or-reject verdict in the table cef-ui-03 reads to answer "was this
reviewed?". The audit record of WHO asked for a redraft lives in
``audit_trail`` under ``dic.redraft`` (migration 20260908071433).
