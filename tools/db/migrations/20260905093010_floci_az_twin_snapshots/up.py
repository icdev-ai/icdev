#!/usr/bin/env python3
# CUI // SP-CTI
"""Migration 20260905093010: create ``floci_az_twin_snapshots`` (flx-az-01).
...
``idc_twin_snapshots``, ``qdc_twin_snapshots``, ...). The floci-az adapter joins
that convention -- a SEPARATE table from floci_twin_snapshots, because the two
emulators are different estates and merging them would make a query for "the
AWS estate" silently include Azure rows.
...
_TABLE = "floci_az_twin_snapshots"
...
def _ddl(backend: str) -> str:
    ...
    return f"""
CREATE TABLE IF NOT EXISTS floci_az_twin_snapshots (
    id              TEXT    NOT NULL,
...
_IDX_TARGET = (
    f"CREATE INDEX IF NOT EXISTS idx_floci_az_twin_snap_target ON {_TABLE}(target_id)"
)
_IDX_CREATED = (
    f"CREATE INDEX IF NOT EXISTS idx_floci_az_twin_snap_created ON {_TABLE}(created_at)"
)
...
def up(conn=None) -> None:
    ...
    try:
        ...
        conn.commit()
        print(f"[20260905093010_floci_az_twin_snapshots] up: {_TABLE} created (or already exists)")
    finally:
        if owned:
            conn.close()
