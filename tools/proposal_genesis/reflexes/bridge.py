...
def _generate_transition_doc(opp: Dict, themes: List[Dict],
                              partners: List[Dict], cost: Optional[Dict],
                              compliance: Dict) -> str:
    ...
    lines = [
        ...
        f"**Award Amount:** ${opp.get('award_amount', 0):,.2f}"
        if opp.get("award_amount") else "**Award Amount:** TBD",
        f"**Generated:** {now}",
        "",
        "---",
        "",
    ...
    if cost:
        lines.append(f"- **Total Price:** ${cost.get('total_price', 0):,.2f}"
                      if cost.get("total_price") else "- **Total Price:** TBD")
