...
def _detect_new_chunks(
    conn: sqlite3.Connection, source_type: str, since: Optional[str] = None,
    limit: int = 100,
) -> List[Dict[str, Any]]:
    ...
    try:
        rows = conn.execute(
            """SELECT id, content, source_id, source_table, created_at
               FROM rag_chunks
               WHERE source_table = ? AND created_at > ? AND tier = 'hot'
...
def _generate_pairs(
    chunks: List[Dict[str, Any]], dataset_id: str, purpose: str = "general",
    ...
    try:
        ...
        chunk_data = [
            {"content": c.get("content", ""), "chunk_id": c.get("id", "")}
...
def _auto_approve_pairs(
    conn: sqlite3.Connection, dataset_id: str, threshold: float = 0.8,
) -> int:
    """Auto-approve unapproved pairs with quality_score >= threshold."""
    try:
        cursor = conn.execute(
            """UPDATE ft_dataset_examples
               SET approved = 1, labeled_by = 'rag_ft_pipeline', labeled_at = ?
               WHERE dataset_id = ? AND approved = 0 AND quality_score >= ?""",
            (_now(), dataset_id, threshold),
        )
        conn.commit()
...
def _ensure_dataset(conn: sqlite3.Connection, name: str, purpose: str) -> str:
    ...
    row = conn.execute(
        "SELECT id FROM ft_datasets WHERE name = ? AND status IN ('draft', 'labeling', 'ready')",
        (name,),
    ).fetchone()
    if row:
        return row["id"]
    ...
    try:
        ...
        conn.execute(
            ...
            "VALUES (?, ?, ?, 'Auto-generated by RAG-FT pipeline', 'labeling', ?)",
            (ds_id, name, purpose, _now()),
        )
        conn.commit()
        return ds_id
...
def run_pipeline(
    ...
    for src in source_types:
        ...
        try:
            ...
            if dry_run:
                conn.execute(
                    "UPDATE ft_pipeline_runs SET chunks_processed=?, status='dry_run', completed_at=? WHERE id=?",
                    (len(chunks), _now(), run_id),
                )
                conn.commit()
                results.append({
                    "source_type": src, "run_id": run_id,
                    "chunks_found": len(chunks), "dry_run": True,
                })
                continue

            if not chunks:
                conn.execute(
                    "UPDATE ft_pipeline_runs SET status='completed', completed_at=? WHERE id=?",
                    (_now(), run_id),
                )
                conn.commit()
                results.append({
                    "source_type": src, "run_id": run_id,
                    "chunks_found": 0, "message": "No new chunks",
                })
                continue
            ...
            conn.execute(
                """UPDATE ft_pipeline_runs
                   SET chunks_processed=?, pairs_generated=?, pairs_approved=?,
                       retrain_triggered=?, status='completed', completed_at=?
                   WHERE id=?""",
                (len(chunks), pairs_generated, approved,
            ...
            results.append({
                "source_type": src, "run_id": run_id,
                "chunks_found": len(chunks), "pairs_generated": pairs_generated,
            ...
            conn.execute(
                "UPDATE ft_pipeline_runs SET status='failed', error=?, completed_at=? WHERE id=?",
                (str(exc), _now(), run_id),
            )
            conn.commit()
            results.append({
...
def get_pipeline_status() -> Dict[str, Any]:
    ...
    try:
        ...
        successful = conn.execute(
            "SELECT COUNT(*) FROM ft_pipeline_runs WHERE status='completed'"
        ).fetchone()[0]
