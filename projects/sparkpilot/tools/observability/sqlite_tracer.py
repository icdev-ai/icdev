...
Writes spans to the `otel_spans` table in sparkpilot.db.
...
DEFAULT_DB_PATH = BASE_DIR / "data" / "sparkpilot.db"
...
class SQLiteTracer(Tracer):
    ...
    Writes completed spans to the `otel_spans` table in sparkpilot.db.
    ...
    def start_span(
        ...
        attrs = {}
        if self._agent_id:
            attrs["sparkpilot.agent_id"] = self._agent_id
        if self._project_id:
            attrs["sparkpilot.project_id"] = self._project_id
        if attributes:
                ...
                try:
                    conn.execute(
                        ...
                        (
                            ...
                            span.attributes.get("sparkpilot.agent_id", self._agent_id),
                            span.attributes.get("sparkpilot.project_id", self._project_id),
