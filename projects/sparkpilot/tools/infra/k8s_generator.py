...
def generate_deployment(project_path: str, app_config: dict = None) -> list:
    ...
    name = config.get("name", "sparkpilot-app")
    ...
    deployment = {
        ...
        "metadata": {
            ...
            "annotations": {
                "classification": "CUI",
                "sparkpilot.io/generated": datetime.now(timezone.utc).isoformat(),
            },
        },
        "spec": {
            "replicas": replicas,
...
# ---------------------------------------------------------------------------
# Default agent definitions matching SPARKPILOT multi-agent architecture
DEFAULT_AGENTS = [
...
def generate_agent_deployments(project_path: str, blueprint: dict = None) -> list:
    ...
    Creates a Deployment and Service for each agent in the SPARKPILOT multi-agent
    ...
    namespace = config.get("namespace", "sparkpilot-agents")
    registry = config.get("registry", "registry.example.com/sparkpilot")

    k8s_dir = Path(project_path) / "k8s" / "agents"
    files = []

    for agent in agents:
        agent_name = f"sparkpilot-{agent['name']}"
        port = agent["port"]
        ...
        deployment = {
            ...
            "metadata": {
                ...
                "labels": _labels(agent_name, {
                    ...
                    "sparkpilot.io/tier": "agent",
                }),
                "annotations": {
                    "classification": "CUI",
                    "sparkpilot.io/generated": datetime.now(timezone.utc).isoformat(),
                    "sparkpilot.io/agent-port": str(port),
                },
            },
            "spec": {
                "replicas": 2,
    ...
    for agent in agents:
        ...
        hpa = {
            ...
            "metadata": {
                ...
                "namespace": namespace,
                "labels": _labels(agent_name, {
                    "sparkpilot.io/scaling-tier": "core" if name_key in core_agents
                    else "domain" if name_key in domain_agents
                    else "support",
                }),
                "annotations": {
                    "classification": "CUI",
                    "sparkpilot.io/scaling-profile": "predictive",
                },
            },
            "spec": {
                "scaleTargetRef": {
...
def main():
    ...
    parser.add_argument("--name", default="sparkpilot-app", help="Application name")
