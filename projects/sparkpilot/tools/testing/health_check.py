# [TEMPLATE: CUI // SP-CTI]
# SPARKPILOT System Health Check
# Adapted from ADW health_check.py for Gov/DoD environment validation

"""
SPARKPILOT Health Check — validates the entire SPARKPILOT system is operational.

Usage:
    python tools/testing/health_check.py [--json] [--project-id <id>]

Checks performed:
1. Environment variables (SPARKPILOT, AWS, optional keys)
2. Database connectivity (sparkpilot.db — 28 tables)
...
def check_env_vars() -> CheckResult:
    ...
    required_vars = {
        "ICDEV_DB_PATH": "Path to SPARKPILOT database (default: data/sparkpilot.db)",
    }
    ...
    for var, desc in required_vars.items():
        ...
        if not val:
            ...
            if var == "ICDEV_DB_PATH":
                default_path = PROJECT_ROOT / "data" / "sparkpilot.db"
...
def check_database() -> CheckResult:
    """Check SPARKPILOT database connectivity and table structure."""
    db_path = os.getenv("ICDEV_DB_PATH", str(PROJECT_ROOT / "data" / "sparkpilot.db"))
...
def check_tools() -> CheckResult:
    """Check that core SPARKPILOT tool modules are importable."""
    tool_modules = {
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="SPARKPILOT System Health Check")
