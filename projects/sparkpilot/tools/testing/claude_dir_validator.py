...
# CUI // SP-CTI
"""SPARKPILOT .claude Directory Governance Validator.

Cross-references .claude directory configuration (hooks, settings, commands,
E2E specs) against the SPARKPILOT codebase to detect drift. Ensures append-only
...
# Tools where --project refers to something other than SPARKPILOT project ID
...
def check_db_path_centralization(
    ...
    hardcoded_pattern = re.compile(
        r'''(?:Path\s*\([^)]*\)\s*(?:/\s*["']data["']\s*){1,2}/\s*["'](?:sparkpilot|memory|platform)\.db["'])'''
        r'''|(?:["']\S*data[/\\](?:sparkpilot|memory|platform)\.db["'])''',
    )
    ...
    for py_file in sorted(tools_dir.rglob("*.py")):
        ...
        try:
            ...
            continue
        if "sparkpilot.db" not in content and "memory.db" not in content and "platform.db" not in content:
            continue
        checked_count += 1
...
def format_human(report: ClaudeConfigReport) -> str:
    ...
    lines = []
    lines.append("=" * 60)
    lines.append("  SPARKPILOT .claude Directory Governance Report")
    lines.append("=" * 60)
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate .claude directory alignment with SPARKPILOT codebase"
    )
