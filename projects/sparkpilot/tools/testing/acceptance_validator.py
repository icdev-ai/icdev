# [TEMPLATE: CUI // SP-CTI]
# SPARKPILOT Acceptance Criteria Validator (V&V)
...
Usage:
    python tools/testing/acceptance_validator.py \
        --plan specs/issue-3-sparkpilot-abc-plan.md \
...
def check_page(base_url: str, page_path: str, timeout: int = 10) -> UIPageCheckResult:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "SPARKPILOT-AcceptanceValidator/1.0"})
...
def main():
    parser = argparse.ArgumentParser(
        description="SPARKPILOT Acceptance Criteria Validator (V&V)"
    )
    parser.add_argument(
        "--plan",
