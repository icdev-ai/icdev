# [TEMPLATE: CUI // SP-CTI]
# SPARKPILOT Event Router — central routing with lane-aware session queue (D133)
...
class EventRouter:
    ...
    def route(self, envelope: EventEnvelope) -> dict:
        ...
        if not workflow:
            # Check if content contains "sparkpilot" keyword (generic trigger)
            if "sparkpilot" in envelope.content.lower():
