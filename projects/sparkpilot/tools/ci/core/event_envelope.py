# [TEMPLATE: CUI // SP-CTI]
# SPARKPILOT Event Envelope — unified event format for all CI/CD triggers (D132)
...
class EventEnvelope:
    ...
    def _extract_command(text: str) -> tuple:
        """Extract sparkpilot workflow command and run_id from text.
    ...
    def _check_bot(text: str, author: str = "") -> bool:
        ...
        Handles Unicode variance in trademark/special symbols so bot
        detection works regardless of encoding artifacts.
        """
        import re
        if BOT_IDENTIFIER in text:
            return True
        if "[SPARKPILOT-BOT]" in text.upper():
            return True
        if author:
            clean = re.sub(r"[^a-zA-Z0-9-]", "", author).lower()
            if clean in ("sparkpilot-bot", "sparkpilot"):
                return True
        return False
    ...
    def from_gitlab_tag(cls, issue_data: dict, icdev_tag: str) -> "EventEnvelope":
        """Create envelope from a GitLab {{sparkpilot: tag}} issue."""
