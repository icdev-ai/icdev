# [TEMPLATE: CUI // SP-CTI]
"""GitLab Task Board Monitor — polls issues for {{sparkpilot: workflow}} tags.

Decision D33: GitLab tags replace Notion's {{prototype: type}} pattern.

Supported tags:
    {{sparkpilot: intake}}     -> Start RICOAS intake session
    {{sparkpilot: build}}      -> Run TDD build workflow
    {{sparkpilot: sdlc}}       -> Full Plan->Build->Test->Review
    {{sparkpilot: comply}}     -> Generate compliance artifacts
    {{sparkpilot: secure}}     -> Run security scanning
    {{sparkpilot: modernize}}  -> Legacy app modernization
    {{sparkpilot: deploy}}     -> Generate IaC and deploy
    {{sparkpilot: maintain}}   -> Maintenance audit
...
TAG_PATTERN = re.compile(r"\{\{sparkpilot:\s*(\w+)\}\}", re.IGNORECASE)

# Bot marker to prevent loops
BOT_MARKER = "[SPARKPILOT-BOT]"


def extract_icdev_tag(text: str) -> Optional[str]:
    """Extract {{sparkpilot: workflow}} tag from issue body or comment."""
    if not text:
        return None
...
def list_open_issues(label: str = "sparkpilot") -> list:
    """List open GitLab issues with the sparkpilot label."""
    try:
    ...
    while running:
        try:
            ...
            for issue in issues:
                ...
                print(f"[GitLab Task Monitor] Found: #{iid} '{title}' -> {{{{sparkpilot: {tag}}}}}")

                # Claim the issue
                claim_issue(iid, web_url, tag)

                # Add processing label
                add_label(iid, "sparkpilot-processing")
...
def main():
    parser = argparse.ArgumentParser(description="SPARKPILOT GitLab Task Board Monitor")
    ...
    if args.once:
        ...
        for issue in issues:
            ...
            tag = extract_icdev_tag(body)
            if tag:
                print(f"Issue #{issue.get('iid')}: {issue.get('title')} -> {{{{sparkpilot: {tag}}}}}")
    else:
