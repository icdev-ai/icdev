...
"""
Webhook server for SPARKPILOT CI/CD — receives events from GitHub, GitLab,
...
def _post_ack_comment(envelope: EventEnvelope, result: dict):
    ...
    try:
        ...
        vcs.comment_on_issue(
            ...
            f"{BOT_IDENTIFIER} SPARKPILOT Webhook: Detected `{workflow}` workflow\n\n"
            f"Run ID: `{run_id}`\n"
...
def health():
    ...
    try:
        ...
        return jsonify({
            ...
            "service": "sparkpilot-webhook-server",
...
if __name__ == "__main__":
    print("CUI // SP-CTI")
    print(f"Starting SPARKPILOT Webhook Server on port {PORT}")
