# [TEMPLATE: CUI // SP-CTI]
# SPARKPILOT Poll Trigger — Cron-based issue polling for GitHub + GitLab
# Refactored to use EventEnvelope + EventRouter (D132, D133)

"""
Cron-based SPARKPILOT trigger that polls GitHub/GitLab issues.

Polls every 20 seconds to detect:
1. New issues without comments
2. Issues where the latest comment contains 'sparkpilot'
...
def should_process_issue(vcs: VCS, issue_number: int) -> tuple:
    ...
    # Check if latest comment is exactly 'sparkpilot'
    if comment_lower == "sparkpilot":
        print(f"INFO: Issue #{issue_number} — latest comment is 'sparkpilot'")
...
def main():
    ...
    print("CUI // SP-CTI")
    print("INFO: Starting SPARKPILOT Poll Trigger (EventRouter-based)")
