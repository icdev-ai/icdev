#!/usr/bin/env python3
# CUI // SP-CTI
"""Generate CI/CD pipeline configs from sparkpilot.yaml (D192).
...
generates 9-stage GitLab CI for the SPARKPILOT framework itself). This generates
lightweight customer project pipelines from declarative sparkpilot.yaml.
...
CUI_HEADER = "# CUI // SP-CTI — Auto-generated by SPARKPILOT pipeline_config_generator"
...
def generate_pipeline(
    ...
    """Generate CI/CD pipeline config from sparkpilot.yaml.
    ...
    if not manifest["valid"] and not manifest["raw"]:
        result["errors"].append("No valid sparkpilot.yaml found")
        return result
    ...
    project_name = config.get("project", {}).get("name", "sparkpilot-project")
    ...
    if platform == "github":
        ...
        output_path = cwd / ".github" / "workflows" / "sparkpilot.yml"
    else:
...
def _generate_github_workflow(
    ...
    lines = [CUI_HEADER, ""]
    lines.append(f"name: SPARKPILOT Compliance Pipeline — {project_name}")
    lines.append("")
    lines.append("on:")
    ...
    if on_pr:
        lines.append("jobs:")
        lines.append("  sparkpilot-pr-checks:")
        ...
        lines.append("")
        lines.append("      - name: Install SPARKPILOT dependencies")
        lines.append("        run: pip install -r requirements.txt")
        lines.append("")
        lines.append("      - name: Initialize SPARKPILOT database")
    ...
    if on_merge:
        if not on_pr:
            lines.append("jobs:")
        lines.append("  sparkpilot-merge-artifacts:")
        ...
        lines.append("        run: pip install -r requirements.txt")
        lines.append("")
        lines.append("      - name: Initialize SPARKPILOT")
        lines.append("        run: python tools/db/init_icdev_db.py")
        lines.append("")
    ...
    # Scheduled job
    if on_schedule:
        lines.append("  sparkpilot-scheduled:")
...
def _generate_gitlab_ci(
    ...
    # Setup anchor
    lines.append(".sparkpilot-setup: &sparkpilot-setup")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate CI/CD pipeline configs from sparkpilot.yaml (D192)"
    )
