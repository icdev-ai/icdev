# [TEMPLATE: CUI // SP-CTI]
# SPARKPILOT Git Operations — branch, commit, push, PR/MR
# Adapted from ADW git_ops.py with GitLab MR support

"""
Git operations for SPARKPILOT CI/CD workflows.
...
def finalize_git_operations(state, logger, vcs=None):
    ...
    if not success:
        ...
        if issue_number:
            vcs.comment_on_issue(int(issue_number), f"[SPARKPILOT-BOT] Push failed: {error}")
        return
    ...
    if pr_url:
        ...
        if issue_number:
            ...
            vcs.comment_on_issue(
                int(issue_number),
                f"[SPARKPILOT-BOT] Updated existing {platform_name}: {pr_url}"
            )
        return
    ...
    title = f"SPARKPILOT-{run_id}: Issue #{issue_number}" if issue_number else f"SPARKPILOT-{run_id}"
    body = (
        f"## Summary\n"
        f"Automated by SPARKPILOT workflow run `{run_id}`\n\n"
        f"Closes #{issue_number}\n\n"
        f"## CUI // SP-CTI\n"
        f"Generated by SPARKPILOT CI/CD system.\n"
    )
    ...
    if pr_url:
        ...
        if issue_number:
            vcs.comment_on_issue(
                int(issue_number),
                f"[SPARKPILOT-BOT] Created {platform_name}: {pr_url}"
            )
    else:
