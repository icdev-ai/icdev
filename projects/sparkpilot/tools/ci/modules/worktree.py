...
Usage:
    ...
    python tools/ci/modules/worktree.py --cleanup --worktree-name sparkpilot-test-123
    python tools/ci/modules/worktree.py --status --worktree-name sparkpilot-test-123
"""
...
def create_worktree(task_id: str, target_dir: str, classification: str = "CUI",
    ...
    Pattern (Phase 41 — Parallel CI/CD):
        git worktree add --no-checkout trees/<task_id> -b sparkpilot-<task_id>
    ...
    """
    worktree_name = f"sparkpilot-{task_id}"
    branch_name = f"sparkpilot-{task_id}"
    ...
    agent_file = worktree_path / ".sparkpilot-agent"
...
def list_worktrees() -> List[WorktreeInfo]:
    """List all SPARKPILOT git worktrees."""
...
def cleanup_worktree(worktree_name: str) -> bool:
    ...
    task_id = worktree_name.replace("sparkpilot-", "")
    worktree_path = TREES_DIR / task_id
...
def main():
    parser = argparse.ArgumentParser(description="SPARKPILOT Git Worktree Manager")
