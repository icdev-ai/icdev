...
Detects the current project (via sparkpilot.yaml or DB lookup), loads compliance
...
Detection order:
    1. sparkpilot.yaml in cwd → manifest_loader.load_manifest()
...
def build_session_context(directory: str = None, db_path: str = None) -> dict:
    ...
    Args:
        ...
        db_path: Path to sparkpilot.db (defaults to data/sparkpilot.db).
    ...
    if detection["source"] == "yaml":
        ...
        if detection.get("db_record"):
            ...
            context["warnings"].append(
                "Project found in sparkpilot.yaml but not registered in SPARKPILOT database. "
                "Run `/sparkpilot-init` or `python tools/project/session_context_builder.py --init` "
                "to register."
            )
        # Manifest warnings
        ...
        context["warnings"].append(
            "No sparkpilot.yaml found and current directory is not a registered SPARKPILOT project."
        )
...
def _detect_project(directory: str, db_path: str) -> dict:
    """Detect whether this directory is an SPARKPILOT project.
...
def _suggest_workflows(context: dict) -> list:
    ...
    if context.get("setup_needed"):
        suggestions.append({
            "command": "/sparkpilot-init",
            "reason": "No project detected — initialize a new SPARKPILOT project",
        })
        return suggestions
    ...
    if context.get("source") == "yaml" and not project.get("db_project_id"):
        suggestions.append({
            "command": "/sparkpilot-init",
            "reason": "sparkpilot.yaml found but project not registered in DB",
        })

    # No SSP generated
    if compliance.get("ssp_status") in (None, "not_generated"):
        suggestions.append({
            "command": "/sparkpilot-comply",
...
def _format_markdown(context: dict) -> str:
    ...
    if context.get("setup_needed"):
        lines.append("## SPARKPILOT Project Context")
        lines.append("")
        lines.append("**No SPARKPILOT project detected in this directory.**")
        lines.append("")
        lines.append("To get started:")
        lines.append("1. Create an `sparkpilot.yaml` manifest (see `docs/dx/sparkpilot-yaml-spec.md`)")
        lines.append("2. Run `/sparkpilot-init` to initialize the project")
        lines.append("")
    ...
    lines.append(f"## SPARKPILOT Project Context")
    lines.append("")
...
def init_from_manifest(directory: str = None, db_path: str = None) -> dict:
    """Create a DB project record from sparkpilot.yaml.

    Bridges the gap between 'sparkpilot.yaml exists' and 'project is in the DB'.
    Also creates a dev profile if the manifest specifies a profile template.

    Args:
        directory: Project directory containing sparkpilot.yaml.
        db_path: Path to sparkpilot.db.
    ...
    try:
        ...
        try:
            conn.execute(
                ...
                (
                    db_id,
                    f"Initialized project '{name}' from sparkpilot.yaml",
                    json.dumps({"manifest_path": str(cwd / "sparkpilot.yaml"), "impact_level": il}),
                    classification,
                ),
            )
        except Exception:
            ...
            try:
                conn.execute(
                    """INSERT INTO dev_profiles
                       ...
                       VALUES ('project', ?, 1, ?, '{}', 'sparkpilot-yaml', ?)""",
...
def main():
    ...
    parser.add_argument("--db", help="Path to sparkpilot.db")
    parser.add_argument("--format", choices=["markdown", "json"],
                        default="markdown", help="Output format")
    parser.add_argument("--json", action="store_true",
                        help="Output JSON (shortcut for --format json)")
    parser.add_argument("--init", action="store_true",
                        help="Initialize DB project record from sparkpilot.yaml")
