...
appropriate pipeline depth:
  ...
  - **Standard**: Moderate complexity, standard SPARKPILOT pipeline
...
def _build_recommendation(overall_score: float, complexity_level: str) -> dict:
    ...
    if complexity_level == "quick_flow":
        ...
        return {
            ...
            "estimated_phases": 3,
            "rationale": (
                "Moderate complexity project. Standard SPARKPILOT pipeline recommended: "
...
def score_complexity(session_id: str, db_path=None) -> dict:
    ...
    Args:
        ...
        db_path: Optional Path override for the SPARKPILOT database.
...
def main():
    parser = argparse.ArgumentParser(
        description="SPARKPILOT Scale-Adaptive Complexity Scorer (BMAD pattern)"
    )
