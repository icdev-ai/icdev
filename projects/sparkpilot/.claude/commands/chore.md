...
Plan, implement, validate through the full sparkpilot DevSecOps pipeline, and commit a chore in one shot. The user provides only a description — sparkpilot handles everything else.
...
7. **Post Plan to GitHub Issue**:
   ```bash
   gh issue comment <number> --body "[sparkpilot-BOT] Chore plan created: specs/<filename>.md
...
### Phase 3: Validate — Full sparkpilot DevSecOps Pipeline (Stress-test)

Run the complete sparkpilot validation pipeline across all 4 tiers. Every **GATE** must pass before committing. Fix failures and re-run until clean. Reference: `args/security_gates.yaml`
...
45. **Post Completion to GitHub Issue** — Include actual results from each gate:
    ```bash
    gh issue comment <number> --body "[sparkpilot-BOT] Chore complete — all sparkpilot DevSecOps gates passed.
...
## Report
- Summarize what was done: issue created, branch created, plan written, chore completed, all sparkpilot DevSecOps gates passed.
