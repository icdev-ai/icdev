...
Plan, implement, validate through the full sparkpilot DevSecOps pipeline, and commit a feature in one shot. The user provides only a description — sparkpilot handles everything else.
...
7. **Post Plan to GitHub Issue**:
   ```bash
   gh issue comment <number> --body "[sparkpilot-BOT] Plan created: specs/<filename>.md
...
46. **Post Completion to GitHub Issue** — Include actual results from each gate (not just pass/fail):
    ```bash
    gh issue comment <number> --body "[sparkpilot-BOT] Feature complete — all sparkpilot DevSecOps gates passed.
...
## Report
- Summarize what was done: issue created, branch created, plan written, code implemented, all sparkpilot DevSecOps gates passed.
