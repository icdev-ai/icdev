...
**Why this matters:** 10 U.S.C. Section 4401 mandates MOSA for all major defense acquisition programs. DoDI 5000.87 reinforces this for software-intensive systems. Systems that fail to demonstrate modular design, open interfaces, and published standards risk acquisition milestone disapproval, vendor lock-in, and inability to integrate with future DoD enterprise services. SPARKPILOT auto-enforces MOSA so compliance is continuous, not a last-minute documentation exercise.
...
- **Manual trigger:** Customer or ISSO explicitly requests MOSA assessment via `/sparkpilot-mosa`
...
Evaluate MOSA gate criteria. This gate integrates with the existing SPARKPILOT security gate framework (`args/security_gates.yaml`).
...
1. **Non-DoD project:** MOSA is not required. Workflow skips entirely. Modularity analysis still available on-demand via `/sparkpilot-mosa` for any project wanting to improve architecture quality.
...
- `/sparkpilot-mosa` — Run full MOSA workflow (Steps 1-8)
- `/sparkpilot-comply` — Includes MOSA in multi-regime assessment when applicable
- `/sparkpilot-intake` — Detects MOSA signals during conversational intake
...
- **Database:** `data/sparkpilot.db` (mosa_assessments, mosa_modularity_metrics, mosa_icds, mosa_tsp tables)
