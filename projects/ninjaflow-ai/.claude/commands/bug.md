...
Plan, implement, validate through the full ninjaflow-ai DevSecOps pipeline, and commit a bug fix in one shot. The user provides only a description — ninjaflow-ai handles everything else.
...
7. **Post Plan to GitHub Issue**:
   ```bash
   gh issue comment <number> --body "[ninjaflow-ai-BOT] Fix plan created: specs/<filename>.md
...
14. **E2E Browser Verification** (MANDATORY — auto-detected) —
    ...
    **If UI changes detected** (or fix explicitly involves dashboard/web pages):
    ...
    3. **Interactive User Verification** — Open the affected page(s) with Playwright MCP and verify the fix works:
       - `browser_navigate` to each affected page
       - Interact as the user would — click buttons, submit forms, trigger the fixed workflow
       ...
       - Confirm the bug is actually fixed from the user's perspective
    ...
    4. **Vision Validation** on screenshots:
       ...
       python tools/testing/screenshot_validator.py \
           ...
           --assert "<bug-fix-specific assertion>" \
           --json
       ```

    **GATE: Do NOT proceed to commit if ANY of these fail:**
    - E2E test specs fail
    - Interactive verification shows broken pages
    - Vision validation assertions fail
    - Error patterns detected on any page (500, traceback, TemplateNotFound)
...
46. **Post Completion to GitHub Issue** — Include actual results from each gate:
    ```bash
    gh issue comment <number> --body "[ninjaflow-ai-BOT] Bug fix complete — all ninjaflow-ai DevSecOps gates passed.
...
## Report
- Summarize what was done: issue created, branch created, plan written, fix implemented, all ninjaflow-ai DevSecOps gates passed.
