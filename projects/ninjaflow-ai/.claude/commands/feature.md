...
Plan, implement, validate through the full ninjaflow-ai DevSecOps pipeline, and commit a feature in one shot. The user provides only a description — ninjaflow-ai handles everything else.
...
7. **Post Plan to GitHub Issue**:
   ```bash
   gh issue comment <number> --body "[ninjaflow-ai-BOT] Plan created: specs/<filename>.md
...
14. **E2E Browser Verification** (MANDATORY — auto-detected) —
    ...
    **If UI changes detected** (or feature explicitly involves dashboard/web pages):
    ...
    3. **Interactive User Verification** — Open the affected page(s) with Playwright MCP and verify the feature works:
       - `browser_navigate` to each affected page
       - Interact as the user would — click buttons, submit forms, watch real-time updates
       ...
       - Verify the feature behaves correctly from the user's perspective
...
46. **Post Completion to GitHub Issue** — Include actual results from each gate (not just pass/fail):
    ```bash
    gh issue comment <number> --body "[ninjaflow-ai-BOT] Feature complete — all ninjaflow-ai DevSecOps gates passed.
...
## Report
- Summarize what was done: issue created, branch created, plan written, code implemented, all ninjaflow-ai DevSecOps gates passed.
