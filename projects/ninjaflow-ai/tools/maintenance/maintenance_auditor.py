...
def _log_audit_event(conn, project_id, action, details, file_path=None):
    ...
    try:
        conn.execute(
            ...
            (project_id, "maintenance_audit", "ninjaflow_ai-maintenance-engine", action,
...
def _parse_simple_yaml(path):
    """Minimal stdlib YAML parser for NINJAFLOW_AI config files (2-level nesting)."""
    if not path.exists():
        return {}
...
def _generate_audit_report(audit_data, output_dir, project_name):
    ...
    L += ["## NIST 800-53 Control Mapping", "",
          ...
          "---", "", f"*Generated by NINJAFLOW_AI Maintenance Auditor on {ts}*", "",
