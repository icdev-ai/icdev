...
def _labels(app_name: str, extra: dict = None) -> dict:
    base = {
        ...
        "app.kubernetes.io/managed-by": "ninjaflow_ai",
        **CUI_LABELS,
    }
    if extra:
        base.update(extra)
...
def generate_deployment(project_path: str, app_config: dict = None) -> list:
    ...
    name = config.get("name", "ninjaflow_ai-app")
    ...
    deployment = {
        ...
        "metadata": {
            ...
            "annotations": {
                "classification": "CUI",
                "ninjaflow_ai.io/generated": datetime.now(timezone.utc).isoformat(),
            },
        },
        "spec": {
            "replicas": replicas,
...
# ---------------------------------------------------------------------------
# Default agent definitions matching NINJAFLOW_AI multi-agent architecture
DEFAULT_AGENTS = [
...
def generate_agent_deployments(project_path: str, blueprint: dict = None) -> list:
    ...
    Creates a Deployment and Service for each agent in the NINJAFLOW_AI multi-agent
    ...
    namespace = config.get("namespace", "ninjaflow_ai-agents")
    registry = config.get("registry", "registry.example.com/ninjaflow_ai")

    k8s_dir = Path(project_path) / "k8s" / "agents"
    files = []

    for agent in agents:
        agent_name = f"ninjaflow_ai-{agent['name']}"
        port = agent["port"]
        ...
        deployment = {
            ...
            "metadata": {
                ...
                "labels": _labels(agent_name, {
                    ...
                    "ninjaflow_ai.io/tier": "agent",
                }),
                "annotations": {
                    "classification": "CUI",
                    "ninjaflow_ai.io/generated": datetime.now(timezone.utc).isoformat(),
                    "ninjaflow_ai.io/agent-port": str(port),
                },
            },
            "spec": {
                "replicas": 2,
    ...
    for agent in agents:
        ...
        hpa = {
            ...
            "metadata": {
                ...
                "namespace": namespace,
                "labels": _labels(agent_name, {
                    "ninjaflow_ai.io/scaling-tier": "core" if name_key in core_agents
                    else "domain" if name_key in domain_agents
                    else "support",
                }),
                "annotations": {
                    "classification": "CUI",
                    "ninjaflow_ai.io/scaling-profile": "predictive",
                },
            },
            "spec": {
                "scaleTargetRef": {
...
def main():
    ...
    parser.add_argument("--name", default="ninjaflow_ai-app", help="Application name")
