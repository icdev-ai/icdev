...
def generate_base(project_path: str, project_config: dict = None) -> list:
    ...
    project_name = config.get("project_name", "ninjaflow_ai-project")
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws-us-gov:logs:${{data.aws_region.current.name}}:${{data.aws_caller_identity.current.account_id}}:log-group:/ninjaflow_ai/*"
      }
    ]
  })
...
def generate_bedrock_iam(project_path: str, config: dict = None) -> list:
    ...
    Creates IAM role and policy that allows NINJAFLOW_AI agents to invoke
...
def main():
    ...
    parser.add_argument("--project-name", default="ninjaflow_ai-project", help="Project name for resource naming")
