...
to external Policy Decision Points. NINJAFLOW_AI does NOT implement PDP logic itself.

ADR D124: PDP modeled as external reference (Zscaler, Palo Alto, DISA ICAM, CrowdStrike) —
NINJAFLOW_AI generates PEP configs (Istio AuthorizationPolicy) but does NOT implement PDP itself.
...
# ---------------------------------------------------------------------------
# PDP reference generation (D124: NINJAFLOW_AI documents but does NOT implement PDP)
# ---------------------------------------------------------------------------

def generate_pdp_reference(project_id: str, pdp_type: str) -> dict:
    """Document external PDP integration point for a project.

    NINJAFLOW_AI does not implement PDP logic. This function generates documentation
    ...
    ADR D124: PDP is an external reference. NINJAFLOW_AI generates PEP configs only.
    ...
    return {
        ...
        "adr_reference": "ADR D124: PDP is external — NINJAFLOW_AI generates PEP configs only",
        "generated_at": now,
    ...
    configs = {
        "disa_icam": {
            ...
            "connection_details": {
                ...
                "audience": f"ninjaflow_ai-{project.get('name', 'app')}.mil",
            },
            "k8s_integration": {
    ...
    docs = {
        "disa_icam": {
            "summary": "DISA ICAM (Identity, Credential, and Access Management) provides DoD-wide identity services. "
                       "NINJAFLOW_AI generates Istio/Linkerd PEP configurations that delegate authorization decisions "
        ...
        "zscaler": {
            "summary": "Zscaler Private Access (ZPA) provides Zero Trust Network Access. "
                       "NINJAFLOW_AI generates K8s Connector DaemonSet manifests and PEP network policies "
        ...
        "palo_alto_prisma": {
            "summary": "Palo Alto Prisma Access combines ZTNA and cloud-delivered NGFW. "
                       ...
                       "NINJAFLOW_AI generates Defender DaemonSet manifests and PEP policies. "
        ...
        "crowdstrike": {
            "summary": "CrowdStrike Falcon provides device trust and endpoint detection. "
                       "NINJAFLOW_AI generates Falcon Sensor DaemonSet manifests and device posture "
        ...
        "microsoft_entra": {
            "summary": "Microsoft Entra ID (formerly Azure AD) provides cloud identity and conditional access. "
                       "NINJAFLOW_AI generates Istio/Linkerd PEP configurations that validate Entra ID JWT tokens "
                       "and enforce conditional access policies. Phishing-resistant MFA and device compliance "
                       "checks are enforced through Entra conditional access — not by NINJAFLOW_AI.",
            "customer_responsibilities": [
        ...
        "custom": {
            "summary": "Customer-provided PDP integration. NINJAFLOW_AI generates PEP configurations "
...
def generate_pep_config(project_id: str, mesh: str = "istio", pdp_type: str = "disa_icam") -> dict:
    ...
    The PEP enforces decisions made by the external PDP — NINJAFLOW_AI does not implement
    ...
    result["adr_reference"] = "ADR D124: PEP generated by NINJAFLOW_AI; PDP is external"
...
def _generate_istio_pep(project_id: str, pdp_type: str, project: dict) -> dict:
    ...
    namespace = f"ninjaflow_ai-{project_id[:8]}"
    ...
    authz_policy = {
        ...
        "metadata": {
            ...
            "namespace": namespace,
            "labels": {
                "app.kubernetes.io/managed-by": "ninjaflow-ai",
                "ninjaflow_ai.mil/project-id": project_id,
                "ninjaflow_ai.mil/classification": project.get("classification", "CUI"),
                "ninjaflow_ai.mil/pdp-type": pdp_type,
            },
            "annotations": {
                "ninjaflow_ai.mil/adr": "D124 — PDP is external; this policy is the PEP",
                "ninjaflow_ai.mil/generated-at": datetime.now(timezone.utc).isoformat(),
            },
        },
        "spec": {
            "action": "CUSTOM",
...
def _generate_linkerd_pep(project_id: str, pdp_type: str, project: dict) -> dict:
    ...
    server_authz = {
        ...
        "metadata": {
            ...
            "annotations": {
                "ninjaflow_ai.mil/adr": "D124 — PDP is external; this is the PEP config",
                "ninjaflow_ai.mil/pdp-reference": _get_grpc_service(pdp_type),
                "ninjaflow_ai.mil/generated-at": datetime.now(timezone.utc).isoformat(),
            },
        },
        "spec": {
    ...
    auth_policy = {
        ...
        "metadata": {
            ...
            "annotations": {
                "ninjaflow_ai.mil/adr": "D124 — external PDP reference",
            },
        },
        "spec": {
            "targetRef": {
...
def generate_device_trust_config(project_id: str, mdm_type: str = "crowdstrike") -> dict:
    ...
    NINJAFLOW_AI generates the K8s manifests and configuration references — the
    ...
    return {
        ...
        "enforcement_model": (
            ...
            "NINJAFLOW_AI generates K8s admission webhook configs and PEP policies "
            "that query the MDM API at admission time."
        ),
        "adr_reference": "ADR D124: device trust PDP is external — NINJAFLOW_AI generates PEP integration configs",
        "generated_at": now,
    }
...
def _build_device_trust_manifests(project_id: str, mdm_type: str, project: dict) -> dict:
    ...
    kyverno_device_policy = {
        ...
        "metadata": {
            ...
            "annotations": {
                ...
                "ninjaflow_ai.mil/adr": "D124 — device trust PDP is external",
            },
            "labels": {
                "app.kubernetes.io/managed-by": "ninjaflow-ai",
                "ninjaflow_ai.mil/project-id": project_id,
            ...
            "rules": [
                {
                    ...
                    "validate": {
                        "message": (
                            ...
                            f"(ninjaflow_ai.mil/device-trust-verified: 'true')"
                        ),
                        "pattern": {
                            "metadata": {
                                "annotations": {
                                    "ninjaflow_ai.mil/device-trust-verified": "true",
                                    "ninjaflow_ai.mil/device-trust-source": f"{mdm_type}",
                                }
                            }
                        },
                    },
    ...
    secret_template = {
        ...
        "metadata": {
            ...
            "annotations": {
                "ninjaflow_ai.mil/secret-source": "aws-secrets-manager",
                "ninjaflow_ai.mil/do-not-commit": "true — populate from AWS Secrets Manager ExternalSecret",
            },
        },
        "type": "Opaque",
        "stringData": {
...
def _build_posture_policy(mdm_type: str, project: dict) -> dict:
    ...
    return {
        ...
        "check_sequence": [
            ...
            "4. Annotate pod with device trust result (ninjaflow_ai.mil/device-trust-verified)",
        ...
        if args.device_trust:
            ...
            if pep:
                ...
                for note in pep.get("integration_notes", [])[:3]:
                    print(f"  Note:     {note}")
            print("ADR:        ADR D124 — PDP is external; NINJAFLOW_AI generates PEP configs only")
