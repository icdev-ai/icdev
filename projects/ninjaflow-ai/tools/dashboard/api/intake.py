...
@intake_api.route("/api/intake/conversation/<session_id>", methods=["GET"])
def get_conversation(session_id):
    """Get conversation messages for a session (alias for chat.js)."""
    conn = _get_db()
    try:
        messages = conn.execute(
            "SELECT turn_number, role, content, content_type, created_at "
        ...
        return jsonify({"messages": [], "error": str(exc)}), 200
    finally:
        conn.close()
