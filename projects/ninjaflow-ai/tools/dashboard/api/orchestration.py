...
def get_agent_grid():
    ...
    try:
        ...
        for agent in agents:
            ...
            if token_row and len(token_row) > 0 and token_row[0].get("total_in"):
...
def get_stats():
    ...
    try:
        ...
        avg_response_ms = 0
        if avg_row and len(avg_row) > 0 and avg_row[0].get("avg_ms"):
            avg_response_ms = round(avg_row[0]["avg_ms"] or 0)
