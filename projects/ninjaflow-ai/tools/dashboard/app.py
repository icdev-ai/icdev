...
# CUI // SP-CTI
"""
NINJAFLOW_AI Web Dashboard - Flask Application
========================================
Provides a web interface for monitoring projects, agents, compliance,
and system health within the NINJAFLOW_AI framework.
...
def create_app() -> Flask:
    ...
    @app.route("/phases")
    def phases_page():
        """Phase roadmap — all NINJAFLOW_AI phases with status, categories, and progress."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NINJAFLOW_AI Dashboard")
    ...
    app = create_app()
    print(f"[NINJAFLOW_AI Dashboard] Starting on http://127.0.0.1:{args.port}")
    print(f"[NINJAFLOW_AI Dashboard] Database: {DB_PATH}")
    print(f"[NINJAFLOW_AI Dashboard] CUI Marking: {CUI_BANNER_TOP}")

    # Use SocketIO runner if available (D170), otherwise plain Flask
    socketio = get_socketio()
    if socketio:
        print("[NINJAFLOW_AI Dashboard] WebSocket enabled (Flask-SocketIO)")
        socketio.run(app, host="0.0.0.0", port=args.port, debug=args.debug)
    else:
        print("[NINJAFLOW_AI Dashboard] WebSocket not available — using HTTP polling")
