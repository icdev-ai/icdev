# [TEMPLATE: CUI // SP-CTI]
# NINJAFLOW_AI Event Router — central routing with lane-aware session queue (D133)
...
class EventRouter:
    ...
    def route(self, envelope: EventEnvelope) -> dict:
        ...
        if not workflow:
            # Check if content contains "ninjaflow_ai" keyword (generic trigger)
            if "ninjaflow_ai" in envelope.content.lower():
