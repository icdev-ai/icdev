...
Usage:
    ...
    python tools/ci/modules/worktree.py --cleanup --worktree-name ninjaflow_ai-test-123
    python tools/ci/modules/worktree.py --status --worktree-name ninjaflow_ai-test-123
"""
...
def create_worktree(task_id: str, target_dir: str, classification: str = "CUI",
    ...
    Pattern (Phase 41 — Parallel CI/CD):
        git worktree add --no-checkout trees/<task_id> -b ninjaflow_ai-<task_id>
    ...
    """
    worktree_name = f"ninjaflow_ai-{task_id}"
    branch_name = f"ninjaflow_ai-{task_id}"
    ...
    agent_file = worktree_path / ".ninjaflow_ai-agent"
...
def list_worktrees() -> List[WorktreeInfo]:
    """List all NINJAFLOW_AI git worktrees."""
...
def cleanup_worktree(worktree_name: str) -> bool:
    ...
    task_id = worktree_name.replace("ninjaflow_ai-", "")
    worktree_path = TREES_DIR / task_id
...
def main():
    parser = argparse.ArgumentParser(description="NINJAFLOW_AI Git Worktree Manager")
