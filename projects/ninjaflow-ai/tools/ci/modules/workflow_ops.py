# [TEMPLATE: CUI // SP-CTI]
# NINJAFLOW_AI Workflow Operations — classify, branch, commit, PR helpers
# Adapted from ADW workflow_ops.py with dual platform support

"""
Shared workflow operations for NINJAFLOW_AI CI/CD pipelines.
...
# Available NINJAFLOW_AI workflows for runtime validation
...
def format_issue_message(
    ...
    """Format a message for issue comments with NINJAFLOW_AI tracking and bot identifier."""
    if session_id:
...
def extract_icdev_info(
    ...
    """Extract NINJAFLOW_AI workflow and run_id from text using classify_workflow agent.
    ...
    try:
        ...
        if not response.success:
            print(f"Failed to classify NINJAFLOW_AI workflow: {response.output}")
            return None, None
...
def find_existing_branch_for_issue(
    ...
    for branch in branches:
        ...
        if f"-issue-{issue_number}-" in branch:
            if run_id and f"-ninjaflow_ai-{run_id}-" in branch:
                return branch
            elif not run_id:
                return branch
