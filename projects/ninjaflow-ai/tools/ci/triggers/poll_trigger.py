# [TEMPLATE: CUI // SP-CTI]
# NINJAFLOW_AI Poll Trigger — Cron-based issue polling for GitHub + GitLab
# Refactored to use EventEnvelope + EventRouter (D132, D133)

"""
Cron-based NINJAFLOW_AI trigger that polls GitHub/GitLab issues.

Polls every 20 seconds to detect:
1. New issues without comments
2. Issues where the latest comment contains 'ninjaflow_ai'
...
def should_process_issue(vcs: VCS, issue_number: int) -> tuple:
    ...
    # Check if latest comment is exactly 'ninjaflow_ai'
    if comment_lower == "ninjaflow_ai":
        print(f"INFO: Issue #{issue_number} — latest comment is 'ninjaflow_ai'")
...
def main():
    ...
    print("CUI // SP-CTI")
    print("INFO: Starting NINJAFLOW_AI Poll Trigger (EventRouter-based)")
