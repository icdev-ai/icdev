# [TEMPLATE: CUI // SP-CTI]
"""GitLab Task Board Monitor — polls issues for {{ninjaflow_ai: workflow}} tags.

Decision D33: GitLab tags replace Notion's {{prototype: type}} pattern.

Supported tags:
    {{ninjaflow_ai: intake}}     -> Start RICOAS intake session
    {{ninjaflow_ai: build}}      -> Run TDD build workflow
    {{ninjaflow_ai: sdlc}}       -> Full Plan->Build->Test->Review
    {{ninjaflow_ai: comply}}     -> Generate compliance artifacts
    {{ninjaflow_ai: secure}}     -> Run security scanning
    {{ninjaflow_ai: modernize}}  -> Legacy app modernization
    {{ninjaflow_ai: deploy}}     -> Generate IaC and deploy
    {{ninjaflow_ai: maintain}}   -> Maintenance audit
...
TAG_PATTERN = re.compile(r"\{\{ninjaflow_ai:\s*(\w+)\}\}", re.IGNORECASE)

# Bot marker to prevent loops
BOT_MARKER = "[NINJAFLOW-AI-BOT]"


def extract_icdev_tag(text: str) -> Optional[str]:
    """Extract {{ninjaflow_ai: workflow}} tag from issue body or comment."""
    if not text:
        return None
...
def list_open_issues(label: str = "ninjaflow_ai") -> list:
    """List open GitLab issues with the ninjaflow_ai label."""
    try:
    ...
    while running:
        try:
            ...
            for issue in issues:
                ...
                print(f"[GitLab Task Monitor] Found: #{iid} '{title}' -> {{{{ninjaflow_ai: {tag}}}}}")

                # Claim the issue
                claim_issue(iid, web_url, tag)

                # Add processing label
                add_label(iid, "ninjaflow_ai-processing")
...
def main():
    parser = argparse.ArgumentParser(description="NINJAFLOW_AI GitLab Task Board Monitor")
    ...
    if args.once:
        ...
        for issue in issues:
            ...
            tag = extract_icdev_tag(body)
            if tag:
                print(f"Issue #{issue.get('iid')}: {issue.get('title')} -> {{{{ninjaflow_ai: {tag}}}}}")
    else:
