# [TEMPLATE: CUI // SP-CTI]
# NINJAFLOW_AI Webhook Server — GitHub + GitLab + Slack + Mattermost webhook support
# Refactored to use EventEnvelope + EventRouter (D132, D133)

"""
Webhook server for NINJAFLOW_AI CI/CD — receives events from GitHub, GitLab,
...
def _post_ack_comment(envelope: EventEnvelope, result: dict):
    ...
    try:
        ...
        vcs.comment_on_issue(
            ...
            f"{BOT_IDENTIFIER} NINJAFLOW_AI Webhook: Detected `{workflow}` workflow\n\n"
            f"Run ID: `{run_id}`\n"
...
def health():
    ...
    try:
        ...
        return jsonify({
            ...
            "service": "ninjaflow_ai-webhook-server",
...
if __name__ == "__main__":
    print("CUI // SP-CTI")
    print(f"Starting NINJAFLOW_AI Webhook Server on port {PORT}")
