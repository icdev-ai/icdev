#!/usr/bin/env python3
# CUI // SP-CTI
"""Generate CI/CD pipeline configs from ninjaflow_ai.yaml (D192).
...
generates 9-stage GitLab CI for the NINJAFLOW_AI framework itself). This generates
lightweight customer project pipelines from declarative ninjaflow_ai.yaml.
...
CUI_HEADER = "# CUI // SP-CTI — Auto-generated by NINJAFLOW_AI pipeline_config_generator"
...
def generate_pipeline(
    ...
    """Generate CI/CD pipeline config from ninjaflow_ai.yaml.
    ...
    if not manifest["valid"] and not manifest["raw"]:
        result["errors"].append("No valid ninjaflow_ai.yaml found")
        return result
    ...
    project_name = config.get("project", {}).get("name", "ninjaflow_ai-project")
    ...
    if platform == "github":
        ...
        output_path = cwd / ".github" / "workflows" / "ninjaflow_ai.yml"
    else:
...
def _generate_github_workflow(
    ...
    lines = [CUI_HEADER, ""]
    lines.append(f"name: NINJAFLOW_AI Compliance Pipeline — {project_name}")
    lines.append("")
    lines.append("on:")
    ...
    if on_pr:
        lines.append("jobs:")
        lines.append("  ninjaflow_ai-pr-checks:")
        ...
        lines.append("")
        lines.append("      - name: Install NINJAFLOW_AI dependencies")
        lines.append("        run: pip install -r requirements.txt")
        lines.append("")
        lines.append("      - name: Initialize NINJAFLOW_AI database")
    ...
    if on_merge:
        if not on_pr:
            lines.append("jobs:")
        lines.append("  ninjaflow_ai-merge-artifacts:")
        ...
        lines.append("        run: pip install -r requirements.txt")
        lines.append("")
        lines.append("      - name: Initialize NINJAFLOW_AI")
        lines.append("        run: python tools/db/init_icdev_db.py")
        lines.append("")
    ...
    # Scheduled job
    if on_schedule:
        lines.append("  ninjaflow_ai-scheduled:")
...
def _generate_gitlab_ci(
    ...
    # Setup anchor
    lines.append(".ninjaflow_ai-setup: &ninjaflow_ai-setup")
    ...
    for check_name in on_pr:
        ...
        lines.append("  <<: *ninjaflow_ai-setup")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate CI/CD pipeline configs from ninjaflow_ai.yaml (D192)"
    )
