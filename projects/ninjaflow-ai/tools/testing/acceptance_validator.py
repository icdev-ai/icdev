# [TEMPLATE: CUI // SP-CTI]
# NINJAFLOW_AI Acceptance Criteria Validator (V&V)
...
Usage:
    python tools/testing/acceptance_validator.py \
        --plan specs/issue-3-ninjaflow_ai-abc-plan.md \
...
def check_page(base_url: str, page_path: str, timeout: int = 10) -> UIPageCheckResult:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "NINJAFLOW_AI-AcceptanceValidator/1.0"})
...
def main():
    parser = argparse.ArgumentParser(
        description="NINJAFLOW_AI Acceptance Criteria Validator (V&V)"
    )
    parser.add_argument(
        "--plan",
