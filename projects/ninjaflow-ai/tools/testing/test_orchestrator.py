# [TEMPLATE: CUI // SP-CTI]
# NINJAFLOW_AI Test Orchestrator
# Adapted from ADW adw_test.py — retry logic, resolution, E2E coordination

"""
NINJAFLOW_AI Test Orchestrator — runs unit, BDD, E2E, security, and compliance tests
...
def run_bandit(project_dir: str, logger) -> TestResult:
    ...
    Adapted from ADW test.md security scan step + NINJAFLOW_AI security tools.
    """
...
def run_tests_with_resolution(
    ...
    while attempt < max_attempts:
        ...
        # In NINJAFLOW_AI, resolution would invoke the builder agent to fix code
...
def evaluate_security_gate(project_dir: str, logger) -> GateEvaluation:
    """Evaluate security gate by running NINJAFLOW_AI security scans."""
    ...
    return GateEvaluation(
        ...
        evaluated_by="ninjaflow_ai-test-orchestrator",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="NINJAFLOW_AI Test Orchestrator")
    parser.add_argument("--project-dir", required=True, help="Path to project under test")
    parser.add_argument("--project-id", help="NINJAFLOW_AI project UUID")
    ...
    logger.info(f"NINJAFLOW_AI Test Orchestrator starting — Run ID: {run_id}")
