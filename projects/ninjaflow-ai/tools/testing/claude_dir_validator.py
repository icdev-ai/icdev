#!/usr/bin/env python3
# CUI // SP-CTI
"""NINJAFLOW_AI .claude Directory Governance Validator.

Cross-references .claude directory configuration (hooks, settings, commands,
E2E specs) against the NINJAFLOW_AI codebase to detect drift. Ensures append-only
...
# Tools where --project refers to something other than NINJAFLOW_AI project ID
...
def check_db_path_centralization(
    ...
    hardcoded_pattern = re.compile(
        r'''(?:Path\s*\([^)]*\)\s*(?:/\s*["']data["']\s*){1,2}/\s*["'](?:ninjaflow_ai|memory|platform)\.db["'])'''
        r'''|(?:["']\S*data[/\\](?:ninjaflow_ai|memory|platform)\.db["'])''',
    )
...
def format_human(report: ClaudeConfigReport) -> str:
    ...
    lines = []
    lines.append("=" * 60)
    lines.append("  NINJAFLOW_AI .claude Directory Governance Report")
    lines.append("=" * 60)
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate .claude directory alignment with NINJAFLOW_AI codebase"
    )
