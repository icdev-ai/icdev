...
Detects the current project (via ninjaflow_ai.yaml or DB lookup), loads compliance
...
Detection order:
    1. ninjaflow_ai.yaml in cwd → manifest_loader.load_manifest()
    ...
    if detection["source"] == "yaml":
        ...
        if detection.get("db_record"):
            ...
            context["warnings"].append(
                "Project found in ninjaflow_ai.yaml but not registered in NINJAFLOW_AI database. "
                "Run `/ninjaflow_ai-init` or `python tools/project/session_context_builder.py --init` "
                "to register."
            )
        # Manifest warnings
        ...
        context["warnings"].append(
            "No ninjaflow_ai.yaml found and current directory is not a registered NINJAFLOW_AI project."
        )
...
def _detect_project(directory: str, db_path: str) -> dict:
    """Detect whether this directory is an NINJAFLOW_AI project.
...
def _suggest_workflows(context: dict) -> list:
    ...
    if context.get("setup_needed"):
        suggestions.append({
            "command": "/ninjaflow_ai-init",
            "reason": "No project detected — initialize a new NINJAFLOW_AI project",
        })
        return suggestions
    ...
    if context.get("source") == "yaml" and not project.get("db_project_id"):
        suggestions.append({
            "command": "/ninjaflow_ai-init",
            "reason": "ninjaflow_ai.yaml found but project not registered in DB",
        })

    # No SSP generated
    if compliance.get("ssp_status") in (None, "not_generated"):
        suggestions.append({
            "command": "/ninjaflow_ai-comply",
...
def _format_markdown(context: dict) -> str:
    ...
    if context.get("setup_needed"):
        lines.append("## NINJAFLOW_AI Project Context")
        lines.append("")
        lines.append("**No NINJAFLOW_AI project detected in this directory.**")
        lines.append("")
        lines.append("To get started:")
        lines.append("1. Create an `ninjaflow_ai.yaml` manifest (see `docs/dx/ninjaflow_ai-yaml-spec.md`)")
        lines.append("2. Run `/ninjaflow_ai-init` to initialize the project")
        lines.append("")
    ...
    lines.append(f"## NINJAFLOW_AI Project Context")
    lines.append("")
...
def init_from_manifest(directory: str = None, db_path: str = None) -> dict:
    """Create a DB project record from ninjaflow_ai.yaml.

    Bridges the gap between 'ninjaflow_ai.yaml exists' and 'project is in the DB'.
    Also creates a dev profile if the manifest specifies a profile template.

    Args:
        directory: Project directory containing ninjaflow_ai.yaml.
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO projects
               ...
               VALUES (?, ?, ?, ?, ?, 'active', ?, ?, 'ninjaflow_ai-yaml',
                       ?, ?, ?, ?, ?, ?)""",
            (
        ...
        try:
            conn.execute(
                ...
                (
                    db_id,
                    f"Initialized project '{name}' from ninjaflow_ai.yaml",
                    json.dumps({"manifest_path": str(cwd / "ninjaflow_ai.yaml"), "impact_level": il}),
                    classification,
                ),
            )
        except Exception:
            ...
            try:
                conn.execute(
                    """INSERT INTO dev_profiles
                       ...
                       VALUES ('project', ?, 1, ?, '{}', 'ninjaflow_ai-yaml', ?)""",
...
def main():
    ...
    parser.add_argument("--init", action="store_true",
                        help="Initialize DB project record from ninjaflow_ai.yaml")
