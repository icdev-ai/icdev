...
logger = logging.getLogger("ninjaflow_ai.observability.sqlite_tracer")
...
class SQLiteTracer(Tracer):
    ...
    def start_span(
        ...
        attrs = {}
        if self._agent_id:
            attrs["ninjaflow_ai.agent_id"] = self._agent_id
        if self._project_id:
            attrs["ninjaflow_ai.project_id"] = self._project_id
        if attributes:
                ...
                try:
                    conn.execute(
                        ...
                        (
                            ...
                            span.attributes.get("ninjaflow_ai.agent_id", self._agent_id),
                            span.attributes.get("ninjaflow_ai.project_id", self._project_id),
