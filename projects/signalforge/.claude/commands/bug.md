...
Plan, implement, validate through the full signalforge DevSecOps pipeline, and commit a bug fix in one shot. The user provides only a description — signalforge handles everything else.
...
7. **Post Plan to GitHub Issue**:
   ```bash
   gh issue comment <number> --body "[signalforge-BOT] Fix plan created: specs/<filename>.md
...
46. **Post Completion to GitHub Issue** — Include actual results from each gate:
    ```bash
    gh issue comment <number> --body "[signalforge-BOT] Bug fix complete — all signalforge DevSecOps gates passed.
...
## Report
- Summarize what was done: issue created, branch created, plan written, fix implemented, all signalforge DevSecOps gates passed.
