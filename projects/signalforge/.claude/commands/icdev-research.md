...
Run the signalforge Industry Research Engine to perform deep, structured research into a target industry vertical. Produces a scored, ranked dossier of industry challenges, regulatory landscape, community pain points, competitive analysis, and build-vs-buy recommendations — feeding directly into the agentic child app generation pipeline.
...
1. **Check signalforge database is initialized** and research tables exist:
...
4. **Run full pipeline** (all 8 stages sequentially):
   ...
   **Stage 3 — REGULATE** (regulatory body + compliance requirements):
   ...
   ```
   Review: primary regulations, signalforge crosswalk coverage, enforcement actions, upcoming changes, compliance gaps.
   ...
   **Stage 5 — ACADEMIC** (papers + patents + emerging tech):
   ...
   ```
   Review: relevant papers (Semantic Scholar, arXiv), patent filings (USPTO), emerging tech trends, signalforge applicability. If air-gapped, skip and note.

   **Stage 6 — BUILD_BUY** (open-source + SaaS + gap analysis):
   ```bash
   python tools/research/build_buy_analyzer.py --session-id $SESSION_ID --json
   ```
   Review: per-challenge build-vs-buy matrix (compliance_control 0.30, integration_effort 0.25, cost 0.20, time_to_market 0.15, competitive_advantage 0.10), license checks, signalforge capability gaps.
   ...
   **Stage 8 — DOSSIER** (template-based report generation):
   ...
   ```
   Review: 11-section dossier (executive summary, market landscape, regulatory environment, community pain points, academic landscape, build-vs-buy analysis, challenge ranking, signalforge capability map, recommended child app scope, risk assessment, appendix).
...
5. **Present dossier for human review**:
   ...
   c. **Regulatory Landscape** — Key regulations, signalforge coverage percentage, gaps requiring new framework catalogs.

   d. **Community Voice** — Most frequent pain points, representative quotes/descriptions, severity distribution.

   e. **Competitive Position** — How signalforge compares to existing solutions, differentiation opportunities.

   f. **Build-vs-Buy Summary** — Challenges where signalforge should build (compliance control + competitive advantage) vs. integrate (commodity features).
...
- **Air-gapped mode:** Stages 2-5 (landscape, regulate, community, academic) require network for full coverage. If unavailable, skip web sources gracefully and note reduced coverage in dossier. Build-vs-buy (stage 6) can still analyze signalforge internal capabilities.
