...
Plan, implement, validate through the full signalforge DevSecOps pipeline, and commit a chore in one shot. The user provides only a description — signalforge handles everything else.
...
7. **Post Plan to GitHub Issue**:
   ```bash
   gh issue comment <number> --body "[signalforge-BOT] Chore plan created: specs/<filename>.md
...
### Phase 3: Validate — Full signalforge DevSecOps Pipeline (Stress-test)

Run the complete signalforge validation pipeline across all 4 tiers. Every **GATE** must pass before committing. Fix failures and re-run until clean. Reference: `args/security_gates.yaml`
...
45. **Post Completion to GitHub Issue** — Include actual results from each gate:
    ```bash
    gh issue comment <number> --body "[signalforge-BOT] Chore complete — all signalforge DevSecOps gates passed.
...
## Report
- Summarize what was done: issue created, branch created, plan written, chore completed, all signalforge DevSecOps gates passed.
