...
## Usage
```
/icdev-maintain <project-id> [--auto] [--dry-run] [--offline]
```
...
### 1. Scan Dependencies
Use the `scan_dependencies` MCP tool from icdev-maintenance:
...
Use the `check_vulnerabilities` MCP tool from icdev-maintenance:
- project_id: from arguments
...
Use the `run_maintenance_audit` MCP tool from icdev-maintenance:
- project_id: from arguments
...
Use the `remediate` MCP tool from icdev-maintenance:
- project_id: from arguments
