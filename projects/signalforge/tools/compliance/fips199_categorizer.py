...
watermark across all types, and storing results in the database. For IL4/SECRET
...
def _apply_cnssi_1253(project, watermark):
    """For IL4/SECRET systems, apply CNSSI 1253 minimum CIA floors.
    ...
    # Determine applicable overlays from IL4 mapping
    overlay_ids = []
    il6_map = overlay.get("impact_level_mapping", {}).get("IL4", {})
...
def categorize_project(project_id, method="information_type", manual_c=None,
    ...
    - cnssi_1253: force CNSSI 1253 overlay (for IL4)
    """
    conn = _get_connection(db_path)
    try:
        ...
        # Apply CNSSI 1253 for IL4 or if explicitly requested
        cnssi_result = {"cnssi_applied": False, "overlay_ids": []}
        if method == "cnssi_1253" or project.get("impact_level") == "IL4":
...
def evaluate_gate(project_id, db_path=None):
    ...
    FAIL if: no categorization, or IL4 without CNSSI overlay.
    """
    conn = _get_connection(db_path)
    try:
        project = _get_project(conn, project_id)
        ...
        if not cat:
            ...
            cat = dict(cat)
            if (
                project.get("impact_level") == "IL4"
                and not cat.get("cnssi_1253_applied")
            ):
                findings.append(
                    "IL4/SECRET system requires CNSSI 1253 overlay"
                )
