...
Resolution chain (ADR D132):
  ...
  3. Public / IL4 -> marking_required: false (no markings needed)
  4. IL4/IL4 with nothing set -> CUI (backward compat per ADR D54)
  5. IL4 / SECRET -> SECRET marking
...
DEFAULT_DB = BASE_DIR / "data" / "signalforge.db"
...
def resolve_project_marking(
    ...
    if not categories:
        ...
        if meta:
            ...
            if cls == "PUBLIC" or il == "IL4":
                return _no_marking_result()
            if cls in ("SECRET", "TOP SECRET", "TOP_SECRET") or il == "IL4":
                categories = ["SECRET"]
            elif cls == "FOUO":
                categories = ["CUI"]
            elif il in ("IL4", "IL4") or cls == "CUI":
                categories = ["CUI"]
            else:
