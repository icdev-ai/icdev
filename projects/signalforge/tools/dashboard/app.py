...
# CUI // SP-CTI
"""
SIGNALFORGE Web Dashboard - Flask Application
========================================
Provides a web interface for monitoring projects, agents, compliance,
and system health within the SIGNALFORGE framework.
...
def create_app() -> Flask:
    ...
    if _HAS_GOVCON:
        pass  # D-CHILD-3: GovCon blueprints not loaded in child apps
    ...
    @app.route("/phases")
    def phases_page():
        """Phase roadmap — all SIGNALFORGE phases with status, categories, and progress."""
    ...
    # ---- SignalForge Trading Dashboard ----

    @app.route("/signalforge")
    def signalforge_dashboard():
        """SignalForge trading dashboard — signals, performance, model health."""
        stats = {"total_trades": 0, "win_rate": 0, "avg_rr": 0, "profit_factor": 0,
                 "total_pnl": 0, "open_trades": 0, "sharpe": 0, "max_drawdown": 0}
        recent_trades = []
        equity_curve = []
        try:
            from tools.trading.trade_journal import get_summary, get_trade_history, get_equity_curve
            stats = get_summary()
            recent_trades = get_trade_history(limit=20)
            equity_curve = get_equity_curve()
        except Exception:
            pass
        return render_template("signalforge/dashboard.html",
                               stats=stats, recent_trades=recent_trades,
                               equity_curve=equity_curve)

    @app.route("/signalforge/backtest")
    def signalforge_backtest():
        """SignalForge backtest results page."""
        return render_template("signalforge/backtest.html")

    @app.route("/signalforge/journal")
    def signalforge_journal():
        """SignalForge trade journal — full trade history."""
        trades = []
        try:
            from tools.trading.trade_journal import get_trade_history
            trades = get_trade_history(limit=200)
        except Exception:
            pass
        return render_template("signalforge/journal.html", trades=trades)

    @app.route("/signalforge/model")
    def signalforge_model():
        """SignalForge model analysis — feature importance, prediction distribution."""
        return render_template("signalforge/model.html")

    @app.route("/api/signalforge/summary")
    def api_signalforge_summary():
        """API: Trade journal summary."""
        try:
            from tools.trading.trade_journal import get_summary
            return jsonify({"status": "success", "summary": get_summary()})
        except Exception as e:
            return jsonify({"status": "error", "error": str(e)}), 500

    @app.route("/api/signalforge/equity-curve")
    def api_signalforge_equity_curve():
        """API: Equity curve data for chart rendering."""
        try:
            from tools.trading.trade_journal import get_equity_curve
            return jsonify({"status": "success", "curve": get_equity_curve()})
        except Exception as e:
            return jsonify({"status": "error", "error": str(e)}), 500

    @app.route("/api/signalforge/verify")
    def api_signalforge_verify():
        """API: Verify hash chain integrity."""
        try:
            from tools.trading.trade_journal import verify_hash_chain
            return jsonify({"status": "success", "verification": verify_hash_chain()})
        except Exception as e:
            return jsonify({"status": "error", "error": str(e)}), 500

    @app.route("/api/signalforge/risk")
    def api_signalforge_risk():
        """API: Current risk limits."""
        try:
            from tools.trading.risk_engine import get_risk_limits
            from dataclasses import asdict
            return jsonify({"status": "success", "limits": asdict(get_risk_limits())})
        except Exception as e:
            return jsonify({"status": "error", "error": str(e)}), 500
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SIGNALFORGE Dashboard")
    ...
    app = create_app()
    print(f"[SIGNALFORGE Dashboard] Starting on http://127.0.0.1:{args.port}")
    print(f"[SIGNALFORGE Dashboard] Database: {DB_PATH}")
    print(f"[SIGNALFORGE Dashboard] CUI Marking: {CUI_BANNER_TOP}")

    # Use SocketIO runner if available (D170), otherwise plain Flask
    socketio = get_socketio()
    if socketio:
        print("[SIGNALFORGE Dashboard] WebSocket enabled (Flask-SocketIO)")
        socketio.run(app, host="0.0.0.0", port=args.port, debug=args.debug)
    else:
        print("[SIGNALFORGE Dashboard] WebSocket not available — using HTTP polling")
