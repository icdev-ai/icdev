...
Completed runs are persisted to ``data/signalforge.db`` tables ``batch_runs``
...
Enhancements:
  ...
  - DB persistence: completed runs written to signalforge.db
...
def _persist_run(run: dict) -> None:
    """Write a completed run and its steps to signalforge.db for history."""
    conn = _get_db()
    try:
        conn.execute(
...
def history():
    """Return persisted batch run history from signalforge.db.
