...
Detects the current project (via signalforge.yaml or DB lookup), loads compliance
...
Detection order:
    1. signalforge.yaml in cwd → manifest_loader.load_manifest()
...
DB_PATH = BASE_DIR / "data" / "signalforge.db"
...
def build_session_context(directory: str = None, db_path: str = None) -> dict:
    ...
    Args:
        ...
        db_path: Path to signalforge.db (defaults to data/signalforge.db).
    ...
    if detection["source"] == "yaml":
        ...
        if detection.get("db_record"):
            ...
            context["warnings"].append(
                "Project found in signalforge.yaml but not registered in SIGNALFORGE database. "
                "Run `/signalforge-init` or `python tools/project/session_context_builder.py --init` "
                "to register."
            )
        # Manifest warnings
        ...
        context["warnings"].append(
            "No signalforge.yaml found and current directory is not a registered SIGNALFORGE project."
        )
...
def _detect_project(directory: str, db_path: str) -> dict:
    """Detect whether this directory is an SIGNALFORGE project.
...
def _suggest_workflows(context: dict) -> list:
    ...
    if context.get("setup_needed"):
        suggestions.append({
            "command": "/signalforge-init",
            "reason": "No project detected — initialize a new SIGNALFORGE project",
        })
        return suggestions
    ...
    if context.get("source") == "yaml" and not project.get("db_project_id"):
        suggestions.append({
            "command": "/signalforge-init",
            "reason": "signalforge.yaml found but project not registered in DB",
        })

    # No SSP generated
    if compliance.get("ssp_status") in (None, "not_generated"):
        suggestions.append({
            "command": "/signalforge-comply",
...
def _format_markdown(context: dict) -> str:
    ...
    if context.get("setup_needed"):
        lines.append("## SIGNALFORGE Project Context")
        lines.append("")
        lines.append("**No SIGNALFORGE project detected in this directory.**")
        lines.append("")
        lines.append("To get started:")
        lines.append("1. Create an `signalforge.yaml` manifest (see `docs/dx/signalforge-yaml-spec.md`)")
        lines.append("2. Run `/signalforge-init` to initialize the project")
        lines.append("")
    ...
    lines.append(f"## SIGNALFORGE Project Context")
    lines.append("")
...
def init_from_manifest(directory: str = None, db_path: str = None) -> dict:
    """Create a DB project record from signalforge.yaml.

    Bridges the gap between 'signalforge.yaml exists' and 'project is in the DB'.
    Also creates a dev profile if the manifest specifies a profile template.

    Args:
        directory: Project directory containing signalforge.yaml.
        db_path: Path to signalforge.db.
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO projects
               ...
               VALUES (?, ?, ?, ?, ?, 'active', ?, ?, 'signalforge-yaml',
                       ?, ?, ?, ?, ?, ?)""",
            (
        ...
        try:
            conn.execute(
                ...
                (
                    db_id,
                    f"Initialized project '{name}' from signalforge.yaml",
                    json.dumps({"manifest_path": str(cwd / "signalforge.yaml"), "impact_level": il}),
                    classification,
                ),
            )
        except Exception:
            ...
            try:
                conn.execute(
                    """INSERT INTO dev_profiles
                       ...
                       VALUES ('project', ?, 1, ?, '{}', 'signalforge-yaml', ?)""",
...
def main():
    ...
    parser.add_argument("--db", help="Path to signalforge.db")
    parser.add_argument("--format", choices=["markdown", "json"],
                        default="markdown", help="Output format")
    parser.add_argument("--json", action="store_true",
                        help="Output JSON (shortcut for --format json)")
    parser.add_argument("--init", action="store_true",
                        help="Initialize DB project record from signalforge.yaml")
