...
def generate_base(project_path: str, project_config: dict = None) -> list:
    ...
    project_name = config.get("project_name", "signalforge-project")
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws-us-gov:logs:${{data.aws_region.current.name}}:${{data.aws_caller_identity.current.account_id}}:log-group:/signalforge/*"
      }
    ]
  })
...
def generate_bedrock_iam(project_path: str, config: dict = None) -> list:
    ...
    Creates IAM role and policy that allows SIGNALFORGE agents to invoke
...
def main():
    ...
    parser.add_argument("--project-name", default="signalforge-project", help="Project name for resource naming")
