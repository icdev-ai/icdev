...
def generate_deployment(project_path: str, app_config: dict = None) -> list:
    ...
    name = config.get("name", "signalforge-app")
    ...
    deployment = {
        ...
        "metadata": {
            ...
            "annotations": {
                "classification": "CUI",
                "signalforge.io/generated": datetime.now(timezone.utc).isoformat(),
            },
        },
        "spec": {
            "replicas": replicas,
...
# ---------------------------------------------------------------------------
# Default agent definitions matching SIGNALFORGE multi-agent architecture
DEFAULT_AGENTS = [
...
def generate_agent_deployments(project_path: str, blueprint: dict = None) -> list:
    ...
    Creates a Deployment and Service for each agent in the SIGNALFORGE multi-agent
    ...
    namespace = config.get("namespace", "signalforge-agents")
    registry = config.get("registry", "registry.example.com/signalforge")

    k8s_dir = Path(project_path) / "k8s" / "agents"
    files = []

    for agent in agents:
        agent_name = f"signalforge-{agent['name']}"
        port = agent["port"]
        ...
        deployment = {
            ...
            "metadata": {
                ...
                "labels": _labels(agent_name, {
                    ...
                    "signalforge.io/tier": "agent",
                }),
                "annotations": {
                    "classification": "CUI",
                    "signalforge.io/generated": datetime.now(timezone.utc).isoformat(),
                    "signalforge.io/agent-port": str(port),
                },
            },
            "spec": {
                "replicas": 2,
    ...
    for agent in agents:
        ...
        hpa = {
            ...
            "metadata": {
                ...
                "namespace": namespace,
                "labels": _labels(agent_name, {
                    "signalforge.io/scaling-tier": "core" if name_key in core_agents
                    else "domain" if name_key in domain_agents
                    else "support",
                }),
                "annotations": {
                    "classification": "CUI",
                    "signalforge.io/scaling-profile": "predictive",
                },
            },
            "spec": {
                "scaleTargetRef": {
...
def main():
    ...
    parser.add_argument("--name", default="signalforge-app", help="Application name")
