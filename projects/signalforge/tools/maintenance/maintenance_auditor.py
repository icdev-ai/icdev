...
def _log_audit_event(conn, project_id, action, details, file_path=None):
    ...
    try:
        conn.execute(
            ...
            (project_id, "maintenance_audit", "signalforge-maintenance-engine", action,
...
def _parse_simple_yaml(path):
    """Minimal stdlib YAML parser for SIGNALFORGE config files (2-level nesting)."""
    if not path.exists():
        return {}
...
def _generate_audit_report(audit_data, output_dir, project_name):
    ...
    L += ["## NIST 800-53 Control Mapping", "",
          ...
          "---", "", f"*Generated by SIGNALFORGE Maintenance Auditor on {ts}*", "",
