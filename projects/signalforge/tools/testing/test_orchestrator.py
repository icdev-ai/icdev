# [TEMPLATE: CUI // SP-CTI]
# SIGNALFORGE Test Orchestrator
# Adapted from ADW adw_test.py — retry logic, resolution, E2E coordination

"""
SIGNALFORGE Test Orchestrator — runs unit, BDD, E2E, security, and compliance tests
...
def run_bandit(project_dir: str, logger) -> TestResult:
    ...
    Adapted from ADW test.md security scan step + SIGNALFORGE security tools.
    """
...
def run_tests_with_resolution(
    ...
    while attempt < max_attempts:
        ...
        # In SIGNALFORGE, resolution would invoke the builder agent to fix code
...
def evaluate_security_gate(project_dir: str, logger) -> GateEvaluation:
    """Evaluate security gate by running SIGNALFORGE security scans."""
    ...
    return GateEvaluation(
        ...
        evaluated_by="signalforge-test-orchestrator",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SIGNALFORGE Test Orchestrator")
    parser.add_argument("--project-dir", required=True, help="Path to project under test")
    parser.add_argument("--project-id", help="SIGNALFORGE project UUID")
    ...
    logger.info(f"SIGNALFORGE Test Orchestrator starting — Run ID: {run_id}")
