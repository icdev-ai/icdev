# [TEMPLATE: CUI // SP-CTI]
# SIGNALFORGE System Health Check
# Adapted from ADW health_check.py for Gov/DoD environment validation

"""
SIGNALFORGE Health Check — validates the entire SIGNALFORGE system is operational.

Usage:
    python tools/testing/health_check.py [--json] [--project-id <id>]

Checks performed:
1. Environment variables (SIGNALFORGE, AWS, optional keys)
2. Database connectivity (signalforge.db — 28 tables)
...
def check_env_vars() -> CheckResult:
    ...
    required_vars = {
        "ICDEV_DB_PATH": "Path to SIGNALFORGE database (default: data/signalforge.db)",
    }
    ...
    for var, desc in required_vars.items():
        ...
        if not val:
            ...
            if var == "ICDEV_DB_PATH":
                default_path = PROJECT_ROOT / "data" / "signalforge.db"
...
def check_database() -> CheckResult:
    """Check SIGNALFORGE database connectivity and table structure."""
    db_path = os.getenv("ICDEV_DB_PATH", str(PROJECT_ROOT / "data" / "signalforge.db"))
...
def check_tools() -> CheckResult:
    """Check that core SIGNALFORGE tool modules are importable."""
    tool_modules = {
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="SIGNALFORGE System Health Check")
