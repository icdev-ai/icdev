...
def generate_signing_config(project_id: str, profile: dict = None) -> dict:
    ...
    signing_config = {
        ...
        "key_management": {
            "provider": "aws_kms",
            "kms_key_alias": f"alias/signalforge-{project_id}-signing",
    ...
    verification_policy = {
        ...
        "metadata": {
            ...
            "labels": {
                ...
                "signalforge.mil/component": "devsecops-attestation",
            },
        },
        "spec": {
            "images": [{"glob": "**"}],
            "authorities": [{
                "key": {
                    "kms": f"awskms:///alias/signalforge-{project_id}-signing",
                },
            }],
        },
    }
...
attestation:sign-image:
  ...
  variables:
    COSIGN_KMS_KEY: "signalforge-${CI_PROJECT_NAME}-signing"
""")
    ...
    for policy in expected_policies:
        if policy == "image_signature":
            verification_commands.append(
                f"cosign verify --key awskms:///alias/signalforge-{project_id}-signing {image_ref}"
            )
        elif policy == "sbom_cyclonedx":
            verification_commands.append(
                f"cosign verify-attestation --key awskms:///alias/signalforge-{project_id}-signing "
