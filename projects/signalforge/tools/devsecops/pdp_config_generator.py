...
to external Policy Decision Points. SIGNALFORGE does NOT implement PDP logic itself.

ADR D124: PDP modeled as external reference (Zscaler, Palo Alto, DISA ICAM, CrowdStrike) —
SIGNALFORGE generates PEP configs (Istio AuthorizationPolicy) but does NOT implement PDP itself.
...
DB_PATH = Path(os.environ.get("ICDEV_DB_PATH", str(BASE_DIR / "data" / "signalforge.db")))
...
# ---------------------------------------------------------------------------
# PDP reference generation (D124: SIGNALFORGE documents but does NOT implement PDP)
# ---------------------------------------------------------------------------

def generate_pdp_reference(project_id: str, pdp_type: str) -> dict:
    """Document external PDP integration point for a project.

    SIGNALFORGE does not implement PDP logic. This function generates documentation
    ...
    ADR D124: PDP is an external reference. SIGNALFORGE generates PEP configs only.
    ...
    return {
        ...
        "adr_reference": "ADR D124: PDP is external — SIGNALFORGE generates PEP configs only",
        "generated_at": now,
    ...
    configs = {
        "disa_icam": {
            ...
            "connection_details": {
                ...
                "audience": f"signalforge-{project.get('name', 'app')}.mil",
            },
            "k8s_integration": {
    ...
    docs = {
        "disa_icam": {
            "summary": "DISA ICAM (Identity, Credential, and Access Management) provides DoD-wide identity services. "
                       "SIGNALFORGE generates Istio/Linkerd PEP configurations that delegate authorization decisions "
        ...
        "zscaler": {
            "summary": "Zscaler Private Access (ZPA) provides Zero Trust Network Access. "
                       "SIGNALFORGE generates K8s Connector DaemonSet manifests and PEP network policies "
        ...
        "palo_alto_prisma": {
            "summary": "Palo Alto Prisma Access combines ZTNA and cloud-delivered NGFW. "
                       ...
                       "SIGNALFORGE generates Defender DaemonSet manifests and PEP policies. "
        ...
        "crowdstrike": {
            "summary": "CrowdStrike Falcon provides device trust and endpoint detection. "
                       "SIGNALFORGE generates Falcon Sensor DaemonSet manifests and device posture "
        ...
        "microsoft_entra": {
            "summary": "Microsoft Entra ID (formerly Azure AD) provides cloud identity and conditional access. "
                       "SIGNALFORGE generates Istio/Linkerd PEP configurations that validate Entra ID JWT tokens "
                       "and enforce conditional access policies. Phishing-resistant MFA and device compliance "
                       "checks are enforced through Entra conditional access — not by SIGNALFORGE.",
            "customer_responsibilities": [
        ...
        "custom": {
            "summary": "Customer-provided PDP integration. SIGNALFORGE generates PEP configurations "
...
def generate_pep_config(project_id: str, mesh: str = "istio", pdp_type: str = "disa_icam") -> dict:
    ...
    The PEP enforces decisions made by the external PDP — SIGNALFORGE does not implement
    ...
    result["adr_reference"] = "ADR D124: PEP generated by SIGNALFORGE; PDP is external"
...
def _generate_istio_pep(project_id: str, pdp_type: str, project: dict) -> dict:
    ...
    namespace = f"signalforge-{project_id[:8]}"
    ...
    authz_policy = {
        ...
        "metadata": {
            ...
            "namespace": namespace,
            "labels": {
                "app.kubernetes.io/managed-by": "signalforge",
                "signalforge.mil/project-id": project_id,
                "signalforge.mil/classification": project.get("classification", "CUI"),
                "signalforge.mil/pdp-type": pdp_type,
            },
            "annotations": {
                "signalforge.mil/adr": "D124 — PDP is external; this policy is the PEP",
                "signalforge.mil/generated-at": datetime.now(timezone.utc).isoformat(),
            },
        },
        "spec": {
            "action": "CUSTOM",
...
def _generate_linkerd_pep(project_id: str, pdp_type: str, project: dict) -> dict:
    ...
    server_authz = {
        ...
        "metadata": {
            ...
            "annotations": {
                "signalforge.mil/adr": "D124 — PDP is external; this is the PEP config",
                "signalforge.mil/pdp-reference": _get_grpc_service(pdp_type),
                "signalforge.mil/generated-at": datetime.now(timezone.utc).isoformat(),
            },
        },
        "spec": {
    ...
    auth_policy = {
        ...
        "metadata": {
            ...
            "annotations": {
                "signalforge.mil/adr": "D124 — external PDP reference",
            },
        },
        "spec": {
            "targetRef": {
...
def generate_device_trust_config(project_id: str, mdm_type: str = "crowdstrike") -> dict:
    ...
    SIGNALFORGE generates the K8s manifests and configuration references — the
    ...
    return {
        ...
        "enforcement_model": (
            ...
            "SIGNALFORGE generates K8s admission webhook configs and PEP policies "
            "that query the MDM API at admission time."
        ),
        "adr_reference": "ADR D124: device trust PDP is external — SIGNALFORGE generates PEP integration configs",
        "generated_at": now,
    }
...
def _build_device_trust_manifests(project_id: str, mdm_type: str, project: dict) -> dict:
    ...
    kyverno_device_policy = {
        ...
        "metadata": {
            ...
            "annotations": {
                ...
                "signalforge.mil/adr": "D124 — device trust PDP is external",
            },
            "labels": {
                "app.kubernetes.io/managed-by": "signalforge",
                "signalforge.mil/project-id": project_id,
            ...
            "rules": [
                {
                    ...
                    "validate": {
                        "message": (
                            ...
                            f"(signalforge.mil/device-trust-verified: 'true')"
                        ),
                        "pattern": {
                            "metadata": {
                                "annotations": {
                                    "signalforge.mil/device-trust-verified": "true",
                                    "signalforge.mil/device-trust-source": f"{mdm_type}",
                                }
                            }
                        },
                    },
    ...
    secret_template = {
        ...
        "metadata": {
            ...
            "annotations": {
                "signalforge.mil/secret-source": "aws-secrets-manager",
                "signalforge.mil/do-not-commit": "true — populate from AWS Secrets Manager ExternalSecret",
            },
        },
        "type": "Opaque",
        "stringData": {
...
def _build_posture_policy(mdm_type: str, project: dict) -> dict:
    ...
    return {
        ...
        "check_sequence": [
            ...
            "4. Annotate pod with device trust result (signalforge.mil/device-trust-verified)",
        ...
        if args.device_trust:
            ...
            if pep:
                ...
                for note in pep.get("integration_notes", [])[:3]:
                    print(f"  Note:     {note}")
            print("ADR:        ADR D124 — PDP is external; SIGNALFORGE generates PEP configs only")
