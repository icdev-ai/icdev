...
Writes spans to the `otel_spans` table in signalforge.db.
...
logger = logging.getLogger("signalforge.observability.sqlite_tracer")

BASE_DIR = Path(__file__).resolve().parent.parent.parent
DEFAULT_DB_PATH = BASE_DIR / "data" / "signalforge.db"
...
class SQLiteTracer(Tracer):
    ...
    Writes completed spans to the `otel_spans` table in signalforge.db.
    ...
    def start_span(
        ...
        attrs = {}
        if self._agent_id:
            attrs["signalforge.agent_id"] = self._agent_id
        if self._project_id:
            attrs["signalforge.project_id"] = self._project_id
        if attributes:
                ...
                try:
                    conn.execute(
                        ...
                        (
                            ...
                            span.attributes.get("signalforge.agent_id", self._agent_id),
                            span.attributes.get("signalforge.project_id", self._project_id),
