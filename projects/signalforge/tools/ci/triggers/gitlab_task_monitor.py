# [TEMPLATE: CUI // SP-CTI]
"""GitLab Task Board Monitor — polls issues for {{signalforge: workflow}} tags.

Decision D33: GitLab tags replace Notion's {{prototype: type}} pattern.

Supported tags:
    {{signalforge: intake}}     -> Start RICOAS intake session
    {{signalforge: build}}      -> Run TDD build workflow
    {{signalforge: sdlc}}       -> Full Plan->Build->Test->Review
    {{signalforge: comply}}     -> Generate compliance artifacts
    {{signalforge: secure}}     -> Run security scanning
    {{signalforge: modernize}}  -> Legacy app modernization
    {{signalforge: deploy}}     -> Generate IaC and deploy
    {{signalforge: maintain}}   -> Maintenance audit
...
TAG_PATTERN = re.compile(r"\{\{signalforge:\s*(\w+)\}\}", re.IGNORECASE)

# Bot marker to prevent loops
BOT_MARKER = "[SIGNALFORGE-BOT]"


def extract_icdev_tag(text: str) -> Optional[str]:
    """Extract {{signalforge: workflow}} tag from issue body or comment."""
    if not text:
        return None
...
def list_open_issues(label: str = "signalforge") -> list:
    """List open GitLab issues with the signalforge label."""
    try:
    ...
    while running:
        try:
            ...
            for issue in issues:
                ...
                print(f"[GitLab Task Monitor] Found: #{iid} '{title}' -> {{{{signalforge: {tag}}}}}")

                # Claim the issue
                claim_issue(iid, web_url, tag)

                # Add processing label
                add_label(iid, "signalforge-processing")
...
def main():
    parser = argparse.ArgumentParser(description="SIGNALFORGE GitLab Task Board Monitor")
    ...
    if args.once:
        ...
        for issue in issues:
            ...
            tag = extract_icdev_tag(body)
            if tag:
                print(f"Issue #{issue.get('iid')}: {issue.get('title')} -> {{{{signalforge: {tag}}}}}")
    else:
