# [TEMPLATE: CUI // SP-CTI]
# SIGNALFORGE Webhook Server — GitHub + GitLab + Slack + Mattermost webhook support
# Refactored to use EventEnvelope + EventRouter (D132, D133)

"""
Webhook server for SIGNALFORGE CI/CD — receives events from GitHub, GitLab,
...
def _post_ack_comment(envelope: EventEnvelope, result: dict):
    ...
    try:
        ...
        vcs.comment_on_issue(
            ...
            f"{BOT_IDENTIFIER} SIGNALFORGE Webhook: Detected `{workflow}` workflow\n\n"
            f"Run ID: `{run_id}`\n"
...
def health():
    ...
    try:
        ...
        return jsonify({
            ...
            "service": "signalforge-webhook-server",
...
if __name__ == "__main__":
    print("CUI // SP-CTI")
    print(f"Starting SIGNALFORGE Webhook Server on port {PORT}")
