# [TEMPLATE: CUI // SP-CTI]
# SIGNALFORGE Poll Trigger — Cron-based issue polling for GitHub + GitLab
# Refactored to use EventEnvelope + EventRouter (D132, D133)

"""
Cron-based SIGNALFORGE trigger that polls GitHub/GitLab issues.

Polls every 20 seconds to detect:
1. New issues without comments
2. Issues where the latest comment contains 'signalforge'
...
def should_process_issue(vcs: VCS, issue_number: int) -> tuple:
    ...
    # Check if latest comment is exactly 'signalforge'
    if comment_lower == "signalforge":
        print(f"INFO: Issue #{issue_number} — latest comment is 'signalforge'")
...
def main():
    ...
    print("CUI // SP-CTI")
    print("INFO: Starting SIGNALFORGE Poll Trigger (EventRouter-based)")
