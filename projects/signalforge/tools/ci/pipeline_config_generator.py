#!/usr/bin/env python3
# CUI // SP-CTI
"""Generate CI/CD pipeline configs from signalforge.yaml (D192).
...
generates 9-stage GitLab CI for the SIGNALFORGE framework itself). This generates
lightweight customer project pipelines from declarative signalforge.yaml.
...
CUI_HEADER = "# CUI // SP-CTI — Auto-generated by SIGNALFORGE pipeline_config_generator"
...
def generate_pipeline(
    ...
    """Generate CI/CD pipeline config from signalforge.yaml.
    ...
    if not manifest["valid"] and not manifest["raw"]:
        result["errors"].append("No valid signalforge.yaml found")
        return result
    ...
    project_name = config.get("project", {}).get("name", "signalforge-project")
    ...
    if platform == "github":
        ...
        output_path = cwd / ".github" / "workflows" / "signalforge.yml"
    else:
...
def _generate_github_workflow(
    ...
    lines = [CUI_HEADER, ""]
    lines.append(f"name: SIGNALFORGE Compliance Pipeline — {project_name}")
    lines.append("")
    lines.append("on:")
    ...
    if on_pr:
        lines.append("jobs:")
        lines.append("  signalforge-pr-checks:")
        ...
        lines.append("")
        lines.append("      - name: Install SIGNALFORGE dependencies")
        lines.append("        run: pip install -r requirements.txt")
        lines.append("")
        lines.append("      - name: Initialize SIGNALFORGE database")
    ...
    if on_merge:
        if not on_pr:
            lines.append("jobs:")
        lines.append("  signalforge-merge-artifacts:")
        ...
        lines.append("        run: pip install -r requirements.txt")
        lines.append("")
        lines.append("      - name: Initialize SIGNALFORGE")
        lines.append("        run: python tools/db/init_icdev_db.py")
        lines.append("")
    ...
    # Scheduled job
    if on_schedule:
        lines.append("  signalforge-scheduled:")
...
def _generate_gitlab_ci(
    ...
    # Setup anchor
    lines.append(".signalforge-setup: &signalforge-setup")
    ...
    for check_name in on_pr:
        ...
        lines.append("  <<: *signalforge-setup")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate CI/CD pipeline configs from signalforge.yaml (D192)"
    )
