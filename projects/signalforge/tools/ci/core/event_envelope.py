# [TEMPLATE: CUI // SP-CTI]
# SIGNALFORGE Event Envelope — unified event format for all CI/CD triggers (D132)
...
class EventEnvelope:
    ...
    def _extract_command(text: str) -> tuple:
        """Extract signalforge workflow command and run_id from text.
    ...
    def _check_bot(text: str, author: str = "") -> bool:
        ...
        if BOT_IDENTIFIER in text:
            return True
        if author and author.lower() in ("signalforge-bot", "signalforge"):
            return True
        return False
    ...
    def from_gitlab_tag(cls, issue_data: dict, icdev_tag: str) -> "EventEnvelope":
        """Create envelope from a GitLab {{signalforge: tag}} issue."""
