# [TEMPLATE: CUI // SP-CTI]
# SIGNALFORGE Event Router — central routing with lane-aware session queue (D133)
...
class EventRouter:
    ...
    def route(self, envelope: EventEnvelope) -> dict:
        ...
        if not workflow:
            # Check if content contains "signalforge" keyword (generic trigger)
            if "signalforge" in envelope.content.lower():
