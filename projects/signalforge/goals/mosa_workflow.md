...
**Why this matters:** 10 U.S.C. Section 4401 mandates MOSA for all major defense acquisition programs. DoDI 5000.87 reinforces this for software-intensive systems. Systems that fail to demonstrate modular design, open interfaces, and published standards risk acquisition milestone disapproval, vendor lock-in, and inability to integrate with future DoD enterprise services. SIGNALFORGE auto-enforces MOSA so compliance is continuous, not a last-minute documentation exercise.
...
- [ ] SIGNALFORGE database initialized (`python tools/db/init_icdev_db.py`)
...
- **Manual trigger:** Customer or ISSO explicitly requests MOSA assessment via `/signalforge-mosa`
...
Evaluate MOSA gate criteria. This gate integrates with the existing SIGNALFORGE security gate framework (`args/security_gates.yaml`).
...
1. **Non-DoD project:** MOSA is not required. Workflow skips entirely. Modularity analysis still available on-demand via `/signalforge-mosa` for any project wanting to improve architecture quality.
...
- `/signalforge-mosa` — Run full MOSA workflow (Steps 1-8)
- `/signalforge-comply` — Includes MOSA in multi-regime assessment when applicable
- `/signalforge-intake` — Detects MOSA signals during conversational intake
...
- **Database:** `data/signalforge.db` (mosa_assessments, mosa_modularity_metrics, mosa_icds, mosa_tsp tables)
