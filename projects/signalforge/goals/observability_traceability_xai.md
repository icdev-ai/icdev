...
## Purpose
Provide full observability into SIGNALFORGE's 15-agent architecture through distributed tracing,
...
- XAI assessment triggered via `/signalforge-trace` skill or `xai_assess` MCP tool
...
### 8. MCP Server
`signalforge-observability` MCP server provides 6 tools + 2 resources:
