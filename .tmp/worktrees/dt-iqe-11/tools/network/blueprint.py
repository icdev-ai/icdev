...
Output ONLY the JSON object. No other text."""
    ...
    _IQE_QUERIES_DIR = _ICDEV_ROOT / "context" / "iqe" / "queries" / "network"

    @bp.route("/iqe")
    @nc_login_required
    def nc_iqe_page():
        """IQE — ICDEV Query Engine dashboard for the NDC."""
        queries = []
        if _IQE_QUERIES_DIR.exists():
            for f in sorted(_IQE_QUERIES_DIR.glob("*.iqe")):
                queries.append({"name": f.stem.replace("_", " ").title(), "filename": f.name})
        return render_template(
            "network/iqe.html",
            queries=queries,
            classification_banner=NC_CONFIG.get("app", {}).get("classification", ""),
    ...
    @bp.route("/api/iqe/queries")
    def nc_iqe_queries():
        """List available .iqe query files with their source."""
        queries = []
        if _IQE_QUERIES_DIR.exists():
            for f in sorted(_IQE_QUERIES_DIR.glob("*.iqe")):
                body = f.read_text(encoding="utf-8")
                queries.append({"name": f.stem.replace("_", " ").title(), "filename": f.name, "body": body})
        return jsonify({"ok": True, "queries": queries, "count": len(queries)})

    @bp.route("/api/iqe/seed", methods=["POST"])
    def nc_iqe_seed():
        """Seed test NDC device fixtures into ni_devices for E2E testing."""
        import uuid as _uid_mod

        conn = get_connection()
        fixtures = [
            ("rtr-01", "Router", "Cisco", "ISR4451", "IOS-XE 17.6"),
            ("rtr-02", "Router", "Juniper", "MX480", "JunOS 22.2"),
            ("sw-01", "Switch", "Cisco", "C9300", "IOS-XE 17.6"),
            ("fw-01", "Firewall", "Palo Alto", "PA-3220", "PAN-OS 11.1"),
            ("srv-01", "Server", "Dell", "PowerEdge R750", "RHEL 9.2"),
        ]
        inserted = 0
        for label, dev_type, vendor, model, fw in fixtures:
            row_id = str(_uid_mod.uuid4())
            try:
                conn.execute(
                    "INSERT OR IGNORE INTO ni_devices "
                    "(id, label, device_type, vendor, model, firmware_version) "
                    "VALUES (?,?,?,?,?,?)",
                    (row_id, label, dev_type, vendor, model, fw),
                )
                inserted += 1
            except Exception:
                pass

        # Seed bgp_peers for bgp_peer_asymmetry.iqe E2E query (non-established sessions)
        try:
            conn.execute(
                "CREATE TABLE IF NOT EXISTS bgp_peers ("
                "id TEXT PRIMARY KEY, device TEXT, peer_ip TEXT, "
                "local_as INTEGER, remote_as INTEGER, session_state TEXT)"
            )
            bgp_fixtures = [
                ("bgp-e2e-01", "rtr-01", "10.0.0.2", 65001, 65002, "idle"),
                ("bgp-e2e-02", "rtr-01", "10.0.0.3", 65001, 65003, "active"),
            ]
            for bid, dev, pip, las, ras, state in bgp_fixtures:
                conn.execute(
                    "INSERT OR IGNORE INTO bgp_peers "
                    "(id, device, peer_ip, local_as, remote_as, session_state) "
                    "VALUES (?,?,?,?,?,?)",
                    (bid, dev, pip, las, ras, state),
                )
        except Exception:
            pass

        conn.commit()
        return jsonify({"ok": True, "seeded": inserted})

    @bp.route("/api/iqe/run", methods=["POST"])
    def nc_iqe_run():
        """Parse and execute an IQE query file against the network DB."""
        data = request.get_json(force=True) or {}
        filename = (data.get("filename") or "").strip()
        if not filename or not filename.endswith(".iqe"):
            return jsonify({"ok": False, "error": "filename required (.iqe)"}), 400
        # Prevent path traversal
        qpath = (_IQE_QUERIES_DIR / filename).resolve()
        if _IQE_QUERIES_DIR.resolve() not in qpath.parents:
            return jsonify({"ok": False, "error": "Invalid filename"}), 400
        if not qpath.exists():
            return jsonify({"ok": False, "error": f"Query not found: {filename}"}), 404
        try:
            ...
            def _devices_adapter(_c):
                rows = conn.execute(
                    "SELECT label, vendor, model, firmware_version FROM ni_devices LIMIT 500"
                ).fetchall()
                return [
                    {"hostname": r[0], "vendor": r[1], "model": r[2], "os_version": r[3]}
                    for r in rows
                ]

            ex = Executor()
            ex.register_collection("network.devices", _devices_adapter)
            results = ex.run(ast, conn=conn)
            return jsonify({"ok": True, "results": results, "count": len(results)})
        except Exception as exc:
            logger.exception("IQE run error: %s", exc)
